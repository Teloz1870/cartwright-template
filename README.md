...
```bash
npx create-cartwright my-store   # scaffold + install + db setup (prints your admin login)
cd my-store
...
> `DATABASE_URL=file:./dev.db`, then `npm run db:setup && npm run dev`. `db:setup` creates the schema +
> seeds in one step and auto-falls-back via the libSQL client if `prisma db push` hits the intermittent
> Prisma 7.8 `Schema engine error:` — so first-run can't get stuck.
