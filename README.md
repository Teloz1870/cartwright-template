...
- **AI**: storefront assistant, voice/vision shopping, MCP server, Agentic Commerce
  Protocol (ACP) endpoints, SEO/GEO autopilot.
...
npx prisma db push             # create the schema in your dev DB
npx prisma db seed             # seed demo data (reads brand.industryTemplate)
npm run dev                    # http://localhost:3000 — first visit → /admin/setup
```

> First visit to `/admin` (with an empty DB) launches a setup wizard: brand identity →
> theme palette → AI key → email/sender → first category. After that, manage products and
> integrations (Stripe, Resend, Gemini, Sentry) from the admin panel.

> **New here?** [`docs/getting-started.md`](./docs/getting-started.md) walks you from scaffold to
> your first admin login — including where your admin password comes from (the seed prints it once
> and saves it to a gitignored `.admin-credentials`) and why email is an optional, separate step.
