...
> **⚠️ Før du går live med rigtige kunder** — tre ting der IKKE må glemmes:
> 1. **Produktionsdatabase**: sæt `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN`. Den
>    lokale `file:./dev.db` er kun til dev — appen nægter at starte i production
>    uden Turso (data tabes ellers ved hvert deploy).
> 2. **Stripe LIVE-keys** (`sk_live_…`), ikke test-keys — ellers tages der ikke imod rigtige betalinger.
> 3. **Resend API-key** — ellers sendes der ingen ordrebekræftelser i production.
>
> Hele go-live-stien står i [`DEPLOY.md`](./DEPLOY.md).

## Cartwright 2.0 Features

- **In-House AI Vibe Designer**: Integreret Vibe Sandkasse i admin-panelet der lader dig bygge og opdatere sider via natural language prompts, uden at kende HTML/CSS.
- **B2B Service Catalog**: Indbygget ydelseskatalog for SaaS og Agency-sites, med lækkert Bento Grid display og dynamiske templates.
- **Auto-Lokalisering**: i18nexus cloud integration og indbygget Gemini-Flash baseret oversættelses-knap for produkter.
- **The Golden Stack**: Next.js 16 (App Router, Server Actions, Turbopack), Prisma, Tailwind v4.

## Stack

- **Next.js 16** (App Router, Server Actions, Turbopack)
- **Prisma + Turso libSQL** (production — managed, skalerbar) / **SQLite-fil** (kun lokal dev)
...
| Variabel | Hvor | Hvad |
|---|---|---|
| `DATABASE_URL` | dev | SQLite-fil til lokal udvikling (`file:./dev.db`) |
| `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN` | **prod** | Produktionsdatabase (Turso libSQL). Påkrævet i prod — appen nægter at starte uden (se DEPLOY.md §2) |
...
| `TURSO_DATABASE_URL/AUTH_TOKEN` | Dev: fallback til lokal SQLite. **Prod: appen nægter at starte** (medmindre `ALLOW_SQLITE_IN_PRODUCTION=1`) |
