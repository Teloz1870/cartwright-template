...
full release notes are in the engine's own
[CHANGELOG](https://github.com/Teloz1870/cartwright-template/blob/main/CHANGELOG.md). Cartwright's
release history and its [`/built-with-cartwright`](https://cartwright.app) capability tour belong to
the engine, not to your project: `scaffold/manifest.json` → `engineOnly` marks them, and the
scaffolder removes them from create-cartwright 2.10. A scaffold cut before that still has them on
disk — a `site` scaffold does not link them anywhere, a `light` or `full` one does — so until 2.10
they are yours to delete: [how, per profile](./docs/simple-site.md#removing-the-stock-pages).
(`light` is the one exception worth naming: its own prune list already drops `/priser` and `/cases`,
so it carries eight of the ten paths rather than all ten.)
