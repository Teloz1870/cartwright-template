...
/**
 * The hero's secondary CTA points at `/start` — Cartwright's own onboarding
 * funnel, listed in `scaffold/engine-only.ts` and deleted from a customer's
 * scaffold. `Boolean(...)`, never `=== true` (the static twin is `as const`
 * false → TS2367 in a materialised scaffold, #550/#551/#572). True in the
 * engine, so the canaries render byte-identically.
 */
 ...
 * fra import.
 */
export default function CorporateBaselineHomepage({ settings, genome }: DesignHomepageProps) {
  const t = useTranslations("WebsiteHome");
  const containerRef = useRef<HTMLDivElement>(null);
  // Voice/Genome hero copy (genomeResolve flag). settings (admin) wins; then the
  // Voice headline; else the i18n fallback below → byte-identical when flag off.
  const heroHeadline = settings?.websiteHeadline || genome?.hero.headline;
    ...
    <div ref={containerRef} className="min-h-screen bg-sol-cream">
      ...
      <div className="relative h-[80vh] w-full flex items-end pb-24 sm:pb-32 bg-[#050A19] overflow-hidden">
        ...
        <div className="container relative mx-auto px-4 sm:px-8 z-20">
          ...
          >
            {settings?.tagline ?? genome?.hero.tagline ?? brand.uiLabels.heroSubtagline}
          </motion.p>
