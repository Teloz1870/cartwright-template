...
export const DESIGN_OPTIONS: DesignOption[] = [
  ...
  // ───── Cartwright Studio premium designs (sketch towards v0.8.0 marketplace)
  {
    ...
    description:
      "Cinematic dark-roast storefront: locked espresso palette, copper accents, full-bleed video hero, Fraunces display type. Makes a small catalogue feel like a collection.",
    mode: "webshop",
    premium: true,
  },
  {
 ...
 *
 * Client-safe (pure data). Who actually reads it, exhaustively: this file
 * (`designIsMixable` → `resolveMixable` → `designTracksPalette`, which drives
 * the mixer-preview locked-look notice) and `isChromeSelectable`
 * (lib/builder/chrome-catalog.ts) — the latter reaching the admin CHROME
 * picker, `chrome.set`, the mixer-preview chrome params and the render path.
 * Until 2026-08 this comment instead named "the Mixer, the Parts gallery
 * manifest, and the admin part-picker to grey out Parts on a non-mixable
 * skin". None of those descriptions holds: the manifest carries no
 * design-level mixability at all (its `mixable` is the CHROME meta's field, a
 * different object), and nothing greys out builder Parts on it — every reader
 * that does consult it goes through `isChromeSelectable` and is about chrome.
 *
 * And it DOES reach the render path, contrary to the same old comment:
 * `getActiveChromeConfig` (lib/theme.ts) runs the `isChromeSelectable` check on
 * every storefront request to decide whether the shop's saved chrome Part still
 * applies. Adding or removing a slug here therefore changes which header/footer
 * such a shop renders — it is not cosmetic metadata. (A shop that has saved no
 * chrome Part at all is unaffected either way: there is nothing to re-check.)
 *
 ...
 * back to the built-in slug set above. Client-safe/pure-data, like
 * `designIsMixable` — and, like it, consulted on the render path through
 * `isChromeSelectable` (see the note above).
 */
