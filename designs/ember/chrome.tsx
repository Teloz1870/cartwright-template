 ...
 * shop sells (isEcommerce — the guarded identity predicate from lib/mode.ts,
 * so website-mode can never show cart UI), the header keeps the storefront
...
import { isEcommerce } from "@/lib/mode";
...
export function EmberHeader({ locale }: DesignChromeProps) {
  const home = `/${locale}`;
  const webshop = isEcommerce(brand);
  return (
    <header
