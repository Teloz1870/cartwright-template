...
/**
 * `/about` belongs to the `trust-pages` module — opt-in in the `site` profile
 * (owner decision 2026-09-07, CW-23). The hero's secondary button falls back
 * to it whenever the agent interfaces are off, which is exactly the profile
 * least likely to have the page: measured on a materialised `--profile site`
 * tree, "Read about this site" pointed at a 404.
 *
 * `Boolean(...)`, never `=== true` — TS2367 against the `as const` static twin
 * (#550/#551/#572). True in the engine, so the canaries are byte-identical.
 */
        ...
        <div className="agentic-container agentic-hero__content">
          ...
          <div className="agentic-hero__actions">
            ...
            {agentInterfaceEnabled ? (
              ...
              <Link href={`${home}/about`} className="agentic-button agentic-button--ghost">{t.secondaryDisabled}</Link>
            ) : null}
          </div>
