...
/**
 * `/about` and `/privacy` belong to the `trust-pages` module — opt-in in the
 * `site` profile (owner decision 2026-09-07, CW-23). This pack is NOT one of
 * the packs the materializer de-registers, so its chrome renders on every
 * page of a site scaffold that selects it: measured on a materialised
 * `--profile site` tree, the header linked `/en/about` and the footer linked
 * `/en/about` + `/en/privacy`, all three 404 (the same class as #572, one
 * design pack further in).
 *
 * `Boolean(...)`, never `=== true` — the static twin is `as const` literal
 * `false`, so an equality comparison is TS2367 in a materialised scaffold
 * (#550/#551/#572). True in the engine tree and in every database-backed
 * profile, so the canaries render byte-identically.
 */
    ...
    <header className="agentic-header">
      <nav aria-label="Primary navigation" className="agentic-container agentic-nav">
        ...
        <div className="agentic-nav__links">
          ...
          {publicAgentApi ? <Link href={`${home}/developers`}>Developers</Link> : trustPages ? <Link href={`${home}/about`}>About</Link> : null}
        </div>
  ...
  return (
    <footer className="agentic-footer">
      <div className="agentic-container agentic-footer__grid">
        ...
        <div>
          <h2>Trust</h2>
          {trustPages ? <Link href={`${home}/about`}>About</Link> : null}
          <Link href={`${home}/contact`}>Contact</Link>
          {trustPages ? <Link href={`${home}/privacy`}>Privacy</Link> : null}
