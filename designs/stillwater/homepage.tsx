  ...
  return (
    <div className="sw-locked-light">
      {/* Stillwater is a locked-LIGHT design, but the reused Studio atoms
          ...
          Re-pin those utilities to their light twins inside this subtree so the
          page stays serene (.class .class beats .class) — the FABLE pattern,
          extended to cover the button variants used here.
          NB (dark-mode unification): `dark:` utilities now follow the `.dark`
          CLASS, not the OS — so under OS-dark these rules are a redundant no-op
          (the utilities never fire). They are kept as the design-local "locked
          light" belt-and-suspenders for the class-dark case (persisted
          theme=dark + OS-dark). Revisit when storefront dark becomes a
          per-design opt-in (Phase 3). */}
      <style>{`
