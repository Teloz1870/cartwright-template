...
export default function AuroraSiteHomepage({
  settings,
  threeD,
  genome,
  editEnabled = false,
  ...
  // In-place editing (annotateEdit): hero headline/tagline skriver til settings
  // (første led i copy-kæden). Genome-slots er kun editbare når genomeResolve
  // OGSÅ er on (genome-prop present) — ellers rendres ankeret og en override
  // ville ikke slå igennem (samme regel som Footer'ens genome-hooks).
    ...
    <div className="bg-cw-paper text-cw-stone-900 dark:bg-cw-ink dark:text-cw-stone-50">
      ...
      {features.length > 0 && (
        <StudioFeatureGrid
          ...
          features={features}
          titleAttrs={editAttr({ kind: "genome", key: "home.features.title" }, genomeEdit)}
          descriptionAttrs={editAttr(
