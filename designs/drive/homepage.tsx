 ...
 * homepage's vertical stack of full-viewport panels in a plain
 * `<div className="drv__stack">`. The pack sets `layout.ownsMain` in index.ts
 * so the engine's shared <main> is suppressed; the single <main> landmark
 * lives in the DriveShell (chrome.tsx) so EVERY Drive page — not just the
 * homepage — keeps exactly one <main>.
 * Each panel is a CSS-only atmospheric backdrop (dusk /
        ...
        {panels.map((p, i) => (
          ...
          >
            ...
            <div className="drv__copy">
              ...
              {/* one <h1> per page: first panel is the h1, the rest are h2 (same class = same look) */}
              {i === 0 ? (
                <h1 className="drv__title" id={`${p.id}-h`}>{p.title}</h1>
              ) : (
                <h2 className="drv__title" id={`${p.id}-h`}>{p.title}</h2>
              )}
