 ...
 * The pack sets `layout.ownsMain` (index.ts), so the engine renders no shared
 * <main> — the Shell owns the single <main> landmark on EVERY page (homepage
 * and content pages alike; the homepage stack below it is a plain <div>).
 */
export function DriveShell({ children }: { children: ReactNode; locale: string }) {
  return (
    <div className={`drv ${display.variable} ${body.variable}`}>
      <main className="drv__wrap">{children}</main>
    </div>
  );
}
