...
/**
 * This pack ships to every customer, and two of its blocks link Cartwright's
 * own marketing pages (`/cases`, `/cartwright`) — routes `scaffold/engine-only.ts`
 * lists and the CLI deletes from a customer's scaffold. `Boolean(...)`, never
 * `=== true`: the static twin is `as const` literal `false`, so an equality
 * comparison is TS2367 in a materialised scaffold while every engine gate stays
 * green (#550/#551/#572). True here, so the engine and all three canaries render
 * byte-identically.
 */
 ...
 */
export default function SaaSDarkHomepage({ settings, threeD, editEnabled = false, genome }: DesignHomepageProps) {
  const t = useTranslations("SaaSHome");

  // Voice/Genome hero copy (genomeResolve flag). Anchor = the existing fallback,
  // so flag-off / no-Voice reads byte-identical; settings (admin) wins first.
  const heroHeadline = settings?.websiteHeadline || genome?.hero.headline || brand.metadata.title;
  const heroTagline = settings?.tagline || genome?.hero.tagline || brand.tagline;
  ...
  return (
    <div className="relative min-h-screen bg-black text-white font-sans selection:bg-white/30 selection:text-white">
      ...
      {/* Cartwright Product Section — the whole block is about OUR product and
          ends in a CTA to `/cartwright`, so it goes with the route. */}
      {engineMarketing && (
      <section className="relative z-10 py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
