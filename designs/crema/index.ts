...
import { parseCoffeeAttributes } from "./webshop/attributes";
...
export const cremaDesign: DesignPack = {
  ...
  webshop: {
    ...
    ownsBreadcrumb: true,
    // What the PDP frame draws for THIS product — asked, not assumed. The
    // frame shows a roast band only when `parseCoffeeAttributes` accepts the
    // value (integer 1–4, its "never guess" rule) and an eyebrow only for the
    // origin/process it could read. Anything it declines stays in the shared
    // spec table, where it printed before typed values existed.
    ownsAttributes: (attributes, { hasVariants }) => {
      const a = parseCoffeeAttributes(attributes);
      const drawn: string[] = [];
      if (a.roast !== null) drawn.push("roast");
      if (a.origin !== null) drawn.push("origin");
      if (a.process !== null) drawn.push("process");
      // Pack size belongs to the VARIANT once there are variants: Colombia
      // Supremo carries weightG: 250 while its picker offers 250 g AND 1 kg,
      // so the shared table would state a size the shopper is about to change.
      // The variant labels carry it ("Whole beans, 250 g"), so nothing is lost.
      if (hasVariants && a.weightG !== null) drawn.push(PACK_SIZE_ATTRIBUTE);
      return drawn;
    },
  },
};
