...
async function main() {
  ...
  if (explicitAdminPw) {
    ...
    const banner =
      ...
      "│  → Log ind på /account/login og skift det ved første login. │\n" +
      "│  → Også gemt i .admin-credentials (slet filen når du har     │\n" +
      "│    gemt password et sikkert sted).                          │\n" +
      "└─────────────────────────────────────────────────────────────┘\n";
    console.log(banner);
    try {
      ...
      fs.writeFileSync(
        ...
        `Cartwright admin-login (genereret ved seed ${new Date().toISOString()})\n\n` +
          `Email:    ${brand.emails.admin}\n` +
          `Password: ${adminPassword}\n\n` +
          `Log ind på /account/login (password-fanen) og skift adgangskoden ved\n` +
          `første login. Slet denne fil når du har gemt password et sikkert sted.\n` +
          `Den er gitignored og bliver aldrig committet eller delt.\n`,
