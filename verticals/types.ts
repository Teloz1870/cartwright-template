...
export type VerticalPreset = {
  ...
  slug: string;
  /** Human name shown in admin + marketplace (e.g. "Kindergarten"). */
  name: string;
  description: string;
  /** Free-text search tags for the gallery. */
  keywords: string[];
  /**
   * Localized industry nouns (e.g. "tømrer", "børnehave") that should match a
   * prompt as STRONGLY as the English name/slug words do in the instant preset
   * path (lib/magic/presets.ts). Display stays English-first — aliases never
   * render anywhere and are not emitted to the marketplace manifest.
   */
  aliases?: string[];
  /**
