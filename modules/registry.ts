...
const core = node(
  ...
  "core",
  {
    ...
    seams: [
      ...
      // Every cron in vercel.json targets a route that only exists with a
      // database (Stripe reconcile, FX refresh, media AI, review prompts, SEO
      // snapshots, token/audit retention, backups, abandoned carts — ten
      // schedules: eight daily, one weekly, one on weekdays). A
      // materialization without db therefore takes vercel.static.json — the
      // same file minus `crons` — instead of handing Vercel's scheduler ten
      // routes that 404. The twin is pinned to "vercel.json minus crons" by
      // tests/unit/repo-hygiene.test.ts.
      "vercel.json",
      // The root layout loads Geist from Google at build time, and nine design
      // packs load their display faces the same way at module scope — the
      // barrel imports every pack, so one slow fetch on a first `next dev`
      // 500s the whole site and Turbopack caches the failure (A5, measured).
      // `google-fonts` provides this seam; without it the static twin (system
      // font stack, no network) ships and those packs are gone with it.
      "app/layout.tsx",
    ],
    knownDeviations: [
...
const MODULE_NODES: CartwrightModuleManifest[] = [
  core,
  node("google-fonts", "Google-hosted typography", "The root layout's Geist pair and the nine design packs whose display faces are fetched from Google Fonts at build time (aerospace, brutalist, drive, editorial-ink, fable, flux, meridian, nocturne, stillwater). Present from managed-site upward; a profile without it renders the system font stack and keeps only the self-contained packs, so its first `next dev` and every build work with Google's CDN unreachable (A5).", [], "module", {
    files: inv(
      "designs/aerospace",
      "designs/brutalist",
      "designs/drive",
    ...
    // `with` === `target`: including this module means the root layout keeps
    // its on-disk (Google-font) content; only a materialization without it
    // takes the core-owned `app/layout.static.tsx`.
    replaces: [{ target: "app/layout.tsx", with: "app/layout.tsx" }],
  }),
  ...
  node(
    "trust-pages",
    "Trust pages",
    "The About and Privacy aliases of the info route. Owner decision 2026-09-07 (CW-23): opt-in in the `site` profile (`--with trust-pages`), included from managed-site upward, where the seed content and the wizard's trust audit already fill them.",
    [],
    "module",
    {
      // Two thin aliases over `app/[locale]/info/[slug]/page.tsx`
      // (`renderPublicInfoPage`), so they are removable without touching that
      // route. `/info/terms` and `/info/cookies` are NOT here on purpose: they
      // are that same seam TARGET, owned by pages-db and provided by core's
      // static variant, so removing them means re-cutting the seam rather than
      // adding a claim — named as B3/B4 input rather than faked here.
      //
      // The chrome links /about and /privacy unconditionally, so this module
      // has a capability: `profileCapabilities.trustPages` is true in the
      // engine (and in every db-backed profile, which include this module) and
      // false in the static twin, and the footers read it. A `--with
      // trust-pages` site therefore SHIPS the pages while the stock footer
      // still hides the links — safe (never a 404), and the honest gap: B4's
      // codemod should flip `trustPages` to true in the twin when the module
      // is opted in.
      files: inv("app/[locale]/about", "app/[locale]/privacy"),
    },
  ),
  node("db", "Database", "Prisma + libSQL/Turso/Postgres data layer; the db seam variants replace core's static content sources AND provide the DB-coupled variants of the app shell (layout/homepage/chrome/sitemap/llms).", [], "module", {
    ...
    replaces: [
      ...
      { target: "vercel.json", with: "vercel.json" },
    ],
    knownDeviations: [
  ...
  node("admin", "Admin", "The /admin surface (essential set), audit log, AI stack, in-place editing, setup/first-run, SEO/GDPR/import tooling.", ["db", "auth"], "module", {
    ...
    files: inv(
      ...
      // The Page Mixer's live preview frame — `app/admin/mixer/page.tsx`
      // renders it and three profile tests import it. Unclaimed it shipped to
      // every customer as a 193-line preview of OUR admin tool (owner CW-23);
      // it belongs where builder-preview already is.
      "app/[locale]/mixer-preview",
      "app/api/support",
  ...
  node("webshop", "Webshop", "PLP/PDP, cart, Stripe checkout + webhooks, orders, VAT/tax, shipping zones, discounts, multi-currency — plus the webshop-coupled design packs and shared product components.", ["db", "auth", "admin"], "module", {
    files: inv(
      // Machine-readable pricing (agentic-polish round) — reads the product
      // table and only exists where something is for sale.
      "app/pricing.md",
      "lib/cart.ts",
      "lib/pricing.ts",
      "lib/tax.ts",
      ...
      // WebMCP per-page tool mounts — cart/PDP/PLP surfaces (import only
      // core + webshop files, so no cross-module edge). The CLI light
      // profile ships the ENTIRE WebMCP surface — registrar, mounts,
      // /webmcp-check, tests — dormant behind the default-off `webMcp`
      // flag (owner decision 2026-08; see cartwright-app profile-light.ts).
      "components/webmcp",
      "designs/apex",
      "designs/atelier",
      "designs/aurora-shop",
  ...
  node("mcp", "MCP + tool surface", "The public MCP server + REST tool registry (/api/mcp, /api/v1/tools) and agent keys. Default-on from managed-site upward (owner decision 2026-07-15).", ["db", "auth", "admin", "agent-core"], "module", {
    files: inv(
      ...
      // Agentic-polish round: the API's own 404 surface + agent auth/discovery
      // docs — all import lib/api-problem or the tool/public-gate stack, so
      // they belong to (and prune with) this module.
      "app/api/route.ts",
      "app/api/[...unknown]",
      "app/auth.md",
      "app/.well-known/ai-catalog.json",
      "lib/openapi.ts",
      "lib/api-problem.ts",
      ...
      // Diagnostics for the WebMCP experiment — previously unclaimed (shipped
      // in every profile); they belong to this module and prune with it.
      "components/WebMcpCheck.tsx",
      // The judge-facing inventory on /webmcp-check: imports the binding
      // consts from BOTH the mcp surfaces and components/webmcp (webshop) —
      // it must prune with this module or its edges leak into the site
      // profile's import closure.
      "components/WebMcpShowcase.tsx",
    ...
    // THIS is why `profileCapabilities` is site-only. Providing the seam means
    // the profile keeps the engine tree's all-true file; `managed-site`/`light`
    // (the CLI default), `commerce` and `agentic`/`full` all include this
    // module, so `site` is the only profile that ever sees
    // `profile-capabilities.static.ts`. That is fine for `agentApi` and
    // `accountAndAdmin` — those really do exist in every mcp profile — and it
    // is a declared gap for `engineMarketing`, which `scaffold/engine-only.ts`
    // promises for EVERY customer scaffold: see ENGINE_ONLY_PENDING_PROFILES
    // there (pinned against the manifest) and the B4 OBLIGATION block in
    // lib/profile-capabilities.ts. Note the materializer never reads `with`:
    // a provided seam keeps the on-disk file, so a third variant here would be
    // inert until B4 changes that contract.
    replaces: [
    ...
    knownDeviations: [
      ...
      "components/WebMcpRegistrar.tsx (mcp) imports the webshop's cart actions for its get_cart tool — same class as the assistant-chat edge above; a webshop-less mcp materialization would need the registrar's cart tool pruned",
    ],
  }),
...
export const PROFILES: readonly ProfileDefinition[] = [
  ...
  },
  {
    ...
    modules: ["db", "auth", "admin", "pages-db", "contact-form", "trust-pages", "agent-core", "mcp", "google-fonts"],
    aliases: ["light"],
  },
  {
    schema: "cartwright-profile-v1",
    ...
    modules: ["db", "auth", "admin", "pages-db", "contact-form", "trust-pages", "agent-core", "mcp", "webshop", "feeds", "google-fonts"],
    aliases: [],
  },
  {
    schema: "cartwright-profile-v1",
