...
const MODULE_NODES: CartwrightModuleManifest[] = [
  ...
  node("webshop", "Webshop", "PLP/PDP, cart, Stripe checkout + webhooks, orders, VAT/tax, shipping zones, discounts, multi-currency — plus the webshop-coupled design packs and shared product components.", ["db", "auth", "admin"], "module", {
    files: inv(
      ...
      // WebMCP per-page tool mounts — cart/PDP surfaces (import only core +
      // webshop files, so no cross-module edge; the CLI light profile KEEPS
      // this dir — see cartwright-app LIGHT_EXCLUDED_PATHS docblock).
      "components/webmcp",
      "designs/apex",
      "designs/atelier",
      "designs/aurora-shop",
