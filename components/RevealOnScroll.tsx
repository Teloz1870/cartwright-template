...
export default function RevealOnScroll({ children, delay = 0, className = "" }: Props) {
  ...
  useEffect(() => {
    ...
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reduced-motion is browser-only; reveal immediately after mount instead of animating. Must start false for SSR parity.
      setRevealed(true);
      return;
    }
