  ...
  return (
    ...
    >
      ...
      <aside className="relative flex h-full w-full max-w-md flex-col bg-[#050A19] shadow-2xl border-l border-[#d4af37]/20">
        ...
        >
          {messages.length === 0 && <WelcomeBubble ecommerceEnabled={ecommerceEnabled} />}
...
function WelcomeBubble({ ecommerceEnabled = true }: { ecommerceEnabled?: boolean }) {
  if (!ecommerceEnabled) {
    return (
      <div className="rounded-2xl border border-sol-ink/10 bg-[#050A19] px-4 py-3 text-sm text-white">
        <p className="font-black text-[#d4af37]">Hej 👋</p>
  ...
  return (
    <div className="rounded-2xl border border-sol-ink/10 bg-white px-4 py-3 text-sm text-sol-ink">
      ...
      <p className="mt-1 leading-6">
        I am {brand.ai.assistantLabel} for {brand.storeName}. Tell me what
        you are looking for, such as your use case, budget, or preferences, and
        I will find options that fit.
      </p>
      <div className="mt-3 flex flex-wrap gap-1.5">
        {[
          "Best everyday products under 500 DKK",
          "Gift ideas",
          "Lightweight options",
        ].map((p) => (
          <span
