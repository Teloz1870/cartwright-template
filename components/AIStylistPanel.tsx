...
export default function AIStylistPanel({ open, onClose, ecommerceEnabled = true }: Props) {
  ...
  const body = (
    // data-cw-ai-assistant: stable design hook for custom designs — target
    // [data-cw-ai-assistant="panel"] instead of Tailwind selectors. The
    // <aside> renders in BOTH the native-<dialog> and fallback paths, so the
    // hook is path-independent. See DESIGN.md §5.
    <aside data-cw-ai-assistant="panel" className="relative flex h-full w-full max-w-md flex-col bg-[#050A19] shadow-2xl border-l border-[#d4af37]/20">
...
function WelcomeBubble({
  ...
  // Vertical-neutral conversation starters — the old set ("Lightweight
  // options", "under 500 DKK") was eyewear-era copy that read as another
  // store's leftovers on every other vertical.
  const chips = en
    ? ["What do you recommend?", "Bestsellers", "Gift ideas"]
    : ["Hvad anbefaler du?", "Bestsellere", "Gaveidéer"];
  return (
...
function ToolResultRenderer({
  ...
  // The rest of this file localizes with `en ? … : …` (see :35, :623, :708).
  // This component was the one without the binding, so its three strings
  // rendered Danish on every shop — and `aiStylist` is default-ON.
  const locale = useLocale();
  const en = locale === "en";
  // Storefront: this tree DOES sit inside app/[locale]'s NextIntlClientProvider,
  // so the payment labels PlanCard needs are resolved here. The admin chat
  // renders the same card with literals — see AdminChatPanel.
  ...
  if (
    ...
    return (
      <PlanCard
        ...
        paymentLabels={{
          securePayment: tTrust("securePayment"),
          paymentMethodsAria: tTrust("paymentMethodsAria"),
        }}
        returnsHref={`/${locale}/info/returns`}
      />
    );
  }
  ...
  if (
    ...
    const order = output as unknown as {
      ...
      // createOrder returns both alongside the client secret in stripe mode.
      // They are guarded below rather than assumed: this is a cast over tool
      // output, and a pay button that falls back to the base-currency total
      // would show an amount the card is not charged.
    ...
    if (
      ...
      typeof order.chargeAmountMinor === "number" &&
    ...
    if (lookup.hasOrders && lookup.lastShipping) {
      return (
        <div className="rounded-lg border border-sol-accent/30 bg-sol-accent/5 px-3 py-2 text-xs text-sol-ink">
          <p className="font-bold">{en ? "✓ Welcome back!" : "✓ Velkommen tilbage!"}</p>
          <p className="mt-1 leading-5 text-sol-muted">
            {en ? "Latest delivery address: " : "Seneste leveringsadresse: "}
              {lookup.lastShipping.name},{" "}
  ...
  // ui.present_products → model-valgt præsentation (Hul B): modellen vælger
  // layout fra et hvidlistet enum; vi rendrer det matchende komponent.
  if (
    toolName === "ui.present_products" &&
    output &&
    typeof output === "object" &&
    "products" in output
  ) {
    const pres = output as unknown as {
      layout: "grid" | "spotlight" | "comparison";
      note: string | null;
  ...
  // products.search → produktkort (data-tool; faldback hvis modellen ikke
  // eksplicit kalder ui.present_products).
  ...
  if (toolName === "cart.add" && output && typeof output === "object" && "added" in output) {
    ...
    return (
      <p className="rounded-lg bg-sol-sand px-3 py-2 text-xs font-bold text-sol-ink">
        {en ? "✓ Added to cart: " : "✓ Lagt i kurven: "}
          {added.name} (×{added.quantity}){" "}
...
function OrderSuccessInline({ orderId, totalDkk }: { orderId: string; totalDkk: number }) {
  // Client component inside app/[locale]'s provider (same as :35/:602),
  // so the hook is legal here — it is the ADMIN tree that has no provider.
  const locale = useLocale();
  return (
    <div className="rounded-xl border-2 border-green-500 bg-green-50 p-4">
      ...
      <p className="mt-2 text-xs text-green-800">
        Total: <strong>{formatPriceDkk(totalDkk, { locale })}</strong>
      </p>
...
function ProductGridInline({ products }: { products: InlineProduct[] }) {
  // Locale-prefixed links: a bare /product/<slug> href gets middleware-
  // redirected to the DEFAULT locale, bouncing an /en visitor into Danish
  // when they open a suggested product (#469's bug class).
  const locale = useLocale();
  const en = locale === "en";
  if (products.length === 0) {
    return (
      <p className="text-xs italic text-sol-muted">
        {en ? "No products matched. Try a broader search." : "Ingen produkter matchede. Prøv en bredere søgning."}
      </p>
    );
  }
  return (
        ...
        >
          ...
          <div className="p-2">
            ...
            <p className="mt-0.5 text-xs font-black text-sol-accent">
              {formatPriceDkk(p.priceDkk, { locale })}
            </p>
          </div>
        </Link>
      ))}
...
/**
 * Model-valgt produktpræsentation (Hul B). Modellen vælger `layout`; vi rendrer
 * det matchende hvidlistede komponent. `note` er en valgfri framing-linje.
 */
function PresentedProducts({
  layout,
  note,
  products,
}: {
  ...
  // Sammenligning af ét produkt ser skævt ud → fald tilbage til spotlight.
  const effectiveLayout =
    layout === "comparison" && products.length === 1 ? "spotlight" : layout;
  return (
    <div className="space-y-2">
      {note && (
        <p className="text-xs font-medium leading-snug text-sol-ink">{note}</p>
      )}
      {effectiveLayout === "spotlight" ? (
        <ProductSpotlightInline product={products[0]} />
      ) : effectiveLayout === "comparison" ? (
        <ComparisonInline products={products.slice(0, 4)} />
      ) : (
        <ProductGridInline products={products} />
      )}
    </div>
  );
}

function ProductSpotlightInline({ product: p }: { product: InlineProduct }) {
  const locale = useLocale();
  const en = locale === "en";
  return (
    <Link
      href={`/${locale}/product/${p.slug}`}
      className="group block overflow-hidden rounded-2xl border border-sol-ink/10 bg-sol-sand transition hover:shadow-md"
    >
      <div className="relative aspect-[16/10] bg-sol-cream">
        {p.firstImage && (
          <Image
            src={p.firstImage}
            alt={p.name}
      ...
      <div className="p-3">
        ...
        <p className="text-sm font-black leading-tight text-sol-ink">{p.name}</p>
        <div className="mt-1 flex items-center justify-between">
          <p className="text-sm font-black text-sol-accent">
            {formatPriceDkk(p.priceDkk, { locale })}
          </p>
          <span className="rounded-full bg-sol-accent px-3 py-1 text-[10px] font-black uppercase tracking-wider text-white">
            {en ? "View" : "Se mere"}
          </span>
        </div>
      </div>
...
function ComparisonInline({ products }: { products: InlineProduct[] }) {
  const locale = useLocale();
  const en = locale === "en";
  return (
    <div
    ...
    >
      {products.map((p) => (
        <Link
          key={p.slug}
          href={`/${locale}/product/${p.slug}`}
          className="group flex flex-col overflow-hidden rounded-xl border border-sol-ink/10 bg-sol-sand transition hover:shadow"
        >
          <div className="relative aspect-square bg-sol-cream">
            {p.firstImage && (
              <Image
          ...
          <div className="flex flex-1 flex-col p-2">
            <p className="text-[9px] uppercase tracking-wide text-sol-muted">
              {p.brand}
            </p>
            <p className="text-[11px] font-black leading-tight text-sol-ink">
              {p.name}
            </p>
            <p className="mt-0.5 text-[11px] font-black text-sol-accent">
              {formatPriceDkk(p.priceDkk, { locale })}
            </p>
            <p className="mt-auto pt-1 text-[9px] uppercase tracking-wide text-sol-muted">
              {p.stock > 0 ? (en ? "In stock" : "På lager") : en ? "Sold out" : "Udsolgt"}
            </p>
          </div>
        </Link>
      ))}
