...
type Props = {
  ...
  /** Optional product name, forwarded to AddToCartButton's accessible name. */
  productName?: string;
};
 ...
 */
export default function VariantPicker({ productId, basePriceDkk, variants, productName }: Props) {
      ...
      {attributeKeys.map((key) => (
        <label key={key} className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            className="rounded-lg border border-sol-ink/15 dark:border-white/15 bg-white dark:bg-sol-sand px-3 py-2 font-semibold text-sol-ink focus:border-sol-accent focus:outline-none focus:ring-2 focus:ring-sol-accent/25"
          >
            <option value="" disabled>
              {t("variantSelect", { attribute: key })}
            </option>
