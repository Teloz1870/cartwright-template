...
/**
 * CometStreak — a comet with a living tail: dust particles stream toward the
 * tip while the head glow pulses.
 *
 ...
 * transform/opacity keyframes, namespaced (`cwsi-cstreak-*`), every rule
 * inside `@media (prefers-reduced-motion: no-preference)` — reduced motion
 * renders the comet frozen mid-flight. All paint reads the cw-* palette
 * tokens with the engine fallback chain.
 *
...
/**
 * Compositor-only motion, all under the reduced-motion gate. Dust drifts
 * head→tip along the tail direction in four phase buckets; the head glow
 * breathes; the tail strands shimmer faintly.
 */
const CSTREAK_CSS = `
@media (prefers-reduced-motion: no-preference) {
  .cwsi-cstreak-p {
    animation: cwsi-cstreak-drift 9.6s linear infinite;
  }
  .cwsi-cstreak-p2 { animation-delay: -2.4s; }
  .cwsi-cstreak-p3 { animation-delay: -4.8s; }
  .cwsi-cstreak-p4 { animation-delay: -7.2s; }
  .cwsi-cstreak-glow {
    transform-box: fill-box;
    transform-origin: center;
    animation: cwsi-cstreak-pulse 8.5s ease-in-out infinite;
  }
  .cwsi-cstreak-tail {
    animation: cwsi-cstreak-shimmer 12s ease-in-out infinite;
  }
  .cwsi-cstreak-tail2 { animation-delay: -4s; }
  .cwsi-cstreak-tail3 { animation-delay: -8s; }
  @keyframes cwsi-cstreak-drift {
    0% { transform: translate(-7px, 7px); opacity: 0; }
    18% { opacity: 1; }
    78% { opacity: 0.3; }
    100% { transform: translate(11px, -11px); opacity: 0; }
  }
  @keyframes cwsi-cstreak-pulse {
    0%, 100% { transform: scale(1); opacity: 0.85; }
    50% { transform: scale(1.14); opacity: 1; }
  }
  @keyframes cwsi-cstreak-shimmer {
    0%, 100% { opacity: 0.85; }
    50% { opacity: 1; }
  }
}
...
export function CometStreak({ className, style }: { className?: string; style?: CSSProperties }) {
  return (
    <svg
      viewBox="0 0 120 120"
    ...
    >
      <defs>
        ...
        <linearGradient id="cwsi-cstreak-outer" x1="0" y1="1" x2="1" y2="0">
          <stop offset="0" stopColor={ACCENT} stopOpacity="0.35" />
          <stop offset="0.6" stopColor={ACCENT} stopOpacity="0.15" />
          <stop offset="1" stopColor={ACCENT} stopOpacity="0" />
        </linearGradient>
        <linearGradient id="cwsi-cstreak-mid" x1="0" y1="1" x2="1" y2="0">
        ...
        <linearGradient id="cwsi-cstreak-core" x1="0" y1="1" x2="1" y2="0">
        ...
        <radialGradient id="cwsi-cstreak-coma" cx="0.4" cy="0.4" r="1">
          <stop offset="0" stopColor={CREAM} />
          <stop offset="0.55" stopColor={ACCENT} />
          <stop offset="1" stopColor={ACCENT_DEEP} />
        </radialGradient>
      </defs>
      <style dangerouslySetInnerHTML={{ __html: CSTREAK_CSS }} />
      ...
      <path
        className="cwsi-cstreak-tail"
        d="M35 81 C 56 69, 81 44, 108 17"
        fill="none"
        stroke="url(#cwsi-cstreak-outer)"
        strokeWidth="7"
        strokeLinecap="round"
      />
      <path
        className="cwsi-cstreak-tail cwsi-cstreak-tail2"
        d="M35 81 C 57 70, 83 47, 106 20"
        fill="none"
        stroke="url(#cwsi-cstreak-mid)"
        strokeWidth="3.5"
        strokeLinecap="round"
      />
      <path
        className="cwsi-cstreak-tail cwsi-cstreak-tail3"
        d="M35 81 C 55 70, 79 45, 102 22"
        fill="none"
        stroke="url(#cwsi-cstreak-core)"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      ...
      {/* Trailing dust — four phase buckets streaming head→tip */}
      {PARTICLES.map((p, i) => (
        <circle
          key={i}
          className={`cwsi-cstreak-p cwsi-cstreak-p${(i % 4) + 1}`}
          cx={p.x}
          cy={p.y}
          r={p.r}
          fill={p.c}
      ...
      {/* Head — glow (pulsing), coma, nucleus, glint */}
      <circle className="cwsi-cstreak-glow" cx="34" cy="82" r="11" fill={ACCENT} fillOpacity="0.12" />
      <circle className="cwsi-cstreak-glow" cx="34" cy="82" r="7.5" fill={ACCENT} fillOpacity="0.14" />
      <circle
        cx="34"
        cy="82"
        r="5.6"
        fill="url(#cwsi-cstreak-coma)"
        stroke={ACCENT_DEEP}
        strokeOpacity="0.5"
        strokeWidth="0.75"
      />
