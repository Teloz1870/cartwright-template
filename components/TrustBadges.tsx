...
/** See the GDPR badge below — `/privacy` is `trust-pages`, opt-in in `site`. */
...
const visibleByVariant = {
  homepage: ["free-shipping", "returns", "primary-feature", "secure-payment"],
  product: ["free-shipping", "returns"],
  checkout: ["free-shipping", "returns", "secure-payment"],
  footer: ["free-shipping", "returns", "primary-feature", "secure-payment", "gdpr"],
  category: ["free-shipping", "returns", "primary-feature"],
} satisfies Record<BadgeVariant, string[]>;
  ...
  const badges: Badge[] = [
    {
      ...
      sublabel: t("freeShippingSub"),
      // Plain `next/link` does not prefix the locale, and `/info/*` only
      // exists under `app/[locale]` — a bare path 307s through the proxy and
      // lands a /en reader of a da-default shop back in Danish.
    ...
    },
    {
      ...
      id: "primary-feature",
      // Tom config-værdi ⇒ lokaliseret neutral default (config-defaults må
      // aldrig vælge sprog — samme regel som footer-disclaimeren, #456).
      label: brand.uiLabels.trustBadgesPrimary || t("primary"),
      sublabel: t("primarySub"),
      href: `/${locale}/info/faq`,
      icon: <SunIcon />,
    },
    {
      id: "secure-payment",
      label: t("securePayment"),
      sublabel: t("securePaymentSub"),
      icon: <CardIcon />,
      stripe: true,
    },
    ...(process.env.NEXT_PUBLIC_EMAERKET === "true"
      ...
      : []),
    // The GDPR badge is a link to `/privacy`, which belongs to the
    // `trust-pages` module — opt-in in the `site` profile (owner decision
    // 2026-09-07, CW-23). Every surface that renders TrustBadges today is
    // webshop-owned, so a site scaffold never paints it; the gate is here
    // because the FILE ships in every profile and a badge whose only content
    // is a link to a missing page is a 404 waiting for the first site
    // scaffold that renders it. `Boolean(...)`, never `=== true` (TS2367
    // against the `as const` static twin — #550/#551/#572).
    ...(trustPages
      ? [
          {
        ...
        const content = (
          ...
          >
            ...
            <span className="min-w-0">
              ...
              {badge.sublabel && (
                <span
                  className={`block text-xs leading-5 opacity-90 ${
