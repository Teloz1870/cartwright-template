...
export default function NewsletterSignup({ source = "footer" }: { source?: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // The browser's OWN email rule, asked directly. With the webMcp flag on
    // the form is noValidate, so constraint validation no longer runs by
    // itself; without this, only the regex below would, and it is looser than
    // the browser's (`name@gmail..com`, `a,b@c.dk` pass it). Flag off, a value
    // the browser rejects never reaches submit, and an empty one is valid
    // without `required` — so there this can only ever agree with the browser.
    const field = event.currentTarget.querySelector<HTMLInputElement>('input[type="email"]');
    const browserRejects = field?.validity.valid === false;

    if (browserRejects || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(submittedEmail)) {
      setSuccess(false);
      setInvalid(true);
      setError(t("newsletterInvalidEmail"));
      // Native validation moves focus to the offending field. Where the form
      // is noValidate (flag on), that no longer happens by itself, so the page
      // does it — otherwise a keyboard user is left on the button. This runs
      // for agent-invoked submits too, deliberately: this form has no
      // autosubmit, so an agent-invoked submit here is always a HUMAN who just
      // pressed its button after the agent filled it in. Flag off, nothing
      // here changes: the browser focuses the values it rejects itself, and an
      // empty value (valid without `required`) never moved focus at all.
      if (event.currentTarget.noValidate) field?.focus();
    ...
    <form
      ...
      {...(webMcp
        ? {
            ...
            // noValidate so the PAGE answers every submit — the localized
            // message, aria-invalid, and a respondWith answer for an agent.
            // Left to the browser, the `required` below would block an empty
            // submit with a bubble in the BROWSER's language, and the agent's
            // call would get a bare "Form validation failed" (native Chrome)
            // or no answer at all (the polyfill). The browser's email rule is not lost:
            // handleSubmit asks the input's own ValidityState (see there).
            noValidate: true,
          }
        : {})}
    >
        ...
        <input
          ...
          {...(webMcp
            ? {
                name: "email",
                required: true,
                // No `format: "email"` exists in either derivation, so the
                // guidance goes in the description. Native Chrome would carry
                // a `pattern` attribute into the schema — deliberately not
                // used: any email regex we wrote would diverge from the
                // browser's own type="email" rule, which handleSubmit enforces.
                toolparamdescription: "The email address to subscribe — a valid address such as name@example.com.",
              }
            : {})}
          value={email}
          onChange={(event) => {
