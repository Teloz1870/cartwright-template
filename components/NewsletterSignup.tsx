    ...
    <form onSubmit={handleSubmit} className="space-y-2">
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={pending}
          className="shrink-0 rounded-md bg-white px-5 py-2 text-sm font-bold text-sol-accent transition hover:bg-sol-sun hover:text-sol-ink disabled:opacity-60"
        >
          {pending ? "…" : "Sign up"}
        </button>
      </div>
      ...
      {success ? (
        <p className="text-xs font-medium text-white/80">Thanks, you are subscribed.</p>
      ) : null}
    </form>
  );
}
