...
type Props = {
  ...
  /**
   * Where the §18 withdrawal note links. Resolved by the CALLER for the same
   * reason `paymentLabels` is: the storefront tree is locale-routed
   * (`/da/info/returns`) while the admin tree has no locale at all.
   *
   * REQUIRED, not optional-with-a-default. A default of `/info/returns` would
   * be correct for the admin and silently locale-stripped for any future
   * storefront caller that forgot it — and by then the href is an identifier,
   * which no source scan can resolve. Required makes the omission a type
   * error at the only place that knows the answer.
   */
  returnsHref: string;
};
