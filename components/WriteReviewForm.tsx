 ...
 *
 * Bruges fra:
 *   - /[locale]/konto/ordrer/[id]/anmeld   (logged-in, sender orderId)
 *   - /[locale]/anmeld/[token]              (unauth, sender reviewToken)
...
export default function WriteReviewForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (rating < 1) {
      setFeedback("Vælg et antal stjerner.");
      return;
    }
    if (body.trim().length < 10) {
      setFeedback("Beskrivelsen skal være mindst 10 tegn.");
      return;
    }
    ...
    startTransition(() => {
      void (async () => {
        ...
        if (!res.ok) {
          ...
          setFeedback(`Fejl: ${data.error ?? `HTTP ${res.status}`}`);
          return;
        }
        setSubmitted(true);
        router.refresh();
  ...
  return (
    <form
      ...
      className="flex flex-col gap-4 rounded-2xl border border-sol-ink/10 bg-white p-5 dark:bg-sol-ink dark:text-white"
    >
      ...
      <Field label="Overskrift (valgfri)">
        <input
          ...
          placeholder="Fx 'Fantastisk kvalitet'"
        />
      </Field>

      <Field label="Din anmeldelse">
        <textarea
          required
          minLength={10}
          maxLength={2000}
          ...
          placeholder="Hvordan oplever du produktet? Hvad bør andre vide?"
        />
      ...
      {!orderId && !reviewToken && (
        <>
          ...
          <Field label="Din email (vises ikke offentligt)">
            <input
              type="email"
              required
              className={inputClass}
      ...
      >
        {isPending ? "Sender…" : "Send anmeldelse"}
      </button>

      <p className="text-[10px] text-sol-muted">
        Din anmeldelse bliver gennemgået af vores team før den vises offentligt.
        Vi bruger din email kun til at kontakte dig om anmeldelsen — den vises ikke.
      </p>
    </form>
  );
}
