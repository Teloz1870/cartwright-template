...
export default function WriteReviewForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (rating < 1) {
      const msg = "Vælg et antal stjerner.";
      setFeedback(msg);
      announce(msg, "assertive");
      return;
    }
    if (body.trim().length < 10) {
      const msg = "Beskrivelsen skal være mindst 10 tegn.";
      setFeedback(msg);
      announce(msg, "assertive");
      return;
    }
    ...
    startTransition(() => {
      void (async () => {
        ...
        if (!res.ok) {
          ...
          const msg = `Fejl: ${data.error ?? `HTTP ${res.status}`}`;
          setFeedback(msg);
          announce(msg, "assertive");
          return;
        }
        setSubmitted(true);
        announce("Tak for din anmeldelse — den vises offentligt så snart vi har godkendt den.");
        router.refresh();
      })();
    });
  }
