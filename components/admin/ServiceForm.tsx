...
export default function ServiceForm({ service }: { service?: ServiceData }) {
  ...
  const [body, setBody] = useState(service?.body || "## Description\n\nDescribe your service here...");
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-sol-sand p-6 rounded-xl border border-sol-ink/10 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-bold text-sol-ink mb-1">Title</label>
          <input
            required
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="w-full rounded-md border border-sol-ink/20 dark:border-white/15 px-3 py-2 bg-white dark:bg-sol-sand"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-bold text-sol-ink mb-1">Short Description</label>
          <input
            value={shortDescription}
            onChange={(e) => setShortDescription(e.target.value)}
            className="w-full rounded-md border border-sol-ink/20 dark:border-white/15 px-3 py-2 bg-white dark:bg-sol-sand"
            placeholder="Shown on the front page of the service catalog"
          />
        </div>
        <div>
          <label className="block text-sm font-bold text-sol-ink mb-1">Price String</label>
          <input
            value={priceString}
            onChange={(e) => setPriceString(e.target.value)}
            className="w-full rounded-md border border-sol-ink/20 dark:border-white/15 px-3 py-2 bg-white dark:bg-sol-sand"
            placeholder="e.g. From €9 / mo"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-sol-muted mb-2">Highlighted features for the B2B price card.</p>
          ...
          {features.map((feature, i) => (
            <div key={i} className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-sol-ink/20 dark:border-white/15 px-3 py-2 bg-white dark:bg-sol-sand text-sm"
      ...
      <div>
        <label className="block text-sm font-bold text-sol-ink mb-1">Sales Copy / Body</label>
        <textarea
          required
          rows={8}
          value={body}
          onChange={(e) => setBody(e.target.value)}
          className="w-full rounded-md border border-sol-ink/20 dark:border-white/15 px-3 py-2 bg-white dark:bg-sol-sand font-mono text-sm"
      ...
      <div className="flex items-center gap-6 p-4 bg-sol-cream/50 rounded-lg border border-sol-ink/5">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm font-bold text-sol-ink">Show in Main Menu</span>
        </label>
        
        <div className="flex items-center gap-2">
          <label className="text-sm font-bold text-sol-ink">Menu Order:</label>
          <input
            type="number"
            value={navOrder}
            onChange={(e) => setNavOrder(parseInt(e.target.value) || 0)}
            className="w-20 rounded-md border border-sol-ink/20 dark:border-white/15 px-2 py-1 bg-white dark:bg-sol-sand"
          />
        </div>
      </div>
