...
type ProductFormProduct = {
  ...
  images: string;
  stock: number;
  // P1.2: nullable after the schema change (eyewear-only fields)
  frameColor: string | null;
  lensColor: string | null;
  brand: string | null;
  featured: boolean;
  categoryId: string;
  /** Shop-Starter Task G: optional JSON attributes for fork shops (may be null) */
  attributes?: Record<string, unknown> | null;
  /** Task B: if the product has variants, render VariantsAdmin below attributes */
  variants?: Array<{
    id: string;
    sku: string;
    priceDkk: number;
  ...
  /** AEO (answer-first) fields — gated on brand.features.aeoContent. */
...
type ProductFormProps = {
  ...
  /** Resolved brand.features.aeoContent (from the server). Shows the AEO fields when true. */
  aeoEnabled?: boolean;
};
...
export default function ProductForm({ categories, product, aeoEnabled = false }: ProductFormProps) {
  ...
  // AI magic-button state: description + attributes are held as state so they
  // can be updated inline without a form submit. Initialised from props or empty.
  const [description, setDescription] = useState(product?.description ?? "");
  const [attributesJson, setAttributesJson] = useState(
    product?.attributes ? JSON.stringify(product.attributes, null, 2) : "",
  );
  // AEO (answer-first) fields. faq is stored as a JSON string in the DB (like Category.faq);
  // useCases/comparisonFacts er JSON-kolonner → vis pretty-printed JSON i editoren.
  const [answerSummary, setAnswerSummary] = useState(product?.answerSummary ?? "");
  const [faqJson, setFaqJson] = useState(
    typeof product?.faq === "string" ? product.faq : "",
  );
  const [useCasesJson, setUseCasesJson] = useState(
    product?.useCases ? JSON.stringify(product.useCases, null, 2) : "",
  );
  const [comparisonJson, setComparisonJson] = useState(
    product?.comparisonFacts ? JSON.stringify(product.comparisonFacts, null, 2) : "",
  );
  // Images state: an array of URLs. ImageUpload appends new URLs.
  ...
  // Mirrors the category version. Only active on existing products (requires productId).
    ...
    startGenerating(() => {
      void (async () => {
        ...
        if (result.ok) {
          ...
          // AEO fields are filled when the generator returns them (gated on aeoContent).
          if (result.data.answerSummary) setAnswerSummary(result.data.answerSummary);
          if (result.data.faq && result.data.faq.length > 0) {
            setFaqJson(JSON.stringify(result.data.faq, null, 2));
          }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {/* AI magic button + content fields — only for existing products.
            The button calls Anthropic to generate description + attributes
            based on product name + brand + category + price + brand config. */}
        <div className="md:col-span-2 rounded-lg border border-sol-glass-border-dark bg-sol-cream/50 p-4">
          ...
          {product && (
            <div className="mb-5 mt-4 rounded-lg border border-[color-mix(in_oklab,var(--cw-brand)_30%,transparent)] bg-[color-mix(in_oklab,var(--cw-brand)_5%,transparent)] p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="text-sm font-black text-[var(--cw-brand-deep)]">
                    🎬 Cinematic Video Banner (M3)
                  </h3>
                  <p className="mt-0.5 text-xs text-[var(--cw-brand-deep)]">
                ...
                <button
                  ...
                  className="rounded-lg bg-[var(--cw-brand)] px-4 py-2 text-sm font-black text-white transition hover:bg-[var(--cw-brand-deep)] disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              {videoNotice && (
                <p className="mt-3 text-xs font-bold text-[var(--cw-brand-deep)]">{videoNotice}</p>
              )}
              {product.videoUrl && (
                <div className="mt-3">
                  ...
                  <video 
                    ...
                    controls 
                    className="h-32 rounded-lg border border-[color-mix(in_oklab,var(--cw-brand)_25%,transparent)]" 
                    autoPlay 
                    loop 
                    muted 
                  />
          ...
          <div className="space-y-4">
            ...
            {/* Shop-Starter Task G: optional attributes field for fork shops.
                Solbrillen can leave it empty (frameColor + lensColor are still
                the primary fields). Panel fencing/agriculture use it for custom
                specifications (height, width, material, weight, origin). */}
            <div>
        ...
        {/* AEO (answer-first) content — gated on brand.features.aeoContent.
            When the flag is off, any existing values are preserved via hidden inputs
            (so a save does not clear them by accident) — only the UI is hidden. */}
        {aeoEnabled ? (
          <div className="md:col-span-2 rounded-lg border border-sol-glass-border-dark bg-sol-cream/50 p-4">
            <div className="mb-3">
              <h3 className="text-sm font-black text-sol-ink">
                AEO content (answer-first)
              </h3>
              <p className="mt-0.5 text-xs text-sol-muted">
                Helps AI answer engines cite the product. Shown on the product page +
                emitted as FAQPage JSON-LD when filled in.
              </p>
            </div>
            <div className="space-y-4">
              <div>
                <label htmlFor="answerSummary" className={labelClass}>
                ...
                <textarea
                  ...
                  placeholder="One direct answer: what the product is, and who it's for."
                  className={inputClass}
                />
                <p className="mt-1 text-xs text-sol-muted">
                  {answerSummary.length}/500 · shown as a highlighted lead at the top of the product page
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-sol-muted">
                  3-5 entries. Each question must stand on its own (self-contained context) → FAQPage JSON-LD.
                </p>
              </div>

              <div>
                <label htmlFor="useCases" className={labelClass}>
                  Use cases (JSON: {"[{ title, description }]"})
                </label>
                <textarea
                  id="useCases"
                  name="useCases"
                  ...
                  placeholder={`[{"title": "For everyday use", "description": "…"}]`}
                  className={`${inputClass} font-mono text-xs`}
                />
              </div>

              <div>
                <label htmlFor="comparisonFacts" className={labelClass}>
                  Comparison facts (JSON object)
                </label>
                <textarea
                  id="comparisonFacts"
                  name="comparisonFacts"
                  rows={4}
                  value={comparisonJson}
                  onChange={(e) => setComparisonJson(e.target.value)}
                  placeholder={`{"vs_standard": "…", "key_advantage": "…"}`}
                  className={`${inputClass} font-mono text-xs`}
                />
              </div>
            </div>
          ...
          <>
            <input type="hidden" name="answerSummary" value={answerSummary} />
            <input type="hidden" name="faq" value={faqJson} />
            <input type="hidden" name="useCases" value={useCasesJson} />
            <input type="hidden" name="comparisonFacts" value={comparisonJson} />
          </>
        )}
        ...
        <div className="md:col-span-2">
          ...
          {/* Image upload: append to the images state. Action: upload → URL → push. */}
          <ImageUpload
            onUploaded={(url) => setImages((curr) => [...curr, url])}
            buttonLabel="Upload image"
          />
          {/* Preview: show the current images with an X button to remove */}
          {images.length > 0 && (
          ...
          {/* The hidden textarea is kept so the server action still receives images.
              Backwards compatibility: an admin can still paste URLs manually if needed. */}
          <textarea
            id="images"
            name="images"
            rows={3}
        ...
        {/* Task B: variants admin. Only rendered for existing products
            — on the "new product" form there is no productId to relate to.
            The admin creates the product first, then edits it to add variants. */}
        {product && (
          <VariantsAdmin
            productId={product.id}
