...
/**
 * Shown in LocalAiForm when Ollama is running but 0 models are pulled.
 * Three cards (e2b/e4b/26b) with hardware guidance + a "Pull this model" button
 * that calls /api/admin/ai/ollama-pull and streams progress via
 * Server-Sent Events.
 *
 * Apple Silicon detection: in v1 we use navigator.userAgent. We cannot
 * reliably detect x86 vs ARM from the browser. So: always show the -mlx
 * variant as the recommendation on Mac. On Linux/Windows -mlx is hidden.
 */
    ...
    <div className="mt-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <p className="text-[10px] text-sol-muted">
          {isMac ? "MLX variant selected automatically for Apple Silicon" : ""}
        </p>
      </div>
...
function useIsMac(): boolean {
  // Lazy initializer — runs only on the first render. Returns false on the server
  // (SSR) and in post-hydration cases without a navigator. A hydration mismatch
  // is acceptable here: the button label "Pull <name>" is identical regardless of
  // isMac; only the mlx variant choice changes post-mount.
