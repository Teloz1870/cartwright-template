...
/** YYYY-MM-DD for <input type=date>, or "" if null. */
  ...
  function save() {
    ...
    startTransition(() => {
      void (async () => {
        ...
        setMessage(res.ok ? "Tracking saved" : res.error);
  ...
  return (
    <section className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-5 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className={labelClass}>Expected delivery from</label>
          <input
            type="date"
            value={from}
        ...
        <div>
          <label className={labelClass}>Expected delivery by</label>
          <input
            type="date"
            value={to}
