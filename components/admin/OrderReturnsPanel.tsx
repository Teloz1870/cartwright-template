...
export default function OrderReturnsPanel({
  ...
  function submitNewReturn() {
    ...
    if (chosen.length === 0) {
      setMessage("Vælg mindst én vare");
      return;
    }
    if (!reason.trim()) {
      setMessage("Angiv en årsag");
      return;
    }
    run(() => createReturn(orderId, { items: chosen, reason }), "Retur oprettet");
    setQty({});
    setReason("");
  }
  ...
  function doRefund(returnId: string) {
    ...
    run(() => refundReturn(returnId, Math.round(kr * 100)), "Retur refunderet");
  }

  return (
    <section className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-5 shadow-sm">
      <h2 className="mb-4 text-xl font-black text-sol-ink">Returneringer</h2>
            ...
            >
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-xs text-sol-muted">
                  {r.items.reduce((s, i) => s + i.quantity, 0)} vare(r)
                  {r.refundDkk > 0 && ` · refund ${formatPriceDkk(r.refundDkk)}`}
                  {r.restocked && " · lager genindsat"}
                </span>
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-end gap-2">
                {r.status === "requested" && (
                  <>
                    <button
                      onClick={() => run(() => approveReturn(r.id), "Retur godkendt")}
                      disabled={busy}
                    ...
                    <button
                      onClick={() => run(() => rejectReturn(r.id, ""), "Retur afvist")}
                      disabled={busy}
                ...
                {r.status === "approved" && (
                  <button
                    onClick={() =>
                      run(() => receiveAndRestock(r.id), "Retur modtaget + lager genindsat")
                    }
                    disabled={busy}
                    className="rounded-lg bg-sol-accent px-3 py-1.5 text-xs font-black text-white disabled:opacity-50"
                  >
                    Modtag + genindsæt lager
                  </button>
                )}
          ...
          {items.map((it) => (
            ...
            >
              <span className="text-sol-ink">
                {it.productName}{" "}
                <span className="text-sol-muted">(bestilt {it.quantity})</span>
              </span>
              <input
                type="number"
                min={0}
