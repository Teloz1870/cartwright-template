...
export default function PageForm({ page }: PageFormProps) {
  ...
  function handleAutoTranslate() {
    ...
    if (!daTitle && !daBody) {
      setAiNotice("Please fill in the source texts first.");
      return;
    }

    setAiNotice("Translating texts to English via AI...");
    setError(null);
      ...
      void (async () => {
        try {
          ...
          if (!res.ok || data.error) {
            ...
            setEnBody(data.body || "");
            setAiNotice("✨ Texts translated to English via AI! Remember to save the form.");
          }
        } catch (err) {
          setAiNotice(null);
    ...
    <div className="grid lg:grid-cols-2 gap-8 items-start">
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-black text-sol-ink">Edit Page</h2>
          <button
            type="submit"
            disabled={isPending}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2 rounded-xl border border-sol-ink/10 bg-sol-sand/30 p-4 mt-2">
            <p className="mb-3 text-xs font-black uppercase text-sol-muted">Hero Image</p>
            <div>
              <label htmlFor="heroImage" className="block text-sm font-semibold text-sol-ink mb-1">
                Image URL (Unsplash or similar)
              </label>
              <input
                id="heroImage"
                name="heroImage"
          ...
          <div className="md:col-span-2 rounded-xl border border-sol-ink/10 bg-sol-sand/30 p-4">
            ...
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm font-semibold text-sol-ink">Show in menu</span>
              </label>
              <div className="flex items-center gap-2">
                <label htmlFor="navOrder" className="text-sm font-semibold text-sol-ink">Order:</label>
                <input
                  id="navOrder"
                  name="navOrder"
                  type="number"
      ...
      <div className="hidden lg:block sticky top-8">
        ...
        <div className="rounded-2xl border border-sol-ink/20 shadow-2xl overflow-hidden bg-sol-cream h-[800px] overflow-y-auto">
          <div className="pointer-events-none origin-top scale-[0.8] w-[125%] h-full">
             <AnimatedPageContent 
               page={{ title: title || "Untitled", heroImage: heroImage || null }}
               blocks={blocks} 
             />
          </div>
        </div>
