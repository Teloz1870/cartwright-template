...
// A curated set of bulk targets — operator-meaningful forward moves. Illegal
// transitions are rejected per order by bulkUpdateStatus and reported as skipped.
const BULK_TARGETS = [
  "processing",
  "shipped",
  "delivered",
...
export default function OrdersWorkspace({
  ...
  // Resetting on filter change is handled via `key` on the parent (a server
  // re-render remounts the component with fresh initial props) — hence no
  // setState-in-effect here (cf. react-hooks/set-state-in-effect).
      ...
      void (async () => {
        ...
        const skippedNote =
          res.skipped.length > 0
            ? ` — ${res.skipped.length} skipped (illegal transition)`
            : "";
        setMessage(`${res.updated} order(s) updated${skippedNote}`);
        setSelected(new Set());
        router.refresh();
      })();
    });
    ...
    <div className="flex flex-col gap-4">
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-sol-accent/30 bg-sol-accent/5 p-3">
          ...
          >
            {isBulking ? "Updating…" : "Apply to selected"}
          </button>
        </div>
      )}
