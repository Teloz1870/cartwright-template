  ...
  return [
    {
      ...
      description:
        "Search this store's product catalogue by free text and return matching products (id, title, price with currency, url, stock). Returns results directly without changing the page.",
      inputSchema: {
        type: "object",
        properties: {
          query: {
    ...
    },
    {
      name: "get_cart",
      description:
        "Read the current shopping cart: line items with ids (usable with the cart page's update and remove tools — navigate to '/cart' to use them), quantities, per-line and total prices with currency, and stock ceilings.",
    ...
    },
    {
      name: "navigate",
      description:
        "Navigate the browser to an internal page on this store, e.g. '/produkter', '/produkter?q=oak', or '/product/<slug>'. Accepts same-origin paths beginning with '/'.",
      inputSchema: {
        type: "object",
        properties: {
          path: {
