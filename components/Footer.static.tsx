...
export default async function Footer() {
  ...
  const footerTagline = g
    ? await readField("footer.tagline").catch(() =>
        localizedBrandCopy("footer.tagline", brand.footer.tagline, locale),
      )
    : localizedBrandCopy("footer.tagline", brand.footer.tagline, locale);
  const footerDisclaimerRaw = g
    ? await readField("footer.disclaimer").catch(() =>
        localizedBrandCopy("footer.disclaimer", brand.footer.disclaimer, locale),
      )
    : localizedBrandCopy("footer.disclaimer", brand.footer.disclaimer, locale);
  // Same fallback as the db variant. `brand.footer.disclaimer` defaults to ""
  // (a config default must not pick the language for a locale-routed
  // storefront), so without this the site profile renders an EMPTY <p> where
  // the copyright line belongs — the db footer got the localized fallback and
  // this twin did not.
        ...
        <section className={`relative border-b border-white/10 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-white/30 before:to-transparent ${isSaas ? "bg-black" : "bg-sol-accent"}`}>
          <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-10 sm:px-6 md:flex-row md:items-center md:justify-between md:gap-10 lg:px-8 lg:py-12">
            <div className="max-w-md">
              ...
              {/* Same rule as the db Footer: dark chrome is a theme hint, not
                  an identity — always render the brand's own newsletter copy. */}
