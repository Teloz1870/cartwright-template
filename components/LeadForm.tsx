...
/**
 * Failure codes from `/api/inquiries` → translation keys in this namespace.
 * Anything unmapped falls back to the generic message, so a new server code
 * degrades to a translated string instead of surfacing a raw identifier.
 */
const INQUIRY_ERROR_KEY: Record<string, string> = {
  invalid_name: "errorName",
  ...
  data_too_large: "errorGeneric",
  data_too_many_keys: "errorGeneric",
  source_too_long: "errorGeneric",
  data_reserved_key: "errorGeneric",
  data_too_deep: "errorGeneric",
};
...
export default function LeadForm({
  ...
  // Instance-scoped field ids so two LeadForms on one page never collide
  // (label htmlFor → input id binding stays correct per instance).
  const uid = useId();
  ...
  // Was a Danish literal array rendered in every locale. The values are copy,
  // not data — an English lead saw Danish budget brackets.
  const budgets = [
    t("budgetUnder"),
    t("budgetMid"),
    t("budgetOver"),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        // The route answers with a machine code, never prose: it has no locale
        // segment and so cannot know the visitor's language. Translating here
        // is what keeps an English shop from showing a Danish error.
        setError(t(INQUIRY_ERROR_KEY[result.code as string] ?? "errorGeneric"));
      }
    } catch {
  ...
  return (
    <div className="bg-black/60 backdrop-blur-2xl rounded-3xl p-8 sm:p-10 border border-white/10 shadow-2xl relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-[var(--cw-brand-on-dark)]/10 rounded-full blur-[80px] pointer-events-none" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {budgets.map((b) => (
                    <button
                      ...
                      className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 text-left ${
                        budget === b 
                          ? "bg-[var(--cw-brand-on-dark)]/20 border-[var(--cw-brand)] text-white shadow-[0_0_15px_rgba(168,85,247,0.3)]" 
                          : "bg-white/5 border-white/10 text-slate-300 hover:bg-white/10 hover:border-white/20"
                      }`}
                    >
                      <span className="font-semibold">{b}</span>
                      {budget === b && <CheckCircle2 className="text-[var(--cw-brand-on-dark)]" size={20} />}
                    </button>
                  ))}
                </div>
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor={`${uid}-name`} className="block text-sm font-medium text-slate-400 mb-1">{t("nameLabel")}</label>
                      <input
                        id={`${uid}-name`}
                        required
                        type="text"
                    ...
                    <div>
                      <label htmlFor={`${uid}-email`} className="block text-sm font-medium text-slate-400 mb-1">{t("emailLabel")}</label>
                      <input
                        id={`${uid}-email`}
                        required
                        type="email"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor={`${uid}-phone`} className="block text-sm font-medium text-slate-400 mb-1">{t("phoneLabel")}</label>
                      <input
                        id={`${uid}-phone`}
                        required
                        type="tel"
                    ...
                    <div>
                      <label htmlFor={`${uid}-company`} className="block text-sm font-medium text-slate-400 mb-1">{t("companyLabel")}</label>
                      <input
                        id={`${uid}-company`}
                        required
                        type="text"
