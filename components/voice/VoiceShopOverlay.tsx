...
/** Failure codes from /api/live/token → keys in the Storefront namespace. */
const VOICE_ERROR_KEY: Record<string, string> = {
  voice_disabled: "voiceDisabled",
  voice_not_configured: "voiceNotConfigured",
  voice_daily_cap: "voiceDailyCap",
  voice_no_tools: "voiceSessionFailed",
};
...
export default function VoiceShopOverlay({
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // The route answers with a code, never prose: it has no locale
          // segment. `body.error ?? t(…)` could never reach the translation,
          // because a truthy server string never falls through.
          setErrorMsg(
            t(VOICE_ERROR_KEY[body.code as string] ?? "voiceSessionFailed"),
          );
          setState("error");
          return;
        }
        ...
        if (!cancelled) {
          setErrorMsg(err instanceof Error ? err.message : t("voiceError"));
          setState("error");
        }
      }
    })();
  ...
  return (
    // data-cw-voice-shop: stable design hook (DESIGN.md §5).
    <div
      data-cw-voice-shop="overlay"
      role="dialog"
      aria-modal="true"
      aria-label={t("voiceDialogAria")}
    ...
    >
      <div className="flex h-[85vh] w-full flex-col rounded-t-2xl bg-white shadow-2xl md:h-full md:max-w-md md:rounded-l-2xl md:rounded-t-none">
        ...
        <div className="flex items-center justify-between border-b border-sol-ink/10 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            {capabilities.visionEnabled && (
              ...
              >
                {cameraOn ? t("voiceCameraOn") : t("voiceCameraOff")}
              </button>
            )}
            <button
              type="button"
              onClick={onClose}
              aria-label={t("voiceCloseAria")}
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto px-4 py-4 scrollbar-thin scrollbar-thumb-sol-ink/20 scrollbar-track-transparent">
          {state === "connecting" && (
            <p className="text-center text-sm text-sol-muted">
              {t("voiceConnectingService")}
            </p>
          )}
          ...
          {state === "ended" && (
            <div className="mt-4 text-center">
              <p className="text-sm text-sol-muted">{t("voiceSessionEnded")}</p>
              <button
                type="button"
                onClick={onClose}
...
function StateBadge({ state }: { state: VoiceShopState }) {
  // All seven labels were Danish literals. Only "Tænker" carried a marker the
  // language guard could see — "Lytter", "Taler", "Slut", "Fejl" and
  // "Forbinder…" have neither æøå nor a listed word, so six of the seven were
  // invisible to it. The word list has a tail; translating the whole set is
  // what actually closes the surface.
  const t = useTranslations("Storefront");
  const variants: Record<
    VoiceShopState,
    { label: string; className: string }
  > = {
    idle: { label: t("voiceIdle"), className: "bg-sol-cream text-sol-muted" },
    connecting: {
      label: t("voiceConnecting"),
      className: "bg-amber-100 text-amber-900",
    },
    listening: { label: t("voiceListening"), className: "bg-green-100 text-green-900" },
    thinking: { label: t("voiceThinking"), className: "bg-blue-100 text-blue-900" },
    speaking: { label: t("voiceSpeaking"), className: "bg-sol-accent text-white" },
    ended: { label: t("voiceEnded"), className: "bg-sol-cream text-sol-muted" },
    error: { label: t("voiceError"), className: "bg-red-100 text-red-900" },
  };
