...
type Props = {
  ...
  /**
   * Ask the AI triage route (`/api/support/triage`, admin-owned) before the
   * human path. Defaults to the profile's own `supportTriage` capability, so a
   * `--profile site` scaffold — where that route does not exist — skips it
   * without any caller having to remember to pass the prop (the static twin
   * of `lib/profile-capabilities` says false there). Override only in tests.
   */
...
/**
 * Server failure codes → keys in this component's own `tr` dictionary.
 *
 * Neither endpoint this form talks to carries a locale segment, so both answer
 * with a machine code rather than prose. Reading `data.error` directly — as
 * this form did — meant whichever language the server happened to be written
 * in won over the visitor's. Anything unmapped falls back to the generic
 * message, so a new server code degrades to a translated string rather than
 * surfacing a raw identifier.
 */
const CONTACT_ERROR_KEY: Record<string, string> = {
  invalid_name: "invalidName",
  invalid_email: "invalidEmail",
  invalid_input: "somethingWrong",
  send_failed: "somethingWrong",
  // `data`/`source` are only sent by a fork's OWN custom form (a calculator,
  // a quote wizard); this shipped form never sends them, so these codes are
  // unreachable from here. Mapped to the generic string rather than inventing
  // a translation in every locale for a message this form cannot produce.
  data_too_large: "somethingWrong",
  data_too_many_keys: "somethingWrong",
  source_too_long: "somethingWrong",
  data_reserved_key: "somethingWrong",
  data_too_deep: "somethingWrong",
  file_too_large: "fileTooLarge",
  file_wrong_type: "fileWrongType",
  file_content_mismatch: "fileContentMismatch",
  upload_failed: "somethingWrong",
};

function errorText(tr: Record<string, unknown>, code: unknown): string {
  const key = typeof code === "string" ? CONTACT_ERROR_KEY[code] : undefined;
  const value = tr[key ?? "somethingWrong"];
  return typeof value === "string" ? value : String(tr.somethingWrong);
}
...
export default function SmartContactForm({
  ...
  triageEnabled = profileCapabilities.supportTriage,
}: Props) {
  const t = THEMES[mode];
  const en = locale === "en";
  const tr = en
    ? {
        ...
        fileTooLarge: "That image is too large. Maximum 5 MB.",
        fileWrongType: "Only images (JPEG, PNG, WebP) can be attached.",
        fileContentMismatch: "That file's contents do not match its type.",
    ...
    : {
        ...
        fileTooLarge: "Billedet er for stort. Maks 5 MB.",
        fileWrongType: "Kun billeder (JPEG, PNG, WebP) kan vedhæftes.",
        fileContentMismatch: "Filens indhold matcher ikke filtypen.",
  ...
  const handleTriage = async (e: React.FormEvent) => {
    ...
    const work = (async () => {
      try {
        if (!triageEnabled) {
          // No triage route in this profile: the human path IS the path.
    ...
    try {
      ...
      if (attachmentsEnabled && files.length > 0) {
        for (const file of files.slice(0, 3)) {
          ...
          if (!up.ok || !upData.url) {
            setError(errorText(tr, upData.code));
            setTriageStatus('idle');
            return false;
          }
      ...
      setError(errorText(tr, data.code));
      setTriageStatus('idle');
      return false;
    } catch {
      setError(tr.noConnection);
