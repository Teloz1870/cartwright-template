...
import WebMcpIntentPicker from "@/components/WebMcpIntentPicker";
    ...
    <div className="mt-8 space-y-8">
      <section aria-labelledby="webmcp-tools-heading">
        ...
        <p className="mt-1 text-sm opacity-80">
          Tools register <em>per route</em> — progressive disclosure: an agent only ever
          sees the collection that makes sense on the page it is on. Contextual tools
          appear and disappear as you navigate; keep this page open in a second tab,
          browse the shop, and watch the live list below change.
        </p>
      ...
      <section aria-labelledby="webmcp-intent-heading">
        <h2 id="webmcp-intent-heading" className="text-lg font-semibold">
          I want to…
        </h2>
        <p className="mt-1 text-sm opacity-80">
        ...
        <WebMcpIntentPicker
          packIntents={
            "calculate_brew_ratio" in packBindings
              ? [
                  {
                    intent: "Get a brew recipe",
                    tool: "calculate_brew_ratio",
                    where: "homepage (crema design pack)",
                    example: { cups: 4, strength: "balanced" },
                    note: "Same math module as the on-page calculator, so the two can never disagree — and it reads the catalogue (products.search) to name the coffee and the pack count. Read-only: it never touches the cart.",
                  },
                ]
              : []
          }
      ...
      <section aria-labelledby="webmcp-setup-heading">
        ...
        <ul className="mt-2 list-disc space-y-1.5 pl-5 text-sm leading-6 opacity-80">
          ...
          <li>
            <strong>Chrome DevTools</strong> → Application → <strong>WebMCP</strong>: lists
            every tool with its schema, logs invocations, and lets you run a tool by hand.
          </li>
          <li>
            <strong>WebMCP Inspector</strong> (Chrome extension): shows every tool
            registered on the current page and lets you invoke them with custom input.
          </li>
          <li>
            <strong>Any other browser</strong>: this page loads the Chrome team&apos;s
            WebMCP polyfill (Apache-2.0) as a fallback, so the live check below works
            everywhere — the panel says when the polyfill is active.
          </li>
        </ul>
      </section>
