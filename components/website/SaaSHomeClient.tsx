...
export default function SaaSHomeClient({ heroHeadline, heroTagline, heroCta, storeName }: Props) {
    ...
    <div className="relative min-h-screen bg-black text-white font-sans selection:bg-white/30 selection:text-white">
      ...
      <section className="relative z-10 pt-40 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        >
          ...
          <h1 className="text-6xl sm:text-8xl font-black text-white tracking-tighter mb-8 max-w-5xl leading-[1.1]">
            {displayHeadline.split(" ").slice(0, -1).join(" ")}{" "}
            <span className="text-indigo-400">{displayHeadline.split(" ").slice(-1)}</span>
          </h1>
          
          <p className="text-xl sm:text-2xl text-white/60 mb-12 max-w-2xl font-light leading-relaxed">
            {heroTagline}
          </p>

          <form action="/kontakt" className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto max-w-lg mx-auto sm:mx-0 bg-white/5 p-2 rounded-xl border border-white/10 backdrop-blur-sm">
            <input 
              type="email" 
              name="email"
