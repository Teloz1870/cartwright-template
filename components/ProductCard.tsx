...
// Task B: udvidet type så ProductCard kan vise "fra X kr" når produktet har
// varianter med forskellige priser. B3: baseret på den strukturelle
// DesignProduct-kontrakt (en Prisma Product opfylder den uændret), så kortet
// deler kontrakt med design-laget i stedet for ORM-typen.
type ProductWithMaybeVariants = DesignProduct & {
...
export function ProductCard({
  ...
  // Threaded like its two neighbours above. It used to read the static config
  // directly, so an admin who turned the wishlist on got a heart on the PDP
  // (which resolves) and none here — while /account/wishlist 404'd.
  ...
  editEnabled?: boolean;
  /**
   * Locale for the wishlist heart's (invisible) aria-label. Defaults to "da"
   * so every existing caller — and the Danish webshop canaries — render
   * byte-identical Danish labels. The PLP/category grids thread the route
   * locale through so English-first shops get an English accessible name on
  ...
  return (
    <TransitionLink href={productHref} className={cardClass}>
      ...
      <div className="flex flex-col gap-1 p-4">
        ...
        {/* Direct price-edit hook — kun når kortet viser produktets EGEN pris
            (ikke "from <variant-min>", hvor et base-pris-edit ville forvirre). */}
        <p
          className="mt-1 text-lg font-black text-sol-accent"
          {...editAttr(
            { kind: "product", slug: product.slug, field: "price" },
            editEnabled && !showFromPrice,
          )}
        >
          {showFromPrice ? "from " : ""}
