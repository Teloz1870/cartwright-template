...
/**
 * `/privacy` belongs to the `trust-pages` module — opt-in in the `site`
 * profile (owner decision 2026-09-07, CW-23). This banner renders from the
 * ROOT `app/layout.tsx`, so it is on every page of every profile, and it was
 * the one `/privacy` link outside the four footers: a scaffold without the
 * module linked a 404 from its cookie banner, on every page.
 *
 * The fallback is `/info/cookies`, not nothing: a consent banner has to point
 * at a policy, and the info seam target ships in every profile (core's static
 * variant renders the built-in legal content, and `app/sitemap.static.ts`
 * advertises `/info/{terms,cookies}` unconditionally for exactly that reason).
 *
 ...
 * (#550/#551/#572). True in the engine tree, so the canaries are unchanged.
 */
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-sol-muted">
              {t.body}{" "}
              <Link
                href={trustPages ? `/${locale}/privacy` : `/${locale}/info/cookies`}
