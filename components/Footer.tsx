...
import { brandCopyTranslation, localizedBrandCopy } from "@/lib/brand-copy";
...
/**
 * `Boolean(...)`, never `=== true`: in a materialised scaffold this file reads
 * `lib/profile-capabilities.static.ts`, whose values are `as const` literal
 * `false`, so an equality comparison is TS2367 there while every engine gate
 * stays green (#550/#551/#572). Both read `true` in the engine tree and in
 * every database-backed profile, so the canaries render byte-identically.
 *
 * `trustPages` — `app/[locale]/{about,privacy}` belong to the `trust-pages`
 * module, which a `site` scaffold only has with `--with trust-pages`.
 * `engineMarketing` — `/built-with-cartwright` and `/changelog` are engine-only
 * routes (`scaffold/engine-only.ts`) the CLI deletes from a customer's repo.
 * `/info/terms` and `/info/cookies` stay ungated: they are the info seam
 * TARGET, which every profile serves.
 */
...
export default async function Footer() {
  ...
  // About is a stable public route (it resolves both the modern `about` source
  // slug and legacy `om-os`), so its href is written inline inside the
  // `trustPages` gate below rather than hoisted here — a hoisted `const
  // aboutHref` puts the route literal outside its own conditional, where the
  // href sweep cannot see the gate. FAQ stays conditional on the seeded page
  // because it has no checked-in fallback.
  ...
  /**
   * An explicit per-locale translation BEATS the genome.
   *
   * The genome resolves one string per field — `readField` takes a key, not a
   * locale — so on a shop with genomeResolve on it shadowed the shop's own
   * translations entirely. Measured on the eyewear canary after
   * copyTranslations was supplied: /en still read "Alle rettigheder
   * forbeholdes", because the Danish genome value won.
   *
   * Precedence is decided by SPECIFICITY: a value written for this exact
   * locale is more specific than one written for no locale at all. This can
   * only add languages — on the default locale, and on any shop that supplies
   * no translations, `brandCopyTranslation` returns undefined and the genome
   * still wins, byte-identically.
   */
  const resolveCopy = async (
    // The genome's own key union, so a typo cannot silently become a
    // never-resolving field.
    path: Parameters<typeof readField>[0],
    base: string,
  ): Promise<string> => {
    const translated = brandCopyTranslation(path, locale);
    if (translated) return translated;
    if (!g) return localizedBrandCopy(path, base, locale);
    return readField(path).catch(() => localizedBrandCopy(path, base, locale));
  };
  const footerTagline = await resolveCopy("footer.tagline", brand.footer.tagline);
  const footerDisclaimerRaw = await resolveCopy(
    "footer.disclaimer",
    brand.footer.disclaimer,
  );
  // Tom disclaimer ⇒ den lokaliserede standardlinje. Den gamle default var en
  // dansk streng der lækkede uoversat til /en på alle engelske sider — en
  // config-default må aldrig vælge sprog for et locale-routet storefront.
  const footerDisclaimer =
    footerDisclaimerRaw?.trim() ||
    `${settings?.storeName ?? brand.storeName} · ${t("allRightsReserved")}`;
        ...
        <section className={`relative border-b border-white/10 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-white/30 before:to-transparent ${isSaas ? "bg-black" : "bg-sol-accent"}`}>
          <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-10 sm:px-6 md:flex-row md:items-center md:justify-between md:gap-10 lg:px-8 lg:py-12">
            <div className="max-w-md">
              ...
              {/* Dark chrome is a THEME hint, not an identity — the old
                  isSaas branch hardcoded Danish SaaS marketing copy for every
                  dark-chrome shop (a source literal must never pick the
                  language for a locale-routed storefront, and a coffee shop
                  must never advertise "platform features"). The brand's own
                  newsletter copy is correct on every chrome. */}
              <h2
        ...
        <div className="grid gap-10 sm:grid-cols-2 md:grid-cols-4">
          <div>
            <Link
              ...
              aria-label={t("homeAria", { storeName: settings?.storeName ?? brand.storeName })}
            >
        ...
        <div className="mt-6 flex flex-col items-center gap-4 border-t border-white/10 pt-6 text-center text-xs text-white/60 sm:flex-row sm:justify-between sm:text-left">
          <div className="flex flex-col sm:items-start items-center gap-1.5">
            <div className="flex items-center gap-2 flex-wrap justify-center sm:justify-start">
              {engineMarketing && brand.features.cartwrightBadge && (
                <p>
                  {t("builtBy")}{" "}
                  <Link
                    href={`/${locale}/built-with-cartwright`}
            ...
            {engineMarketing && brand.features.cartwrightBadge && (
              <p className="text-[11px] text-white/60">
                {t("wantSimilar")}{" "}
                <Link href={`/${locale}/built-with-cartwright`} className="text-[var(--cw-brand)] underline">
                  {t("readMore")}
                </Link>
              </p>
            )}
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 md:justify-end">
            {engineMarketing && brand.website.showAuditFeed && (
              <Link
