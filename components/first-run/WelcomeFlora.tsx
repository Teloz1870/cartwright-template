 ...
 * hindwings + body + antennae) filled with a translucent vermilion/blue glass
 ...
 *
 * COLOUR (vermilion rebrand, 2026-08-15): the warm stops derive from the brand
 * ramp via `rgb(var(--cw-brand-rgb) / a)` rather than repeating a literal — the
 * purple era hand-copied rgba(124,92,255,…) here, which is why the ornaments
 * survived a change to the five brand variables unaltered.
 *
 * The blue stops (rgba(140,196,255,…) / rgba(130,198,255,…)) are KEPT ON
 * PURPOSE and are not leftovers. Glass reads as glass because it refracts warm
 * AND cool; recolouring every stop vermilion collapses the ornaments into the
 * hero gradient behind them and they stop looking like glass. Against a warm
 * hero the cool stops are complementary, so they separate better than they did
 * on the purple one.
 */
    ...
    >
      ...
      <svg width="0" height="0" className="absolute" aria-hidden>
        <defs>
          <linearGradient id="cwBflyGlass" x1="0" y1="0" x2="1" y2="1">
            ...
            <stop offset="42%" stopColor="rgb(var(--cw-brand-rgb) / 0.18)" />
      ...
      <style>{`
        ...
        .cw-bfly-svg { width: var(--s,72px); height: calc(var(--s,72px) * 0.9); overflow: visible;
          filter: drop-shadow(0 8px 10px rgb(var(--cw-brand-deep-rgb) / 0.30)); }
        ...
        .cw-bbody { fill: rgb(var(--cw-brand-deep-rgb) / 0.62); stroke: rgba(255,255,255,0.5); stroke-width: 0.8; vector-effect: non-scaling-stroke; }
        .cw-bant  { fill: none; stroke: rgb(var(--cw-brand-deep-rgb) / 0.55); stroke-width: 1.4; vector-effect: non-scaling-stroke; stroke-linecap: round; }
        .cw-btip  { fill: rgb(var(--cw-brand-rgb) / 0.7); }
        ...
        .cw-g {
          background:
            linear-gradient(135deg,
              ...
              rgba(255,196,170,0.12) 26%,
              rgb(var(--cw-brand-rgb) / 0.13) 58%,
              rgba(130,198,255,0.19) 100%);
          -webkit-backdrop-filter: blur(5px) saturate(150%);
          backdrop-filter: blur(5px) saturate(150%);
          border: 1px solid rgba(255,255,255,0.45);
          box-shadow:
            0 12px 26px -12px rgb(var(--cw-brand-deep-rgb) / 0.28),
            inset 0 1px 2px rgba(255,255,255,0.7),
            inset 0 -8px 14px -8px rgb(var(--cw-brand-rgb) / 0.22);
        }
        ...
        .cw-flower .core {
          ...
          background: radial-gradient(circle at 35% 30%, rgba(255,255,255,0.9), rgba(251,231,166,0.55) 60%, rgb(var(--cw-brand-rgb) / 0.25));
