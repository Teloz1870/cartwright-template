        ...
        <div className="mx-auto max-w-6xl px-6 py-14">
          <div className="flex flex-col items-center gap-8 text-center sm:flex-row sm:items-end sm:justify-between sm:text-left">
            ...
            <div className="flex flex-col items-center gap-3 sm:items-end">
              {/* `/built-with-cartwright` is engine-only (scaffold/engine-only.ts):
                  the CLI deletes the route from a customer's scaffold, so the
                  first thing a new owner sees must not link it. `Boolean(...)`,
                  never `=== true` — the static twin is `as const` false and an
                  equality comparison is TS2367 there (#550/#551/#572). */}
