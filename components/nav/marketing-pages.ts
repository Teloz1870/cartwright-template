...
export const MARKETING_PAGES: readonly MarketingPage[] = [
  ...
  // The last three are Cartwright's OWN marketing pages, listed in
  // `scaffold/engine-only.ts`: the CLI deletes them from a customer's scaffold,
  // so the nav has to ask whether this tree is the engine's before linking
  // them. In the engine and in every database-backed profile the capability is
  // true and all four render exactly as before.
  { href: "/cases", key: "cases", requires: "engineMarketing" },
  { href: "/priser", key: "pricing", requires: "engineMarketing" },
  { href: "/cartwright", label: "Cartwright", requires: "engineMarketing" },
];
