  ...
  return [
    {
      ...
      description: `Change the quantity of a line in the shopping cart. Quantity 0 removes the line. Current lines: ${lineList(items)}. Returns the updated cart so the result can be verified.`,
      inputSchema: {
        type: "object",
        properties: {
          cartItemId: {
          ...
          quantity: {
            type: "integer",
            description: "New quantity (0–99). 0 removes the line.",
            minimum: 0,
            maximum: 99,
          },
        },
      ...
      async execute(input) {
        ...
        // quantity 0 er DESTRUKTIVT (fjerner linjen) — så coercion er
        // forbudt her: Number(null|false|"") er alle 0, og et misdannet
        // kald må aldrig kunne fjerne noget. Kun et ægte tal accepteres.
        ...
        if (
          ...
          return { error: "quantity must be an integer between 0 and 99." };
        }
        try {
          ...
          const stockWarning =
            line && line.quantity > line.maxQuantity
              ...
              : undefined;
          return {
            status: quantity === 0 ? "removed" : "updated",
            cartItemId,
            quantity,
          ...
          return { error: "Updating the cart failed — the store had a temporary error." };
        }
      },
    },
    {
      ...
      async execute(input) {
        ...
        try {
          ...
          return { error: "Removing the item failed — the store had a temporary error." };
        }
      },
    },
    {
      name: "go_to_checkout",
      description:
        "Open the checkout page so the human can review shipping, payment and place the order themselves. Does NOT place an order.",
      inputSchema: { type: "object", properties: {} },
      execute() {
        // Upræfikset sti — samme konvention som navigate-toolet og
        // kurvsidens egne links; next-intl-middlewaren sætter locale på.
        window.location.assign("/checkout");
        return { status: "navigating", path: "/checkout" };
      },
    },
  ];
}
