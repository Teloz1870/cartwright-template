...
export default async function AdminReviewsPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Reviews"
        subtitle="Moderate customer reviews before they become visible on the PDP. Approved reviews count toward AggregateRating JSON-LD once there are at least 3 approved per product."
      />
      ...
      {reviews.length === 0 ? (
        <AdminCard>
          <p className="text-sm text-sol-muted">
            No reviews in &quot;{status}&quot;.
            {status === "pending" && (
              <span className="block mt-2 text-xs">
                Customers can write reviews via /konto/ordrer/[id]/anmeld (logged-in)
                or via a token link in the post-purchase email (cron sends after 7
                days).
              </span>
            )}
          </p>
          ...
          <AdminTable minWidth="860px">
            <AdminThead>
              <tr>
                ...
                <AdminTh>Stars</AdminTh>
                <AdminTh>Author</AdminTh>
                <AdminTh>Excerpt</AdminTh>
