...
type Props = {
  ...
  intensity: number;
  /**
   * MERGED with (never replaces) the full-bleed default
   * `pointer-events-none absolute inset-0 h-full w-full` via `cn()` —
   * pass additions (`opacity-70`, `-z-10`) or overrides (`h-screen` wins
   * over the default `h-full` through tailwind-merge).
   */
  className?: string;
};
