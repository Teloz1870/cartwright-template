...
export function ThreeDForm({ initial }: { initial: ThreeDUiData }) {
  ...
  function save(next: {
    ...
    startTransition(async () => {
      ...
      if (res.ok) setMsg({ ok: true, text: "Saved — takes effect on the storefront within 30 sec." });
    ...
    <div className="flex flex-col gap-8">
      <section>
        ...
        <p className="mb-4 text-xs text-sol-muted">
          Choose the built-in 3D scene. Colors are pulled automatically from your theme.
        </p>
      ...
      <section className="max-w-md">
        <h2 className="mb-1 text-lg font-black text-sol-ink">Intensity</h2>
        <p className="mb-3 text-xs text-sol-muted">
          Density, speed and amplitude. {Math.round(intensity * 100)}%
        </p>
        <input
          type="range"
          min={0}
      ...
      <section className="max-w-md">
        <h2 className="mb-1 text-lg font-black text-sol-ink">Color source</h2>
        <label className="mt-2 flex items-center gap-3">
          ...
          <span className="text-sm text-sol-ink">
            Use theme colors (recommended) — the scene matches your brand automatically
          </span>
        </label>
      </section>
