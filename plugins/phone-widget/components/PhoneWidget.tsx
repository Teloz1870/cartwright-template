...
export function PhoneWidget({ workspaceId }: { workspaceId?: string }) {
  ...
  if (workspaceId) {
    return (
      <>
        ...
        />
        <div
          data-cw-phone-widget
          className="fixed bottom-6 left-6 z-50 bg-green-500 text-white text-xs px-2 py-1 rounded-full animate-pulse shadow-lg"
        >
  ...
  return (
    // data-cw-phone-widget: stable design hook for custom designs — target
    // [data-cw-phone-widget] instead of Tailwind selectors. See DESIGN.md §5.
    <div data-cw-phone-widget className={`fixed ${bottomClass} left-6 z-50`}>
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            ...
            <div className="p-6 flex flex-col items-center text-center">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-4 transition-colors duration-500 ${
                ...
                callState === "ringing" ? "bg-sol-accent/20 text-sol-accent-light animate-pulse" :
              ...
              <div className="flex gap-4 w-full justify-center">
                {callState === "idle" ? (
                  <button 
                    onClick={startCall}
                    className="flex-1 py-3 bg-sol-accent hover:bg-sol-accent-light text-white rounded-xl font-bold shadow-lg shadow-black/20 transition-all active:scale-95"
                  >
                    Start Web Call
                  </button>
                ) : (
      ...
      {!isOpen && (
        <motion.button
          ...
          className="w-16 h-16 bg-sol-accent text-white rounded-full flex items-center justify-center shadow-2xl shadow-black/30 border border-white/20 group relative"
          aria-label="Open phone widget"
        >
          <div className="absolute inset-0 rounded-full bg-sol-accent/30 animate-ping" style={{ animationDuration: "3s" }} />
