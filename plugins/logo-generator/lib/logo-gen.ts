/**
 * logo-generator plugin (cartwright-plugin-v1) — Gemini raster generation.
 * Moved from lib/ai/logo-gen.ts (a shim keeps that path). The Gemini client
 * layer (@/lib/ai/gemini) is SHARED core (media alt-text, embeddings,
 * media-ai cron also use it) and deliberately stays in core.
 */
import "server-only";
...
export async function generateLogoImage(prompt: string): Promise<LogoGenResult> {
  ...
  if (!key) {
    return {
      ok: false,
      error:
        "No Google Gemini key. Set one in /admin/integrations, or GOOGLE_GEMINI_API_KEY in .env.",
    };
  }
  ...
  try {
    ...
    if (error instanceof GeminiSafetyBlock) {
      return { ok: false, error: "Gemini blocked the prompt (safety). Try a different wording." };
    }
    if (error instanceof GeminiRateLimit) {
      return { ok: false, error: "Gemini rate limit reached. Try again shortly." };
    }
    if (error instanceof GeminiApiError) {
      return { ok: false, error: "The Gemini call failed. Try again." };
    }
    return { ok: false, error: "Unexpected error while generating the logo." };
  }
}
