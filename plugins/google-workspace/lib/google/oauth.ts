...
import { composeGoogleScopes } from "@/plugins/google-workspace/lib/google/scopes";
...
type RefreshResult =
  | { ok: true; accessToken: string; tokenExpiresAt: Date | null }
  | GoogleOAuthInertResult;

// Single-flight guard: collapse concurrent refreshes within one server instance
// into ONE token request. Without it, parallel admin tasks could each POST the
// refresh token, then race their DB writes — clobbering a rotated refresh token
// or persisting a transient error state that disconnects the connection. (Cross-
// instance refreshes remain possible but Google tolerates refresh-token reuse and
// the last successful write wins with a valid token.)
let inFlightRefresh: Promise<RefreshResult> | null = null;

export function refreshGoogleConnectionAccessToken(): Promise<RefreshResult> {
  if (inFlightRefresh) return inFlightRefresh;
  inFlightRefresh = doRefreshGoogleConnectionAccessToken().finally(() => {
    inFlightRefresh = null;
  });
  return inFlightRefresh;
}

async function doRefreshGoogleConnectionAccessToken(): Promise<RefreshResult> {
  const credentials = await getGoogleOAuthCredentials();
  if (!credentials) {
    return {
      ok: false,
