...
import PdpWebMcpMount from "@/components/webmcp/PdpWebMcpMount";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const productName = await getDynamicTranslation(product, "name", product.name, locale);
  const productDescription = await getDynamicTranslation(product, "description", product.description, locale);
...
export default async function ProductPage({ params }: Props) {
  ...
  const productName = await getDynamicTranslation(product, "name", product.name, locale);
  const translatedDescription = await getDynamicTranslation(product, "description", product.description, locale);
  // Per-entity voiced copy (genomeEntityCopy, default-off): prefer a genome
  // entity-override over the entity's own description. Flag-off → identical.
  const productDescription = (brand.features as { genomeEntityCopy?: boolean }).genomeEntityCopy
    ? await readEntityCopy("product", product.id, "description", translatedDescription)
    : translatedDescription;
  const productCategoryName = product.category
    ? await getDynamicTranslation(product.category, "name", product.category.name, locale)
    : null;
  ...
  const relatedProducts = await Promise.all(
    relatedProductsBase.map(async (relatedProduct) => ({
      ...relatedProduct,
      name: await getDynamicTranslation(relatedProduct, "name", relatedProduct.name, locale),
      description: await getDynamicTranslation(relatedProduct, "description", relatedProduct.description, locale),
    })),
  );
  ...
  const breadcrumbItems: Array<Record<string, unknown>> = [
    { "@type": "ListItem", position: 1, name: homeBreadcrumbLabel(locale), item: brand.url },
    {
      "@type": "ListItem",
      position: 2,
  ...
  const pdpBreadcrumbs: { label: string; href?: string }[] = [
    ...
    ...(product.category
      ? [
          {
            label: productCategoryName ?? product.category.name,
  ...
  // Design-owned PDP layout (DesignPack.webshop.pdpLayout) — wraps the whole
  // product-detail tree so a design can frame the shop's PDP. Unset → the tree
  // renders directly (byte-identical). The functional body (gallery, variants,
  // add-to-cart) is untouched inside.
  const activeDesign = await getActiveDesign().catch(() => null);
  const PdpLayout = activeDesign?.webshop?.pdpLayout;
  // Visible breadcrumb: runtime flag on AND the active design doesn't own one
  // (halo's PdpLayout draws its own → avoid doubling). Flag off → byte-identical.
  // `brand` is already the merged getBrand() result (top of this component), so
  // this honors the runtime /admin/features toggle without a second fetch.
  const showPdpBreadcrumb =
    brand.features.breadcrumbs && !activeDesign?.webshop?.ownsBreadcrumb;
  // Attributes the design's own PDP frame renders for THIS product — skipped by
  // the shared spec table so one fact is not printed twice. Asked per product,
  // not declared once: crema draws its roast band only for values its parser
  // accepts, and a blanket list would hide a value it declined to draw. Only
  // when the pack actually supplies the layout (DesignPack.webshop.ownsAttributes).
  const ownsAttributes = PdpLayout ? activeDesign?.webshop?.ownsAttributes : undefined;
  const pdpTree = (
    <div className="pb-24 md:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {showPdpBreadcrumb ? (
          <div className="mb-6">
            <Breadcrumbs items={pdpBreadcrumbs} />
          </div>
        ) : null}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
            ...
            >
              {product.videoUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-sol-sun/30">
                  <span className="text-sol-muted text-sm">{t("noImage")}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* Direct price-edit hook — kun på simple-pris-benet (et base-pris-
                edit mens "from <variant-min>" vises ville forvirre). */}
            <p
              className="text-sol-accent font-black text-3xl"
              {...editAttr(
                { kind: "product", slug: product.slug, field: "price" },
                editEnabled && !(hasVariants && minVariantPrice !== product.priceDkk),
              )}
            >
              {hasVariants && minVariantPrice !== product.priceDkk ? (
                <>
                  {t("priceFrom")} <Price oere={minVariantPrice} />
                </>
              ) : (
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm border-t border-sol-ink/10 pt-5">
              {product.category && (
                <>
                  <dt className="text-sol-muted font-medium">{t("category")}</dt>
              ...
              {(() => {
                // Locale-variant fra translations.<locale>.attributes merges
                // over basen (lib/product-attributes) — base-locale er urørt.
                const attrs = localizedAttributes(product, locale) as
                  | Record<string, unknown>
                  | null;
                // specTableRows narrows, formats and labels in one pass, and
                // skips whatever the active design already draws itself.
                // Before, this filtered on `typeof v === "string"`, so the
                // engine's own coffee seed (`roast: 2`, `notes: [...]`) was
                // authored, stored — and then silently absent from the page.
                // `ownedAttributes` is the ownsBreadcrumb idea for attributes:
                // crema's PDP frame draws roast/origin/process itself, and
                // origin+process were already printed twice before this change.
                const attrRows = specTableRows(
                  attrs,
                  ownsAttributes?.(attrs ?? {}, { hasVariants: product.variants.length > 0 }) ?? [],
                );
                if (attrRows.length > 0) {
                  return attrRows.map(({ key, value }) => (
                ...
                return (
                  <>
                    {product.frameColor && (
                      <>
                        <dt className="text-sol-muted font-medium">{t("frameColor")}</dt>
                        <dd className="text-sol-ink">{product.frameColor}</dd>
                      </>
                    )}
                    {product.lensColor && (
                      <>
                        <dt className="text-sol-muted font-medium">{t("lensColor")}</dt>
      ...
      <PDPStickyAtcBar
        ...
        inStock={inStock}
      />
      {/* Flag-gated WebMCP-mount (gaten bor i mountet; null når off). Inde i
          pdpTree så design-packs' PdpLayout-wraps bevarer den. Genbruger
          sidens deriverede værdier: oversat navn + castede variantOptions. */}
      <PdpWebMcpMount locale={locale} product={{ id: product.id, name: productName, slug: product.slug, priceDkk: product.priceDkk, stock: product.stock, attributes: localizedAttributes(product, locale), variants: variantOptions }} />
    </div>
  );
