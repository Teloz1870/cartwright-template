...
import WebsiteHomeClient from "@/components/website/WebsiteHomeClient";
import SaaSHomeClient from "@/components/website/SaaSHomeClient";
import StudioHomeClient from "@/components/website/StudioHomeClient";
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const headline = settings?.websiteHeadline || brand.metadata.title;
  const tagline = settings?.tagline || brand.tagline;
  const cta = settings?.heroCta || "Book Møde";
  ...
  const isTeloz = storeName.toLowerCase().includes("teloz");

  // Website Mode (CMS only)
  if (!ecommerceEnabled) {
    if (industryTemplate === "saas") {
      return (
        <SaaSHomeClient
    ...
    // Studio (v0.6.0) — cartwright.app-inspired premium-warm-tech design.
    // Læser brand.website.* + DB BrandingSettings (websiteHeadline/tagline/heroCta).
    if (industryTemplate === "studio") {
      return <StudioHomeClient settings={settings} />;
    }
  ...
  // ==========================================
  // VIBE TEMPLATE MODE (Software 3.0)
  // ==========================================
  if (homePage?.vibeHtml) {
    // Tjek databasens `translations`-objekt (Software 3.0 Localization)
  ...
  // ==========================================
  // E-COMMERCE MODE (Webshop Default)
  // ==========================================
  return (
