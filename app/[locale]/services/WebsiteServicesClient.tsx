...
export default function WebsiteServicesClient() {
  ...
  return (
    <div className="min-h-screen bg-black pt-32 pb-24">
      {/* Background glow */}
      <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-[var(--cw-brand-on-dark)]/5 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <h1 className="text-6xl sm:text-8xl font-black text-white tracking-tighter mb-6 leading-[1.1]">
            Ydelser der flytter <span className="text-[var(--cw-brand-on-dark)]">grænser.</span>
          </h1>
        ...
        <div className="space-y-8">
          {fullServices.map((service, index) => (
            ...
            >
              {/* Highlight line */}
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-[var(--cw-brand)] transform scale-y-0 group-hover:scale-y-100 origin-top transition-transform duration-500" />
              ...
              <div className="flex flex-col lg:flex-row gap-12 lg:items-center">
                ...
                <div className="lg:w-1/3 flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end justify-between lg:justify-center gap-6 border-t lg:border-t-0 lg:border-l border-white/10 pt-8 lg:pt-0 lg:pl-12">
                  <div className="text-left lg:text-right">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--cw-brand-on-dark)] mb-2">Prisindikation</p>
                    <p className="text-2xl font-medium text-white">{service.price}</p>
                  </div>
                  <Link href="/contact" className="inline-flex items-center justify-center rounded-full px-6 py-3 text-sm font-bold tracking-wide bg-white !text-black hover:bg-[var(--cw-brand)] hover:text-white w-full sm:w-auto transition-colors">
                    Start Dialog
                  </Link>
                </div>
              </div>
