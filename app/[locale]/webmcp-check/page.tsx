...
export default async function WebMcpCheckPage() {
  ...
  return (
    <main className="mx-auto max-w-2xl p-8">
      {/* GoogleChromeLabs' WebMCP polyfill (Apache-2.0, vendored verbatim at
          public/vendor/): its first statement returns when document.modelContext
          already exists, so native browsers pay nothing — browsers WITHOUT
          WebMCP get a working document.modelContext so this page can
          demonstrate the tool surface anywhere. Loaded synchronously ON THIS
          PAGE ONLY, so it is installed before the registrar's effects run;
          the live panel below discloses when the polyfill is active. */}
      {/* eslint-disable-next-line @next/next/no-sync-scripts -- must define document.modelContext before hydration effects register tools */}
      <script src="/vendor/webmcp-polyfill.js" />
