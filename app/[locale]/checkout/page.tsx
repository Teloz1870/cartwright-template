...
export default async function CheckoutPage() {
  ...
  // Hoisted ABOVE the legacy branch on purpose: it used to be declared after
  // it, so the four price calls in that branch had no locale in scope — and
  // that is the branch a shop without `designSurfaces` actually renders.
  const [cart, stripeReady, locale, chargeCurrency] = await Promise.all([
    getCart(),
    isStripeReady(),
    getLocale(),
    getCheckoutCurrency(),
  ...
  const fmt = (minorBase: number) =>
    formatPrice(minorBase, { currency: chargeCurrency, locale });
            ...
            <div className="rounded-2xl border border-sol-ink/10 bg-sol-cream p-6 shadow-sm lg:sticky lg:top-6">
                ...
                {items.map((item) => (
                  <li key={item.id} className="flex justify-between gap-4 text-sm">
                    ...
                    <span className="whitespace-nowrap font-bold text-sol-ink">
                      {fmt((item.variant?.priceDkk ?? item.product.priceDkk) * item.quantity)}
                    </span>
                  </li>
                ))}
              </ul>
              ...
              <div className="space-y-2 border-t border-sol-ink/10 pt-4 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-sol-ink">{fmt(subtotalDkk)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sol-muted">{t("shipping")}</span>
                  <span className="font-semibold text-sol-ink">
                    {shippingDkk === 0 ? t("free") : fmt(shippingDkk)}
                  </span>
                </div>
                <div className="flex justify-between border-t border-sol-ink/10 pt-3 text-base">
                  ...
                  <span className="font-black text-sol-ink">{fmt(totalDkk)}</span>
                </div>
              </div>
            </div>
          </aside>
              ...
              <div className={`space-y-2 border-t pt-4 text-sm ${surface.divider}`}>
                <div className="flex justify-between">
                  ...
                  <span className={`font-semibold ${surface.ink}`}>
                    {fmt(adaptiveBreakdown!.subtotalDkk)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className={`flex justify-between border-t pt-3 text-base ${surface.divider}`}>
                  ...
                  <span className={`font-black ${surface.ink}`}>
                    {fmt(adaptiveBreakdown!.totalDkk)}
                  </span>
                </div>
              </div>
            </div>
