...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (locale === "en" && category.translations) {
    const translations = category.translations as { en?: { name?: string; description?: string; descriptionLong?: string } } | null;
    if (translations?.en?.name) categoryName = translations.en.name;
    if (translations?.en?.description) categoryDescription = translations.en.description;
  }
...
export default async function CategoryPage({ params }: Props) {
  ...
  let categoryName = category.name;
  let categoryDescription = category.description;
  let categoryDescriptionLong = category.descriptionLong;

  if (locale === "en" && category.translations) {
    const translations = category.translations as { en?: { name?: string; description?: string; descriptionLong?: string } } | null;
    if (translations?.en?.name) categoryName = translations.en.name;
    if (translations?.en?.description) categoryDescription = translations.en.description;
    if (translations?.en?.descriptionLong) categoryDescriptionLong = translations.en.descriptionLong;
  }
