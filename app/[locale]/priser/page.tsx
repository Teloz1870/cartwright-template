...
export default async function PricingPage() {
  ...
  const accentClass = ds ? "text-sol-accent" : "text-indigo-400";
  const heroPClass = ds
  ...
  const fromLabelClass = ds
    ...
    : "text-xs font-semibold uppercase tracking-widest text-indigo-400 mb-3 block";
  const priceClass = ds
  ...
  const checkColor = ds ? "text-sol-accent" : "text-indigo-400";
  ...
  const showcaseClass = ds
    ...
    : "mb-32 rounded-[2.5rem] p-10 lg:p-16 border border-indigo-500/30 bg-gradient-to-r from-[#050510] via-[#0A0A1A] to-[#050510] backdrop-blur-xl relative overflow-hidden group shadow-2xl shadow-indigo-500/5";
  const showcaseBadgeClass = ds
    ...
    : "inline-flex items-center gap-2 px-4 py-1.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-wider rounded-full mb-8";
  ...
  const mockAvatarClass = ds
    ...
    : "w-16 h-16 rounded-full bg-indigo-500/20 flex items-center justify-center mb-6 border border-indigo-500/30";
  ...
  const mockCodeClass = ds
    ...
    : "w-full bg-black/80 rounded-lg border border-white/10 p-4 font-mono text-sm text-left flex items-center justify-between group/code cursor-pointer hover:border-indigo-500/50 transition-colors";
