...
import CartWebMcpMount from "@/components/webmcp/CartWebMcpMount";
...
export default async function KurvPage() {
  ...
  // Canonical, locale-prefixed catalogue link. The unprefixed "/produkter"
  // 307s through the i18n middleware — measured on the live demo, next/link
  // prefetch retried that redirect in a loop (~5 req/s from an idle empty
  // cart). Same class as #459: links follow the CANONICAL routes.
  const [pageLocale, chargeCurrency] = await Promise.all([
    getLocale(),
    getCheckoutCurrency(),
  ]);
  const fmt = (minorBase: number) =>
    formatPrice(minorBase, { currency: chargeCurrency, locale: pageLocale });
  ...
  if (!designSurfaces) {
    if (!hasItems) {
      return (
        <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 px-4 py-20 text-center">
          ...
          <Button href={`/${pageLocale}/produkter`}>{t("viewAllProducts")}</Button>
        </div>
      );
    }
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        ...
        {/* Flag-gated WebMCP-mount (gaten bor i mountet; null når off). */}
        <CartWebMcpMount items={cart.items} />
          ...
          <div className="overflow-hidden rounded-2xl border border-sol-ink/10 bg-sol-cream">
            <div className="flex flex-col divide-y divide-sol-ink/10">
              {cart.items.map((item) => {
                ...
                const lineTotal = (item.variant?.priceDkk ?? item.product.priceDkk) * item.quantity;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <Link
                        href={`/${pageLocale}/product/${item.product.slug}`}
                      ...
                      <p className="mt-0.5 text-xs text-sol-muted">
                        {fmt(item.variant?.priceDkk ?? item.product.priceDkk)} {t("each")}
                      </p>
                      <div className="mt-3">
                        <CartQuantity
                          cartItemId={item.id}
          ...
          <h2 className="mb-5 text-xl font-black text-sol-ink">{t("orderSummary")}</h2>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between text-sm text-sol-ink">
                ...
                <span className="font-bold">{fmt(breakdown.subtotalDkk)}</span>
              </div>
              ...
              <div className="flex justify-between text-base font-black text-sol-ink">
                <span>{t("total")}</span>
                <span>{fmt(breakdown.totalDkk)}</span>
              </div>
            </div>

            <Button href={`/${pageLocale}/checkout`} className="mt-6 w-full">
              {t("goToCheckout")}
            </Button>
  ...
  const body = !hasItems ? (
    ...
    <DesignSurface className={surface.page}>
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
        ...
        {/* Flag-gated WebMCP-mount — samme som legacy-grenen ovenfor. */}
        <CartWebMcpMount items={cart.items} />
          ...
          <div className={`h-fit p-6 lg:sticky lg:top-6 ${surface.card}`}>
            ...
            <div className="flex flex-col gap-3">
              <div className={`flex justify-between text-sm ${surface.ink}`}>
                <span>{t("subtotal")}</span>
                <span className="font-bold">{fmt(adaptiveBreakdown!.subtotalDkk)}</span>
              </div>
              ...
              <div className={`flex justify-between text-sm ${surface.ink}`}>
                ...
                <span className="font-bold">
                  {adaptiveBreakdown!.shippingDkk === 0
                    ? t("free")
                    : fmt(adaptiveBreakdown!.shippingDkk)}
                </span>
              </div>
              ...
              <div className={`flex justify-between text-base font-black ${surface.ink}`}>
                <span>{t("total")}</span>
                <span>{fmt(adaptiveBreakdown!.totalDkk)}</span>
              </div>
            </div>
