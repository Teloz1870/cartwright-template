...
const CAPABILITY_DETAILS: Record<string, { blurb?: string; proof?: Proof[] }> = {
  ...
  hoptify: {
    ...
    proof: [{ label: "Import onboarding", href: "/admin/hoptify" }],
  },
  logoGenerator: {
    blurb:
  ...
  webMcp: {
    ...
    proof: [{ label: "Live agent-tools check", href: "/webmcp-check" }],
  },
};
...
export default async function BuiltWithCartwrightPage() {
  ...
  const cardFor = (f: { key: string; label: string; description: string; enabled: boolean }) => {
    const detail = CAPABILITY_DETAILS[f.key];
    return {
      title: f.label,
      blurb: detail?.blurb ?? f.description,
      // Proof links only when the feature is ON for THIS shop — a proof that
      // 404s (flag-gated admin panel or route on a shop that disabled it) is
      // the opposite of proof. The card itself stays: this page is the
      // engine tour, not the shop's settings.
      proof: f.enabled ? (detail?.proof ?? []) : [],
    };
  };
  ...
  return (
    <>
      ...
      {/* Hero band — same soft lilac→blue glass gradient + frosted ornaments as
          ...
          The gradient is HARDCODED light, so the band re-pins its own cw-stone
          ink values to their light-theme literals (the admin-skin mechanism at
          micro scale): under a dark palette bridge (crema) the page body
          correctly inverts, but this band must stay light-on-light-proof —
          without the pin its foam-inverted ink washed out on the lilac.
          Values equal the light defaults ⇒ light shops render identically. */}
      <section className="relative isolate overflow-hidden bg-[linear-gradient(157deg,#dcd5fb_0%,#cfcbf9_34%,#c6cef7_66%,#c4d7f3_100%)] pb-20 pt-28 sm:pt-32 text-cw-stone-900 [--color-cw-stone-300:#d6d3d1] [--color-cw-stone-600:#525252] [--color-cw-stone-900:#171717]">
        <WelcomeFlora />
