...
export default async function BuiltWithCartwrightPage() {
  ...
  const renderCard = (cap: { title: string; blurb: string; proof: Proof[] }) => (
    <article
      key={cap.title}
      className="rounded-3xl border border-cw-stone-200 bg-cw-stone-50/95 backdrop-blur p-7 shadow-[0_30px_70px_-36px_rgba(23,20,17,0.4)] transition duration-300 hover:-translate-y-1 hover:border-[var(--cw-brand)]/40 hover:shadow-[0_44px_80px_-32px_rgba(124,92,255,0.45)]"
    >
        ...
        <div className="relative z-10 mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[var(--cw-brand)] to-[var(--cw-brand-glow-1)] px-7 text-sm font-semibold text-white shadow-[0_12px_30px_-10px_rgba(124,92,255,0.7)] transition hover:from-[var(--cw-brand-glow-2)] hover:to-[var(--cw-brand-glow-3)]"
            >
              cartwright.app →
            </Link>
            <a
