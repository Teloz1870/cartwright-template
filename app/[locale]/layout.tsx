...
/**
 * The shop's own description, in the language of the page.
 *
 * The root layout sets `description` from `brand.metadata.description` — one
 * string — and it sits ABOVE the `[locale]` segment, so it cannot know which
 * language it is describing. Measured on the eyewear canary: /en served a
 * fully Danish `<meta name="description">` while every visible word around it
 * was English. Search engines and shopping agents read that tag; it is the
 * one piece of copy on the page written FOR them.
 *
 * Overriding it HERE is the narrowest place that knows the locale. Next.js
 * merges metadata root → layout → page, so this fills the gap only for pages
 * that set no description of their own; the PDP, contact and privacy pages
 * still win with theirs. And `localizedBrandCopy` returns the base value
 * whenever the shop supplies no translation, so a single-locale shop and every
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Only the three places the root layout writes it. Everything else in the
  // root's metadata — metadataBase, og:image, twitter card type — is untouched
  // and still inherited.
  return {
    description,
...
export default async function LocaleLayout({ children, params }: Props) {
  ...
  // (layout.ownsMain → engine renders no <main>; the design MUST render one).
  // Unset → <main className="min-h-[60vh]"> exactly as before (byte-identical).
