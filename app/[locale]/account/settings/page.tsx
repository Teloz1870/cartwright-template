...
export async function generateMetadata() {
  ...
  return { title: t("settings_metaTitle") };
}
...
export default async function AccountSettingsPage() {
  ...
  const backLinkClass = designSurfaces
    ...
    : "text-sm font-bold text-[var(--cw-brand-on-dark)] transition-colors hover:text-[var(--cw-brand-on-dark-hi)]";
    ...
    <main className={mainClass} {...(designSurfaces ? { "data-design-surface": true } : {})}>
      <div className="mx-auto w-full max-w-md">
        <div className="mb-6 flex items-center justify-between">
          ...
          >
            {t("settings_backToAccount")}
          </Link>
        </div>

        <div className={infoCardClass}>
          {t("settings_emailLabel")} <span className="font-bold text-white">{user.email}</span>
          <p className="mt-1 text-xs text-white/40">
            {t("settings_emailHelp")}
          </p>
        </div>

        <section className={`mb-6 ${sectionClass}`}>
          <h2 className="mb-4 text-lg font-black">{t("settings_profileHeading")}</h2>
          <ProfileForm
            initial={{
              name: user.name ?? "",
        ...
        <section className={sectionClass}>
          <h2 className="mb-1 text-lg font-black">{t("settings_passwordHeading")}</h2>
          <p className="mb-4 text-sm text-white/50">
            {user.passwordHash
              ? t("settings_passwordChangeHelp")
              : t("settings_passwordCreateHelp")}
          </p>
