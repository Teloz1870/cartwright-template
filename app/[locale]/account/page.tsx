...
export default async function KontoPage() {
  ...
  const cardClass = designSurfaces
    ...
    : "bg-[#0A0A0A] rounded-3xl shadow-2xl shadow-[var(--cw-brand-on-dark)]/10 border border-white/10 px-8 py-10 flex flex-col gap-6";
      ...
      <div className="w-full max-w-md">
        ...
        <div className={cardClass}>
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* Resolved: the PDP renders its wishlist heart on the merged
                value, so this link has to agree or the feature looks absent. */}
            {(brandSettings?.features ?? brand.features).wishlist && (
              <Link
