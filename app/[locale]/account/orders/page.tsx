...
export default async function OrdrerPage() {
  // Same gap as order/[id]: no locale in scope, so prices fell back to the
  // currency-derived default and rendered Danish on /en.
  ...
  return (
    <main className="min-h-screen bg-sol-cream px-4 py-16">
      <div className="w-full max-w-2xl mx-auto">
        ...
        {orders.length === 0 ? (
          <div className="bg-white dark:bg-sol-sand rounded-3xl shadow-sm border border-sol-ink/10 dark:border-white/10 px-8 py-12 text-center flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col gap-4">
            {orders.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-white dark:bg-sol-sand rounded-3xl shadow-sm border border-sol-ink/10 dark:border-white/10 px-8 py-6 flex flex-col gap-4"
                >
                  ...
                  <div className="flex flex-wrap gap-6 text-sm text-sol-muted">
                    ...
                    <div>
                      ...
                      {formatPriceDkk(order.totalDkk, { locale: locale })}
                    </div>
                  </div>
