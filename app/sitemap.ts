...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const baseRoutes: MetadataRoute.Sitemap = [
    ...
    // `/manifest` and `/changelog` are engine-only routes
    // (scaffold/engine-only.ts): the CLI deletes them from a customer's
    // scaffold, and a sitemap must never advertise a route that is gone
    // (owner CW-24). `Boolean(...)`, never `=== true` (#550/#551/#572).
