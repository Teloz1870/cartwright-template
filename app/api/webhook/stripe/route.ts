...
export async function POST(request: NextRequest) {
  ...
  if (isSubscriptionEvent(event) && !subscriptionsFeatureEnabled()) {
    return NextResponse.json({ received: true, disabled: "subscriptions" });
  }
  ...
  try {
    switch (event.type) {
      ...
      case "invoice.payment_failed":
        if (subscriptionsFeatureEnabled()) {
          await handleSubscriptionInvoice(event.data.object as Stripe.Invoice);
        }
        break;
      default:
...
/**
 * Håndterer customer.subscription.created/updated/deleted
...
async function handleSubscriptionUpdated(subscriptionInput: Stripe.Subscription) {
  await syncStripeSubscription(subscriptionInput);
}
