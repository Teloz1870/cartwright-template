...
const schema = z.object({
  ...
  // WHICH form this came from. Free-form (a custom site names its own forms),
  // but bounded and trimmed — it is written by an unauthenticated endpoint and
  // shown verbatim in /admin/leads.
  source: inquirySourceSchema,
  // WHAT the visitor configured. Schemaless by design: a hegnsberegner sends
  // {meters, model, colour}, a booking form sends something else, and the
  // engine cannot know either. Previously such fields were dropped SILENTLY —
  // zod strips unknown keys — so a custom calculator could POST its whole
  // result set and store nothing but a name and an email.
  //
  // Bounds exist because this is a PUBLIC write: an object (so /admin/leads can
  // render predictable key/value rows), a key cap, and a serialised byte cap.
  // This bounds the SCHEMALESS field specifically — an open-ended object is a
  // far easier thing to abuse than a text field. Note the sibling text columns
  // (message, company, phone…) carry no .max() today, so this is not the whole
  // story for endpoint abuse; bounding them is a separate change.
  ...
  try {
    ...
    const inquiry = await prisma.lead.create({
      data: {
        ...
        // undefined (not null) so Prisma omits the column entirely when the
        // form sent nothing — same reasoning as attachmentUrls below.
        //
        // The cast is the seam between two deliberately separate worlds:
        // sanitizeLeadData lives in lib/inquiry-errors.ts, which the no-DB
        // (site-profile) route also imports, so it must never reference Prisma
        // types. Zod has already proved this is a plain JSON object of bounded
        // size, which is exactly InputJsonObject's contract.
        data: sanitizeLeadData(data.data) as Prisma.InputJsonObject | undefined,
