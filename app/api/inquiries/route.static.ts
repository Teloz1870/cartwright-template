...
const schema = z.object({
  ...
  // Mirrors the db route's source/data. There is no row to write here, so both
  // are folded into the mail body instead — a `site`-profile scaffold that
  // silently dropped a calculator's answers would have exactly the defect the
  // db route just fixed, only harder to notice because nothing is stored.
  source: inquirySourceSchema,
  data: inquiryDataSchema,
});

/** Render `data` as readable mail lines; never JSON-blob-dump at a human. */
function dataLines(data: Record<string, unknown> | undefined): string[] {
  // Through the SHARED sanitizer, not a local filter: this variant used to
  // inline its own one-level key filter, so the moment the db route's
  // sanitizer learned to walk nested keys, the two would have disagreed about
  // what "safe" means — the exact drift lib/inquiry-errors.ts exists to prevent.
  const safe = sanitizeLeadData(data);
  if (!safe) return [];
  ...
  try {
    ...
    const delivered = await sendContactMail({
      ...
      text: [
        ...
        data.source ? `Form: ${data.source}` : null,
        `Budget: ${data.budget}`,
        "",
        data.message ?? "",
        ...dataLines(data.data),
    ...
    if (!delivered) {
      return NextResponse.json(
        { ok: false, ...inquiryFailure("send_failed") },
        { status: 500 },
      );
    }
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { ok: false, ...inquiryFailure(inquiryErrorCode(error.issues[0].message)) },
        { status: 400 },
      );
    }
    return NextResponse.json(
