...
/**
 * `contactAttachments` is declared `tier: "runtime"` in the feature manifest,
 * so /admin/features writes a DB override for it — and the contact page reads
 * that override through `getBrand()`. This route read the STATIC config, so an
 * admin who turned attachments on got a file input on the form and a 404 from
 * the endpoint behind it. Read the same resolved value the form does.
 */
async function attachmentsEnabled(): Promise<boolean> {
  const features = (await getFeatures()) as { contactAttachments?: boolean };
  return Boolean(features.contactAttachments);
}

export async function POST(request: NextRequest) {
  if (!(await attachmentsEnabled())) {
  ...
  if (!IMAGE_MIME.has(file.type)) {
    return NextResponse.json(
      { code: "file_wrong_type", error: "Only JPEG, PNG or WebP images can be attached." },
      { status: 400 },
    );
  }
  if (file.size > MAX_BYTES) {
    return NextResponse.json(
      {
        code: "file_too_large",
        error: `That image is too large. Maximum ${Math.round(MAX_BYTES / 1_000_000)} MB.`,
      },
      { status: 400 },
    );
  }
  ...
  if (!hasValidMagicBytes(header, file.type)) {
    return NextResponse.json(
      { code: "file_content_mismatch", error: "The file contents do not match its type." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    // Server-side only: the old response named BLOB_READ_WRITE_TOKEN to
    // whoever posted the form, which tells a stranger how the shop is
    // configured. The operator needs that detail; the visitor does not.
    console.error("[contact-upload] Vercel Blob put failed:", err);
    return NextResponse.json(
      { code: "upload_failed", error: "The upload failed. Please try again." },
      { status: 500 },
    );
  }
}
