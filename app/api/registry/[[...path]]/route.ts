...
/**
 * The preflight reads the SAME runtime gate as GET, and answers GET's own bare
 * 404 when it is shut. A hand-written `OPTIONS` that replies before the gate is
 * a stronger tell than no handler at all: `204` + `Access-Control-Allow-Methods`
 * announces a live registry on a shop where every real verb says `404`. Gate
 * OPEN keeps today's CORS-open preflight — this surface exists to be read by
