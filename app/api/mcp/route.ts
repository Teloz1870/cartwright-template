  ...
  server.registerResource("public-trust", `${resolvedBase}/${resolvedBrand.defaultLocale}/about`, {
    ...
    const trustPage = async (slug: "about" | "privacy") => {
      const page = slug === "about"
        ? await findFirstPublishedPageBySlugs(publicPageSourceSlugs("about"))
...
async function unknownToolCallRejection(
  request: NextRequest,
  ...
  try {
    ...
    // Notifications (no id) never get a response body — the transport answers
    // 202. And batches (array bodies parse with method undefined above) stay
    // the SDK's problem: a batch answer must be a batch, and the anonymous
    // path already rejects batches wholesale before this point.
    if (body.id === undefined || body.id === null) return null;
    const name = body.params?.name;
    if (typeof name !== "string") return null;
    if (listTools().some((tool) => tool.name === name)) return null;
    return Response.json({
      ...
      error: {
        code: -32602,
        message: `Tool ${name} not found. Call tools/list to see the available tools.`,
      },
    });
  } catch {
    // Malformed bodies stay the transport's problem — this shim only answers
    // the one well-formed case it exists for.
    return null;
  }
}
...
async function guard(request: NextRequest): Promise<Response | null> {
  ...
  return await mcpOriginRejection(request.headers.get("origin"), request);
}
...
/**
 * Refusing the batch is right; answering with a problem document was not.
 *
 * A caller that sends JSON-RPC expects JSON-RPC back. This endpoint replied
 * `application/problem+json`, which no conformant client can parse as a
 * protocol response — it reads as "the server broke", not "that request was
 * refused". Measured 2026-08-29 against demo.cartwright.app: a batched
 * handshake returns `400 {"type":"https://cartwright.app/problems/…"}`.
 *
 * `-32600 Invalid Request` is the correct code: MCP removed JSON-RPC batching
 * in the 2025-06-18 revision, so "this endpoint does not take batches" is a
 * statement about the request, not an internal failure. The machine-readable
 * code and the resolution move into `error.data`, so anything that consumed
 * them still can.
 *
 * The guard itself is unchanged: one anonymous HTTP request must not fan out
 * into N tool calls that each escape the per-request rate limit.
 */
  ...
  return new Response(
    JSON.stringify({
      ...
      error: {
        code: -32600,
        message:
          "Anonymous MCP requests must contain exactly one JSON-RPC message.",
        data: {
...
async function serve(request: NextRequest) {
  ...
  {
    // Unknown tool → JSON-RPC protocol error (-32602), not a tool RESULT with
    // isError. The SDK's call-tool handler wraps every throw — the not-found
    // throw included — into an execution result, but "this tool does not
    // exist" is a protocol-level condition: clients (and scanners) branch on
    // `error.code`, and an isError text blob gives them nothing structured.
    // Names are checked against the FULL registry, so a real-but-unauthorized
    // tool still flows through to the scope layer's structured error.
    const rejection = await unknownToolCallRejection(request);
    if (rejection) {
      if (authRate) return applyAuthAttemptRateLimitHeaders(rejection, authRate);
      return publicRate ? applyRateLimitHeaders(rejection, publicRate) : rejection;
    }
  }
  ...
  const response = await transport.handleRequest(
    withLenientMcpAccept(await normalizeLegacyArguments(request)),
  );
...
export async function GET(request: NextRequest) {
  ...
  if (!hasAuth) {
    // Streamable-HTTP conformance: a GET that explicitly asks for
    // `text/event-stream` is a client opening the server-initiated SSE
    // stream. The spec's contract is binary — serve the stream, or answer
    // 405. This stateless endpoint offers no server-push stream, and the
    // human-friendly JSON descriptor below would read as a broken handshake
    // to such a client, so it is reserved for browsers (whose Accept lists
    // text/html) and plain fetches. Parsing is case-insensitive and honours
    // quality values: `text/event-stream;q=0` explicitly DECLINES the stream.
    const acceptEntries = (request.headers.get("accept") ?? "")
      .toLowerCase()
      .split(",")
      .map((entry) => {
    ...
    const accepts = (type: string) =>
      acceptEntries.some((e) => e.type === type && e.q > 0);
    if (accepts("text/event-stream") && !accepts("text/html")) {
    ...
    const response = Response.json(
      {
        ...
        // Engine-only routes (scaffold/engine-only.ts) — omitted rather than
        // advertised to an agent in a scaffold where the CLI removed them.
        // `Boolean(...)`, never `=== true` (#550/#551/#572).
        ...(Boolean(profileCapabilities.engineMarketing)
          ? { manifest: "/manifest", liveChangelog: "/changelog" }
          : {}),
      },
      { status: 200 },
    );
