...
export async function GET(request: NextRequest) {
  ...
  // No-op når altTextAi er off, så shops uden feature ikke brænder Gemini-budget
  // på uploads ("don't ship default-on"). Compile-time flag → static brand.features
  // (samme mønster som abandoned-cart-cron'en). Dependency mediaLibrary håndteres
  // i getFeatureView's enable-gate; her er flag-checket nok.
  if (!(brand.features as { altTextAi?: boolean }).altTextAi) {
    return NextResponse.json({ ok: true, reason: "altTextAi-feature-disabled" });
  }
