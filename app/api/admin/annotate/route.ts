...
export async function POST(request: NextRequest) {
  ...
  const rl = adminChatRateLimiter.check(guard.user.id);
  ...
  // ── CURRENT (direct-edit prefill) ──────────────────────────────────────────
  // Strukturerede felter (pris) redigeres direkte — overlayet henter den
  // autoritative nuværende værdi til input-feltet (DOM-teksten kan være
  // currency-konverteret/formateret og er ikke til at stole på).
  if (b.phase === "current") {
    if (!isDirectTarget(target)) {
      return Response.json(
        { error: "Only structured fields support direct editing" },
        { status: 400 },
      );
    }
    ...
    // product.price: kanonisk øre → major units til input-feltet ("249.50").
    // service.price: freeform passthrough.
    if (target.kind === "product" && target.field === "price") {
      const oere = Number(current);
      const rest = oere % 100;
      const value =
        `${Math.floor(oere / 100)}` + (rest ? `.${String(rest).padStart(2, "0")}` : "");
      return Response.json({ value, currency: brand.policies.currency });
    }
    return Response.json({ value: current, currency: null });
  }

  // ── DIRECT (struktureret felt → valider + mint token, ingen LLM) ──────────
  // Samme sikkerhedssti som propose: server-udstedt confirmation-token binder
  // de præcise tool-args, og apply-fasen consumer tokenet før invokeTool.
  // Forskellen er kun at "after" kommer fra admin'ens eget input i stedet for
  // modellen — pris er data, ikke copy.
  if (b.phase === "direct") {
    if (!isDirectTarget(target)) {
      return Response.json(
        { error: "This field is copy — use the AI note flow" },
        { status: 400 },
      );
    }
    const raw = typeof b.value === "string" ? b.value : "";
    const parsed = parseDirectInput(target, raw);
    if (!parsed.ok) {
    ...
    if (!valid.ok) {
      return Response.json({ error: valid.error }, { status: 422 });
    }
  ...
  // ── PROPOSE ────────────────────────────────────────────────────────────────
  if (b.phase === "propose") {
    // Strukturerede felter må ALDRIG AI-omskrives — de har deres egen
    // deterministiske "direct"-fase ovenfor.
    if (isDirectTarget(target)) {
      return Response.json(
        { error: "This field is edited directly, not via AI" },
        { status: 400 },
      );
    }
    ...
    return withAuditContext(
      ...
      async () => {
        const result = await invokeTool(
          expectedTool,
          args,
          { actor: `annotation:${guard.user.id}`, requestId: randomUUID(), ip, userAgent },
          ADMIN_CHAT_SCOPES,
        );
        if (!result.ok) {
