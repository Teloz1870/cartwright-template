...
export async function POST(req: Request) {
  ...
  try {
    ...
    // v0.9.4: dry-run → parse + normalisér + returnér en preview af det parsede
    // design UDEN at skrive filer. Driver "bekræft før import"-UX'en i
    // DesignUploader (vis navn/mode/palette/#sektioner + advar ved mode-mismatch).
    ...
    // v0.9.4: design-import skriver filer til projekt-træet → virker kun hvor
    // filsystemet er skrivbart (lokal `npm run dev` eller `npx cartwright design
    // import`). På en deployed shop (Vercel: read-only/ephemeral FS) kaster
    // fs.writeFile EROFS/EACCES. Map det til en tydelig besked i stedet for en
    // rå 500 så kunden forstår at import er en lokal/scaffold-time handling.
    const msg = (e as NodeJS.ErrnoException)?.code;
    if (msg === "EROFS" || msg === "EACCES" || msg === "EPERM") {
      return NextResponse.json(
        {
          ok: false,
          error:
            "Design-import skriver filer til projektet og kan derfor kun køres " +
            "lokalt (npm run dev) eller via `npx cartwright design import`. " +
            "På en hostet shop: importér designet lokalt, commit ændringerne, og " +
            "deploy. (Filsystemet er skrivebeskyttet i produktion.)",
        },
        { status: 400 },
      );
    }
