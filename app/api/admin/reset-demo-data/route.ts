...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const p of template.products) {
      ...
      // translations, attributes are nullable/optional and we skip them.
      await client.execute({
        sql: `INSERT INTO Product
              (id, name, slug, description, priceDkk, images, stock,
               frameColor, lensColor, brand, featured, categoryId)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          id,
          p.name,
    ...
    // Matches the seed in prisma/seed.ts:106-115 so demo visitors can log
    // in to /admin with the documented credentials (admin@<domain> /
    // admin1234). Pure-SQL UPSERT via INSERT OR REPLACE for SQLite.
    ...
    try {
      ...
      log.push(`  ✓ admin user "${adminEmail}" / password "${DEMO_ADMIN_PASSWORD}"`);
    } catch (err) {
