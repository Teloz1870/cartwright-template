...
/**
 * Operatør-chat-endpoint. Lever bag requireAdminApi-guard. Sender SSE-
...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    return Response.json(
      {
        error:
          "Operator AI is not configured. Go to /admin/integrations to add an Anthropic API key.",
      },
      { status: 503 },
    );
  }

  // Narrowing across the hoisted executeAdminTool closure: capture the admin
  // user id in a plain const (TS can't carry the instanceof-narrowing of
  // `guard` into the nested function).
  const adminUserId = guard.user.id;
  const actor = `operator-chat:${adminUserId}` as const;
  ...
  async function executeAdminTool(
    ...
    if (!isAdminTool(name)) {
      await prisma.auditLog.create({
        data: {
          ...
          ok: false,
          errorMsg: "Operator chat tried to call a disallowed tool",
          requestId: randomUUID(),
          ip,
          userAgent,
        },
      });
      return { error: "That tool is not available to operator chat." };
    }
    ...
    if (suggestMode && CONFIRM_REQUIRED.has(name as never)) {
      return {
        error:
          "Suggest mode is active - write tools are disabled. Turn off suggest mode to execute the action.",
      };
    }
  ...
  // under denne session får provider+model stamps på audit-rows.
...
function buildPlanPreview(name: string, args: unknown): string {
  ...
  switch (name) {
    ...
    case "products.update":
      return `Update product '${a.slug}' with changes: ${JSON.stringify(a.patch ?? {})}`;
    case "orders.update_status":
      return `Set order ${a.orderId} to status '${a.status}'`;
    ...
    case "docs.import":
      return `Importér Google Doc som ${a.target === "post" ? "blogindlæg" : "side"}`;
    case "settings.update_shipping":
      return `Set shipping: ${formatPrice(a.shippingFeeOere)} | free above ${formatPrice(a.freeShippingThresholdOere)}`;
