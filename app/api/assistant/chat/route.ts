...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    return Response.json(
      {
        error:
          "The AI assistant is not configured yet. Go to /admin/integrations to add an Anthropic API key.",
      },
      { status: 503 },
    );
  }
  ...
  async function executeCustomerTool(
    ...
    if (toolCallCount >= MAX_TOOL_CALLS_PER_SESSION) {
      return {
        error:
          "You have used a lot of tool calls in this session. Refresh the page if you want to start over.",
      };
    }
    toolCallCount++;
    ...
    if (!isCustomerTool(name)) {
      await prisma.auditLog.create({
        data: {
          ...
          ok: false,
          errorMsg: "Customer chat tried to call a disallowed tool",
          requestId: randomUUID(),
          ip,
          userAgent,
        },
      });
      return { error: "That tool is not available to you." };
    }
    ...
    if (CUSTOMER_CONFIRM_REQUIRED.has(name)) {
      ...
      if (!verify.ok) {
        return { error: `Confirmation rejected: ${verify.reason}` };
      }
  ...
  const brandConfig = await import("@/lib/brand").then((m) => m.getBrand());
  const actualSystemPrompt = brandConfig.ecommerceEnabled
    ? SYSTEM_PROMPT
    : `You are an AI Consultant for ${brandConfig.storeName}, a digital agency.
Your goal is to qualify leads, understand their project needs, and guide them towards booking a meeting or using the Project Builder on the site.
Do NOT act as a shop assistant or talk about shopping carts.
Instead, ask them about their budget, timeline, and whether they need a Webshop, a Website, or AI Integrations.
Be professional, concise, and helpful.`;

  // Local-AI plan: wrap streamText i withAuditContext så customer-chat tool-calls
  ...
  const response = await withAuditContext(
    ...
    async () => {
      const result = streamText({
        ...
        // Hul E: optag faktisk token-forbrug (aggregeret over alle tool-steps).
        // AI SDK v7: onFinish→onEnd, totalUsage→usage (begge var deprecated-men-
        // virkende aliaser i v7; migreret til den stabile flade).
        async onEnd({ usage }) {
          await recordAiUsage({
            provider: resolved.provider,
            model: resolved.model,
    ...
    case "orders.create": {
      ...
      try {
        ...
        if (cart && cart.items.length > 0) {
          ...
          return `${itemsCount} ${itemsCount === 1 ? "item" : "items"} to ${a.shippingName}, ${a.shippingAddress}, ${a.shippingZip} ${a.shippingCity}`;
        }
      } catch {
        // Pre-compute fejler — fall back til simpel preview
      }
      return `Create order for ${a.shippingName} (${a.email}) - ${a.shippingAddress}, ${a.shippingZip} ${a.shippingCity}`;
    }
    default:
      return `Run ${name}`;
  }
