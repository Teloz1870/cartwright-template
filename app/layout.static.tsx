/**
 * app/layout.static.tsx — the root layout's seam twin for profiles WITHOUT the
 * `google-fonts` module (`--profile site`).
 *
 * Identical to app/layout.tsx except that no font is fetched from Google at
 * build time: the on-disk layout loads Geist + Geist Mono through
 * `next/font/google`, and together with the nine packs that do the same at
 * module scope that made a first `next dev` on a slow network answer
 * `GET /en 500` — with the failure cached by Turbopack (A5, measured). Here the
 * two CSS variables resolve to the system stack instead, so nothing on the
 * boot path needs the network.
 *
 * NOTHING imports it in the shipped engine, so the engine's own output stays
 * byte-identical until a materializer copies this file over app/layout.tsx.
 * tests/unit/site-font-safe.test.ts pins three things: the twin differs from
 * the real layout ONLY in the font block, no core-claimed pack uses a Google
 * font, and the site profile's included file set contains no
...
export default async function RootLayout({
  ...
  return (
    ...
    >
      <head>
        {/* The Google-font layout defines --font-geist-sans/mono via next/font;
            this twin resolves the same variables to the system stack, so a
            pack that names them (the showcase pack) still gets a real font
            stack — the system's, not Geist — and nothing on the boot path
            fetches from the network. */}
        <style
          id="site-font-stack"
          dangerouslySetInnerHTML={{
            __html:
              ":root{--font-geist-sans:ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;--font-geist-mono:ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',monospace}",
          }}
        />
