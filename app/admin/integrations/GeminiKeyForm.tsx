...
export default function GeminiKeyForm({ initialStatus }: Props) {
  ...
  function handleClear() {
    if (
      !confirm(
        "Remove Google Gemini API key? AI-generated SEO content, theme palettes, and category descriptions will stop working until a new key is set.",
      )
    ) {
      return;
    }
      ...
      <div className="flex flex-wrap items-center gap-2">
        {savedPreview ? (
          ...
          <span className="inline-flex items-center gap-2 rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-800">
            ...
            Not configured — the AI SEO/theme generator won&apos;t work
          </span>
        )}
      </div>
      ...
      <form action={handleSubmit} className="space-y-3">
        <label className="block">
          ...
          <span className="mt-1 block text-xs text-sol-muted">
            Used for AI-generated SEO content, category descriptions, and theme palettes. Get a free key
            at{" "}
            <a
            ...
            . The key is only sent to the server and never comes back in plain text
            tekst.
          </span>
        </label>
