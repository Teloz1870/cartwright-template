...
import { setAnthropicKeyAction, clearAnthropicKeyAction } from "./actions";
...
export default function AnthropicKeyForm({ isSet, preview, envFallback }: Props) {
  const [pending, startTransition] = useTransition();
  const [input, setInput] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [savedPreview, setSavedPreview] = useState<string | null>(preview);

  function handleSubmit(formData: FormData) {
    setError(null);
    startTransition(async () => {
      const r = await setAnthropicKeyAction(formData);
      if (r.ok) {
  ...
  function handleClear() {
    if (!confirm("Fjern Anthropic API-key? AI-stylisten stopper med at virke indtil en ny key er sat.")) {
      return;
    }
    startTransition(async () => {
      ...
      <div className="flex flex-wrap items-center gap-2">
        {savedPreview ? (
          ...
          <span className="inline-flex items-center gap-2 rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-800">
            ...
            Ikke konfigureret — AI-stylisten virker ikke
          </span>
        )}
      </div>
      ...
      <form action={handleSubmit} className="space-y-3">
        <label className="block">
          ...
          <span className="mt-1 block text-xs text-sol-muted">
            ...
            . Keyen sendes kun til serveren og kommer aldrig tilbage i klar tekst.
          </span>
        </label>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {pending ? "Saving…" : isSet ? "Replace key" : "Save key"}
          </button>
          {savedPreview && (
            <button
              type="button"
