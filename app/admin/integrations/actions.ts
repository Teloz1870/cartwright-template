...
/**
 * Fetch settings + masked key. The plaintext key is NEVER returned to the
 * frontend — only a preview ("sk-ant-...xx") so an admin can recognise
 * which one is set without exposing the full value in the DOM.
 */
export async function getIntegrationStatus() {
  ...
  // Decrypt so we can build a masked preview (review finding #9 — the field is
  // encrypted in the DB; we still show prefix/suffix for the admin's visual match)
...
export async function setAnthropicKeyAction(
  ...
  // Encrypt before storage (AES-256-GCM with a KEK from AUTH_SECRET).
  // A DB leak alone does not yield a working key — the attacker also needs
  // the server's AUTH_SECRET (review finding #9).
...
export async function setGeminiKeyAction(
  ...
  // Google AIza keys are typically 39 characters but only contractually
  // guaranteed to be >= 35. We accept 35-45 to avoid rejecting legitimate
  // keys if Google extends the format.
...
export async function setResendKeyAction(
  ...
  if (!raw.startsWith("re_")) {
    return {
      ok: false,
      error: "Wrong format — Resend keys start with 're_'",
    };
  }
...
export async function getResendStatus() {
  ...
  const dbKey = row?.resendApiKey ? decryptSecret(row.resendApiKey) : null;
  // The runtime reader (lib/mailer/resend.ts) falls back to the env var, so a
  // status that only looked at the DB row reported "not set" for a shop whose
  // mail demonstrably works — the one admin surface an operator is told to
  // trust, under-reporting production. Same `envFallback` shape as
  // getIntegrationStatus/getGoogleOAuthStatus.
  const envKey = process.env.RESEND_API_KEY?.trim() || null;
  return {
    isSet: !!(dbKey ?? envKey),
    // preview stays DB-only, exactly like getIntegrationStatus: a masked env key
    // would read as "stored here" and invite an operator to clear it.
    preview: dbKey ? maskGenericKey(dbKey) : null,
    envFallback: !dbKey && !!envKey,
  };
}
...
/**
 * Fetch the current AI settings for UI rendering. Returns a serialisable
 * shape without secrets (apiKey is exposed only as a boolean configured flag).
 */
...
/**
 * Save a new AI provider config. Validates every field; empty strings become
 * NULL in the DB so getAiSettings() falls back to env/defaults.
 */
export async function setAiSettingsAction(
  ...
  if (localAiModel && (localAiModel.length > 80 || !/^[a-z0-9._:-]+$/i.test(localAiModel))) {
    return { ok: false, error: "Invalid local model id (only a-z, 0-9, . _ - :)" };
  }
  if (localAiFallbackMode && !isValidFallbackMode(localAiFallbackMode)) {
    return { ok: false, error: "Invalid fallback mode" };
  }
...
/**
 * List the models available on the Ollama endpoint. Doubles as an online
 * check of the endpoint. Returns model names + capability tier so the UI
 * can show which models have which tool cap (read-only/low-risk/all).
 */
...
> {
  ...
  // Ollama's tags endpoint lives at the root (not /v1). If the admin typed
  // /v1 into the URL, we normalise it away.
    ...
    return {
      ...
      error:
        err instanceof Error && err.name === "TimeoutError"
          ? `Timeout (5s) — is Ollama running at ${baseUrl}?`
...
/**
 * Send a minimal test prompt to the configured provider and return the
 * response + latency. Used by the "Test connection" button in LocalAiForm.
 *
 * intent="chat" so it respects aiProvider; intent="vibe" is always forced
 * to Anthropic (used when we want to prove vibe routing works).
 */
...
> {
  ...
  try {
    ...
    return {
      ok: false,
      error: err instanceof Error ? err.message : "Unknown error during the test call",
    };
  }
}

/**
 * Delete a local Ollama model. Frees disk space + RAM (if the model was
 * loaded). Mirrors ollama-pull's allow-list so an admin can only delete models
 * we recognise — protects against command injection via DB state.
 *
 * Audit-log row with actor=user:<id>, tool="ollama.delete", provider="local",
 * model=<name>. A trail for "who deleted which models when".
 */
export async function deleteOllamaModelAction(
  ...
  if (!ALLOWED_DELETE_MODELS.includes(modelName)) {
    return {
      ok: false,
      error: `Model '${modelName}' is not in the allow-list. Delete it from a terminal with 'ollama rm ${modelName}' if you want to.`,
    };
  }
  ...
  try {
    ...
    await prisma.auditLog
      ...
      .catch(() => {
        // An audit failure must not block
      });
    ...
    return {
      ...
      error:
        err instanceof Error && err.name === "TimeoutError"
          ? "Timeout (30s) — Ollama took too long to delete"
...
/**
 * Validate that a voice session can be minted with the current settings.
 * Returns latency + diagnostics without actually opening a WS (we just mint a
 * token and close it immediately). Catches: invalid API key, empty allowedTools,
 * cap errors.
 */
...
> {
  ...
  const s = await getVoiceShopSettings();
  if (!s.enabled) {
    return { ok: false, error: "Voice shop is not enabled." };
  }
  if (!s.apiKey) {
    return {
      ok: false,
...
/**
 * Masking: show prefix + suffix, hide the middle. Follows the same pattern as
 * the GitHub/Stripe dashboards so an admin has a visual match clue when
 * troubleshooting but cannot copy the key out.
 */
...
function maskGenericKey(key: string): string {
  if (key.length < 12) return "•".repeat(key.length);
  // Detect prefix length dynamically: everything before the first underscore + the first character after
