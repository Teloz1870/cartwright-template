...
export async function getIntegrationStatus() {
  ...
  const row = await prisma.integrationSettings.findUnique({
    where: { id: 1 },
    select: { anthropicApiKey: true, googleGeminiApiKey: true, videoGenerationApiKey: true, videoGenProvider: true, updatedAt: true },
  });
...
> {
  ...
  if (!clientId || !clientSecret) {
    return {
      ok: false,
      error: "Client ID og client secret skal begge udfyldes.",
    };
  }
  if (!clientId.includes(".apps.googleusercontent.com")) {
    return {
      ok: false,
      error: "Client ID ligner ikke en Google OAuth client ID.",
    };
  }
  if (clientSecret.length < 10) {
    return {
      ok: false,
      error: "Client secret er for kort - tjek at hele værdien er kopieret.",
    };
  }
