...
export default function GoogleOAuthForm({ initial }: Props) {
  ...
  function handleClear() {
    if (!confirm("Remove the Google OAuth client ID and secret?")) return;
    startTransition(async () => {
  ...
  function handleDisconnect() {
    if (!confirm("Disconnect Google and remove the stored tokens?")) return;
    setError(null);
    startTransition(async () => {
    ...
    <div className="space-y-4">
      {hasCredentials ? (
        <div className="rounded-xl border-2 border-green-500 bg-green-50 px-4 py-3 text-sm font-bold text-green-900">
          Google OAuth client is set — ready to connect the Workspace API.
        </div>
      ) : (
        <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <strong>T0 connector:</strong> save the OAuth client ID + secret first.
          Then you can connect Google with offline access for the server-side
          Sheets/Drive/Docs modules.
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-sol-muted">
              Server-side Google API client. Not customer login.
            </p>
          </div>
          {hasCredentials ? (
        ...
        <div className="mt-3 space-y-1">
          ...
          {initial.clientId.envFallback && (
            <p className="text-xs font-bold text-sol-muted">
              Client ID is read from GOOGLE_OAUTH_CLIENT_ID.
            </p>
          )}
          {initial.clientSecret.envFallback && (
            <p className="text-xs font-bold text-sol-muted">
              Secret is read from GOOGLE_OAUTH_CLIENT_SECRET.
            </p>
          )}
        </div>
      ...
      <div className="rounded-xl border border-sol-ink/10 bg-sol-sand p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h4 className="text-sm font-black text-sol-ink">Google connection</h4>
            <p className="text-xs text-sol-muted">
              Stores refresh/access tokens encrypted in the singleton connection.
            </p>
          </div>
          {connected ? (
            <span className="inline-flex items-center gap-2 rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800">
        ...
        <div className="mt-3 space-y-1 text-xs text-sol-muted">
          ...
          {lastError && (
            <p className="font-bold text-red-700">Last error: {lastError}</p>
          )}
        </div>
