...
/**
 * v0.9.4 — design.md upload with a "confirm before import" flow.
 *
 * Flow: pick/drop a file → POST dryRun=true → show a parsed preview (name, mode,
 * palette swatches, sections, a warning on mode mismatch) → the customer confirms
 * → POST the real import. This avoids the earlier "drop = instant, opaque
 * import" and lets the customer see WHAT they are importing + that the palette
 * is actually captured correctly, before anything is written.
 */
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded-lg border border-sol-ink/15 bg-sol-sand px-3 py-1.5 text-sm font-semibold text-sol-ink focus:border-sol-accent focus:outline-none focus:ring-2 focus:ring-sol-accent/25"
        >
        ...
        <button
          ...
          className="ml-auto rounded-lg border border-sol-ink/15 px-3 py-1.5 text-xs font-bold text-sol-ink transition-colors hover:border-sol-ink/40"
        >
          ↓ Download eksempel design.md
        </button>
      </div>

      {/* Dropzone — hidden while a preview is shown */}
      {!preview ? (
        <div
          onDragOver={(e) => {
            e.preventDefault();
          ...
          className={`flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed bg-white p-12 text-center transition-colors ${
            isDragging
              ? "border-sol-accent bg-sol-accent/5"
              : "border-sol-ink/20 hover:border-sol-ink/40"
        ...
        >
          ...
          <p className="text-sm font-bold text-sol-ink">
            {pending ? "Analyzing..." : "Drag-drop design.md or click to choose"}
          </p>
          <p className="text-xs text-sol-muted">
            .md (Cartwright / Stitch) or .tsx (Claude Design / v0)
          </p>
        </div>
      ) : null}

      {/* Preview card: confirm before import */}
      {preview ? (
        <div className="flex flex-col gap-4 rounded-2xl border-2 border-sol-accent/40 bg-white p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-sol-accent">
                Ready to import · adapter: {preview.detectedAdapter}
              </p>
              <h3 className="mt-1 text-xl font-black text-sol-ink">
                {preview.name}
              </h3>
            ...
            <code className="shrink-0 rounded bg-sol-ink/5 px-2 py-1 font-mono text-xs text-sol-ink">
              {preview.slug}
            </code>
          </div>
          ...
          {/* Palette swatches */}
          <div>
            <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-sol-muted">
              Palette
            </p>
            <div className="flex flex-wrap gap-2">
              {(
                ...
                <div key={name} className="flex flex-col items-center gap-1">
                  <span
                    className="h-9 w-9 rounded-lg border border-sol-ink/15"
                    style={{ backgroundColor: hex }}
                    title={`${name}: ${hex}`}
                  />
                  <span className="text-[9px] font-semibold text-sol-muted">
                    {name}
                  </span>
                </div>
              ))}
          ...
          {/* Mode + sektioner */}
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wider text-sol-muted">
            <span>mode: {preview.mode}</span>
            <span className="text-sol-ink/30">·</span>
            <span>
          ...
          {modeMismatch ? (
            <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900">
              ⚠ This design is <strong>{preview.mode}</strong>-mode, but your shop
              is <strong>{shopMode}</strong>-mode. It will import fine, but parts of
              the homepage layout may suit a {preview.mode}-shop better.
            </div>
          ) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {pending ? "Importing..." : "✓ Import this design"}
            </button>
            <button
              type="button"
              onClick={reset}
              disabled={pending}
              className="rounded-xl border border-sol-ink/15 px-5 py-2.5 text-sm font-bold text-sol-ink transition-colors hover:border-sol-ink/40 disabled:opacity-50"
            >
              Annullér
            </button>
          </div>
      ...
      {success ? (
        <div className="rounded-lg border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">
          ...
          the list above — click to activate it, and the whole shop adopts
          designets palette.
        </div>
      ) : null}
    </div>
