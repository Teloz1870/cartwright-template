...
import { INDUSTRY_TEMPLATE_PICKER_OPTIONS } from "@/industry-templates";
...
type Props = {
  ...
  /** Seed the business-model picker from brand.config (website ⇒ no cart). */
  initialEcommerceEnabled?: boolean;
  /** Seed the industry-template picker from brand.config. */
  initialIndustryTemplate?: string;
  /** Is an AI provider configured? Gates the Magic Init (AI bootstrap) banner. */
  aiConfigured?: boolean;
  /**
   * Non-null when brand.config owns identity — the reason, in one sentence.
   * The wizard then shows the two identity fields as locked rather than asking
   * a question whose answer it would drop on the way to the database.
   */
  identityLockNotice?: string | null;
  /** Resolved `cartwrightPlus` — see the destructuring comment below. */
 ...
 *
 * State machine in the client (no URL routing per step) — the admin can navigate
 * back and forth without losing form state. Each step saves via a server action
 * when you press "Next"; the admin can also "Skip".
 *
 * The theme step is guidance-only: we cannot switch themes/<slug>.css at runtime
 * (it is build-time resolved), so we show the palette + instructions. An actual
 * theme change requires a code deploy (copy the theme file in a new fork).
 */
export default function SetupWizard({
  ...
  initialEcommerceEnabled = true,
  initialIndustryTemplate = "eyewear",
  aiConfigured = false,
  identityLockNotice = null,
  // Resolved by the server parent. A client component cannot call
  // getFeatures(), and reading the compile-time value here meant the design
  // picker kept marking premium packs "⭐ Pro" after the owner had turned Plus
  // on in /admin/features — where /admin/seo-performance had already unlocked.
  cartwrightPlus = false,
}: Props) {
  const identityLocked = identityLockNotice !== null;
  const router = useRouter();
  const [step, setStep] = useState<StepId>("brand");
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  // Brand step state (ULTRAPLAN-lite UL5: now with tagline/domain/email/industry)
  ...
  // P1.4: derive industries dynamically from INDUSTRY_TEMPLATE_OPTIONS — drop the
  // literal type so new industries appear automatically without a code edit
  const [industryTemplate, setIndustryTemplate] = useState<string>(initialIndustryTemplate);
  const [vibeTemplate, setVibeTemplate] = useState<"ecommerce" | "saas" | "minimalist">(
    initialEcommerceEnabled ? "ecommerce" : "saas",
  );
  ...
  async function probeOllama() {
    ...
    try {
      ...
      // Auto-select the first model if the admin has not typed anything
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {error && (
        <div
          role="alert"
          className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-bold text-red-700"
        >
          {error}
        </div>
      )}
      ...
      <div className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-6 shadow-sm">
        {step === "brand" && (
          <div className="space-y-4">
            {/* Software 3.0: Magic Init Banner — only when an AI provider is
                configured. Magic Init calls the AI to bootstrap the store, so on
                a fresh no-AI scaffold it would just fail; the manual builder
                below is the honest path until AI is set up in /admin/integrations. */}
            {aiConfigured && (
              <>
            ...
            <div>
              ...
              {identityLocked && (
                <p className="text-xs text-sol-muted mb-4">🔒 {identityLockNotice}</p>
              )}
            ...
            <div>
              ...
              <input
                ...
                disabled={identityLocked}
                aria-describedby={identityLocked ? "wizard-storeName-lock" : undefined}
                className={`${inputClass} disabled:opacity-60 disabled:cursor-not-allowed`}
                placeholder="For example Direct Fence Panels"
                required
              />
              {identityLocked && (
                <p id="wizard-storeName-lock" className="mt-1 text-xs text-sol-muted">
                  🔒 {identityLockNotice}
                </p>
              )}
            </div>
            ...
            {/* ULTRAPLAN-lite UL5: extended brand fields — all optional.
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  className={inputClass}
                  placeholder="you@cartwright.app"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className={inputClass}
                >
                  {/* P1.4: dynamic from INDUSTRY_TEMPLATE_OPTIONS — new industries
                      registered in industry-templates/index.ts appear here without a code edit.
                      v0.7.0: industry controls ONLY seed data (categories/products/pages).
                      The visual design is chosen separately in the Design dropdown below. */}
                  {INDUSTRY_TEMPLATE_PICKER_OPTIONS.map(({ slug, label }) => (
              ...
              <div>
                ...
                <select
                  ...
                  className={inputClass}
                >
                  {/* v0.7.0: design packs from designs/index.ts. Filter by
                      ecommerceEnabled so we do not show webshop designs for
                      website-mode shops (and vice versa). The "Auto" option lets
            ...
            <div className="rounded-lg border border-sol-accent/30 bg-sol-accent/5 p-3 mt-6">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={themeGenerating || brandDescription.length < 5 || !aiConfigured}
                  title={!aiConfigured ? "Configure an AI provider in the AI step to use AI theming" : undefined}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-sol-muted">
                ...
                theme generation, voice-shop. Get one at{" "}
                <a
        ...
        {step === "domain" && (
          <div className="space-y-4">
            ...
            {/* 2. Email sending — Resend key + sender name. Without a key, order
                confirmations / magic links / password resets go to
                .mail-previews/ (no real delivery). emailFrom/emailFromName are
                read at runtime by lib/brand.ts getBrand(), so this is not
                dead data. Reuses setResendKeyAction via saveDomainStep. */}
