...
const aiSchema = z.object({
  ...
  aiProvider: z.enum(["anthropic", "local"]).default("anthropic"),
  localAiEndpoint: z.string().trim().url().optional().or(z.literal("")),
...
export async function saveAiStep(formData: FormData): Promise<ActionResult> {
  ...
  const parsed = aiSchema.safeParse({
    ...
    aiProvider: formData.get("aiProvider") ?? "anthropic",
  ...
  const data = parsed.data;
  const anthropic = data.anthropicApiKey?.trim() || null;
  const gemini = data.googleGeminiApiKey?.trim() || null;
  ...
  await prisma.integrationSettings.upsert({
    ...
    update: { 
      ...
      localAiEndpoint: data.localAiEndpoint || null,
