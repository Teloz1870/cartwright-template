...
const domainSchema = z.object({
  ...
  // setupEmail = shoppens hoved-email (afsender + admin-modtager). Læses runtime
  // af lib/brand.ts getBrand() → emailFrom/emailAdmin (med brand.config-fallback).
  ...
  // Valgfri Resend-key — tænder rigtig email-udsendelse (ellers .mail-previews/).
...
export async function saveDomainStep(formData: FormData): Promise<ActionResult> {
  ...
  if (!parsed.success) {
    return { ok: false, error: "Ugyldig data — tjek email-formatet" };
  }
  ...
  // 1. Domæne + email-identitet → BrandingSettings. emailFrom/emailAdmin/
  //    emailFromName læses runtime af lib/brand.ts getBrand() (med
  //    brand.config-fallback), så det her er IKKE dead data — det overstyrer
  //    afsender/kontakt på tværs af shoppen.
  ...
  // 2. Valgfri Resend-key → genbrug den fuldt-wirede integrations-action
  //    (validerer 're_'-prefix, krypterer, invaliderer cache). Først her, så en
  //    ugyldig key fanges og rapporteres frem for at advancere wizard'en.
  if (resendKey) {
...
export async function bootstrapStoreAction(prompt: string): Promise<ActionResult> {
  ...
  return { ok: false, error: res.error ?? "AI-bootstrap fejlede" };
}
