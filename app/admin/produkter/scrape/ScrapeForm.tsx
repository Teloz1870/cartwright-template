  ...
  return (
    <div className="flex max-w-2xl flex-col gap-4">
      ...
      <p className="text-xs text-sol-muted">
        Kun fra sider du ejer eller har tilladelse til at importere fra. Kræver
        FIRECRAWL_API_KEY.
      </p>
      ...
      {draft && (
        <div className="flex flex-col gap-3 rounded-xl border-2 border-sol-ink/10 bg-sol-sand p-4">
          <label className="flex flex-col gap-1">
            <span className="text-xs font-bold uppercase text-sol-muted">Navn</span>
            <input className={input} value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs font-bold uppercase text-sol-muted">Beskrivelse</span>
            <textarea
          ...
          {draft.attributes.length > 0 && (
            <p className="text-xs text-sol-muted">
              {draft.attributes.length} attributter · {draft.imageUrls.length} billeder fundet
            </p>
          )}
          ...
          >
            {pending ? "Opretter…" : "Opret produkt"}
          </button>
        </div>
      )}
    </div>
