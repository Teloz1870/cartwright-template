...
export async function bulkUpdateProducts(
  ...
  if (productIds.length === 0) return { ok: false, count: 0, error: "Ingen produkter valgt." };
  ...
  try {
    ...
    return { ok: false, count: 0, error: err instanceof Error ? err.message : "Kunne ikke opdatere." };
  }
