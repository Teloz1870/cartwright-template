...
export default async function EditProductPage({ params }: EditProductPageProps) {
  ...
  const [product, categories, features] = await Promise.all([
    prisma.product.findUnique({
      where: { id },
      include: {
        // Task B: load variants so VariantsAdmin can render the table.
    ...
    <div className="flex flex-col gap-6">
      ...
      <ProductForm
        ...
        product={{
          ...
          // P1.2: the ProductForm type was widened to string | null after the schema change
          frameColor: product.frameColor,
          lensColor: product.lensColor,
          brand: product.brand,
          // (TypeScript narrowing is handled by the updated ProductFormProduct type)
          featured: product.featured,
          categoryId: product.categoryId,
          attributes: product.attributes as Record<string, unknown> | null,
          // AEO (answer-first) fields — gated on aeoContent.
