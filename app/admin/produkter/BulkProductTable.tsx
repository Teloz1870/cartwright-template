...
export function BulkProductTable({
  ...
  function apply() {
    ...
    startTransition(async () => {
      ...
      if (r.ok) {
        setMsg(`${r.count} produkt(er) opdateret.`);
        setSelected(new Set());
        setPriceKr("");
        setStock("");
        setCategoryId("");
    ...
    <div className="flex flex-col gap-3">
      {selected.size > 0 && (
        <div className="flex flex-wrap items-end gap-2 rounded-xl border-2 border-sol-accent/30 bg-sol-accent/5 p-3">
          <span className="text-sm font-bold text-sol-ink">{selected.size} valgt:</span>
          <input className={`${input} w-24`} placeholder="Pris (kr)" value={priceKr} onChange={(e) => setPriceKr(e.target.value)} />
          <input className={`${input} w-20`} placeholder="Lager" value={stock} onChange={(e) => setStock(e.target.value)} />
