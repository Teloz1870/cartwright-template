...
function actorLabel(raw: string): { label: string; color: string } {
  if (raw.startsWith("apikey:")) return { label: "AI (API-key)", color: "bg-sol-accent text-sol-cream" };
  if (raw.startsWith("user:")) return { label: "Admin (web)", color: "bg-sol-ink text-sol-cream" };
  if (raw.startsWith("operator-chat:")) return { label: "Admin (chat)", color: "bg-purple-600 text-sol-cream" };
    ...
    <div className="flex flex-col gap-8">
      <header>
        ...
        <p className="mt-2 max-w-2xl text-sm font-medium text-sol-muted">
          ...
          Click &quot;Show payload&quot; to see input/before/after snapshots. Destructive
          tools markeret revertible kan rulles tilbage med ét klik.
        </p>
      </header>

      {/* Filter-form (GET-baseret så links er delbare) */}
      <form className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-4 shadow-sm">
