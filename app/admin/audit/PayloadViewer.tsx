...
/**
 * Client-side expandable JSON viewer for audit entry payloads. Hidden by
 * default because they are bulky; the user clicks "show payload" to open.
 */
export default function PayloadViewer({
  ...
  return (
    <div className="space-y-2 text-xs">
      ...
      {errorMsg && (
        <Block label="Error" content={errorMsg} tone="error" />
      )}
      {argsJson && <Block label="Input (args)" content={pretty(argsJson)} />}
      {beforeJson && <Block label="Before state" content={pretty(beforeJson)} />}
