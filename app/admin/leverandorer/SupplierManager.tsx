    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end gap-2 rounded-xl border-2 border-sol-ink/10 bg-sol-sand p-3">
        ...
        <select className={input} value={mode} onChange={(e) => setMode(e.target.value)}>
          ...
          <option value="email">Email + bekræft</option>
        </select>
        <button
          type="button"
          disabled={pending}
      ...
      {suppliers.length === 0 ? (
        <p className="text-sol-muted">Ingen leverandører endnu.</p>
      ) : (
        <ul className="flex flex-col gap-1 text-sm">
          {suppliers.map((s) => (
            <li key={s.id} className="flex items-center justify-between rounded-lg border-2 border-sol-ink/10 px-3 py-2">
              ...
              <button type="button" disabled={pending} onClick={() => run(() => deleteSupplier(s.id))} className="text-xs font-bold text-red-600 hover:underline">Slet</button>
            </li>
          ))}
        </ul>
      )}
