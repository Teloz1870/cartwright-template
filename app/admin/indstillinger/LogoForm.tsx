...
import { updateLogoSettings, generateLogoWithGemini } from "./actions";
...
export default function LogoForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        setMessage({ type: "success", text: "Logo gemt og opdateret!" });
      } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Nyt logo genereret! Husk at gemme, hvis du vil beholde det." });
    } catch (err: unknown) {
  ...
  const handleGenerateRaster = async () => {
    ...
    try {
      ...
      setImageUrl(res.url);
      setMessage({ type: "success", text: "Gemini-logo genereret, uploadet og gemt! 🎨" });
    } catch (err: unknown) {
      setMessage({ type: "error", text: (err instanceof Error ? err.message : "") || "Gemini-generering fejlede." });
    } finally {
  ...
  return (
    <div className="bg-sol-sand p-6 rounded-xl border border-sol-ink/10 space-y-6">
      ...
      <div className="space-y-4 p-4 bg-white/50 rounded-lg border border-sol-ink/5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-black uppercase text-sol-muted">1. Upload Logo Billede</h3>
            <p className="text-sm text-sol-muted mt-1">Upload et .png eller .svg billede for at overskrive outline-logoet.</p>
          </div>
      ...
      <div className="space-y-4 p-4 bg-sol-cream/50 rounded-lg border border-sol-ink/5">
        <h3 className="text-sm font-black uppercase text-sol-muted">2. ✨ Eller AI Generér et Outline-Logo</h3>
        <p className="text-sm text-sol-muted">Hvis du ikke har et uploadet billede, vises dette AI-genererede stroke-ikon.</p>
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="F.eks. 'Et minimalistisk solbrille outline' eller 'En kaffekop'"
          ...
          >
            {isGenerating ? "Genererer..." : "Generér Logo"}
          </button>
        </div>
      </div>
      ...
      {logoGeneratorEnabled && (
        <div className="space-y-4 p-4 bg-sol-accent/5 rounded-lg border border-sol-accent/20">
          <h3 className="text-sm font-black uppercase text-sol-muted">3. 🎨 Gemini-logo (raster, ægte billede)</h3>
          <p className="text-sm text-sol-muted">
            Genererer et rigtigt billed-logo med Gemini, uploader til Vercel Blob og sætter det
            som brandets logo. Kræver Gemini-key + BLOB_READ_WRITE_TOKEN.
          </p>
          <div className="flex gap-2">
            <input
              type="text"
              ...
              placeholder="F.eks. 'Hoptify — en glad grøn frø der hopper, moderne flad logo'"
              className="flex-1 rounded-md border border-sol-ink/20 px-3 py-2 text-sol-ink focus:border-sol-accent focus:outline-none focus:ring-1 focus:ring-sol-accent"
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
            ...
            >
              {isGeneratingRaster ? "Genererer…" : "Generér med Gemini"}
            </button>
          </div>
        </div>
      )}
