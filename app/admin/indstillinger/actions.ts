...
): Promise<{ ok: true; ignored?: string[] } | { ok: false; error: string }> {
  try {
    await requireAdmin();
    ...
    // A locked policy owns identity, so those columns are dropped from the
    // write rather than stored inert. The form is disabled in that state, but
    // a Server Action is a public endpoint — the check belongs here too.
    const { data: identity, ignored } = withoutLockedIdentity({
    ...
    await prisma.brandingSettings.update({
      ...
      data: {
        ...
        defaultLocale: defaultLocale?.trim() || brand.defaultLocale,
      },
    });
    ...
    if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
      try {
        ...
        await redis.set(
          "cartwright_default_locale",
          defaultLocale?.trim() || brand.defaultLocale,
        );
      } catch (err) {
    ...
    return ignored.length ? { ok: true, ignored } : { ok: true };
  } catch (error) {
