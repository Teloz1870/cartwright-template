 ...
 *
 * Grid-view af alle MediaAsset-rækker med filtre på aiStatus + type. Per asset
 * kan admin se thumbnail, alt-tekst-snippet, AI-status og hvor mange steder
 ...
 *   ?attached=yes|no                 (har relationer til Product/etc eller ikke)
 */
...
export default async function AdminMediaPage({
  ...
  // attached-filter er post-query (Prisma kan ikke aggregate-filter cross-relation)
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-sol-ink">Media-bibliotek</h1>
          <p className="mt-1 text-sm font-semibold text-sol-muted">
            Centraliseret oversigt over alle uploadede billeder + videoer.
            Alt-tekst og SEO/GEO-metadata genereres automatisk af Gemini.
          </p>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <section className="sol-card-elevated px-5 py-8 text-sm font-semibold text-sol-muted">
          Ingen media-rækker matcher filteret. Upload via /admin/produkter eller
          tilsvarende UI for at se rækker her — eller kør backfill-scriptet:
              ...
              >
                ...
                <div className="flex flex-col gap-1 px-3 py-3 text-xs">
                  <div className="line-clamp-2 font-semibold text-sol-ink">
                    {asset.altDa ?? <span className="italic text-sol-muted">— mangler alt-tekst —</span>}
                  </div>
