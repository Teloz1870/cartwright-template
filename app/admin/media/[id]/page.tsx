 ...
 * client-side form (server actions from /admin/media/actions.ts).
 */
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        <AdminButton href="/admin/media" variant="plain" size="sm">
          ← Media library
        </AdminButton>
        <AdminBadge tone="neutral">{asset.aiStatus}</AdminBadge>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr,1.2fr]">
        <AdminCard padding="none" className="overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 px-5 py-4 text-xs text-sol-muted">
            ...
            <Meta k="Size" v={`${(asset.sizeBytes / 1024).toFixed(0)} KB`} />
            <Meta k="Dimensions" v={dimensions(asset.width, asset.height, asset.durationSec)} />
            ...
            <Meta k="AI attempts" v={String(asset.aiAttempts)} />
            <Meta k="Uploaded by" v={asset.uploadedBy ?? "—"} />
          </div>
          {asset.aiLastError && (
            <div className="border-t border-rose-200 bg-rose-50 px-5 py-3 text-xs text-rose-900">
              <span className="font-black">Last error:</span> {asset.aiLastError}
            </div>
          )}
      ...
      <AdminCard title={`Used in ${usageItems.length} place${usageItems.length === 1 ? "" : "s"}`}>
        {usageItems.length === 0 ? (
          <EmptyState
            title="Not attached to anything yet."
            description="You can delete this asset without breaking anything."
          />
        ) : (
