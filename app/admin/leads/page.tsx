...
/**
 * Leads & project inquiries — the canonical inbox for incoming enquiries from
 * the Project Estimator / contact form (the Lead table).
 *
 * Merges the former /admin/henvendelser into this page (both ran the same
 * prisma.lead.findMany): this page keeps the richer enquiry view
 * (message, attachments, AI triage summary + suggested reply) alongside the
 * leads' budget/company fields.
 */
            ...
            {leads.length === 0 ? (
              ...
              leads.map((lead) => (
                <AdminTr key={lead.id} className="align-top">
                  ...
                  <AdminTd className="font-semibold align-top">
                    {lead.projectType}
                    {/* WHICH form produced this lead. A site with a calculator,
                        a booking form and a plain contact form used to receive
                        three indistinguishable rows. */}
                    {lead.source && (
                      <div className="text-xs font-normal text-sol-muted mt-1 break-words">
                        via {lead.source}
                      </div>
                    )}
                    <LeadDetails data={lead.data} />
                  </AdminTd>
                  <AdminTd className="align-top">
