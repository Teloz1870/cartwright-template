  ...
  return (
    <div className="rounded-xl border-2 border-sol-ink/10 p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button type="button" disabled={pending} onClick={() => run(() => deleteZone(zone.id))} className="text-xs font-bold text-red-600 hover:underline">Delete zone</button>
      </div>
      <ul className="mb-3 flex flex-col gap-1 text-sm">
        {zone.rates.map((r) => (
          <li key={r.id} className="flex items-center justify-between border-t border-sol-ink/10 py-1">
            ...
            <button type="button" disabled={pending} onClick={() => run(() => deleteRate(r.id))} className="text-xs text-red-600 hover:underline">Delete</button>
          </li>
        ))}
      </ul>
