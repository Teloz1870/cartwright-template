...
export default function VisualBuilderClient({
  ...
  const runMagic = useCallback(async () => {
    ...
    if (intent.length < 8) {
      setStatus("Beskriv siden lidt mere (min. 8 tegn).");
      return;
    }
    setMagicBusy(true);
    setMagicStatuses([]);
    setStatus("Planlægger siden …");

    const planRes = await magicPlanAction(intent);
    if (!planRes.ok) {
      setMagicBusy(false);
      setStatus(`Fejl: ${planRes.error}`);
      return;
    }
    ...
    setStatus(`Genererer ${plan.length} sektioner …`);
    ...
    setMagicBusy(false);
    setStatus("Magic-side genereret ✓ — gennemse i preview og publicér.");
        ...
        <section className="rounded-lg border border-cw-terracotta/30 bg-cw-terracotta/5 p-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-stone-500">
              Beskriv en hel side — AI lægger en plan af whitelisted sektioner og fylder dem on-brand.
            </span>
          </div>
          <div className="mt-2 flex flex-col gap-2 sm:flex-row">
            <textarea
              className="min-h-[60px] flex-1 rounded border px-2 py-1 text-sm"
              placeholder="fx: En landingsside for et kafferisteri: hero, 3 værdi-kort, priser, anmeldelser, FAQ og en afsluttende CTA."
              value={magicIntent}
            ...
            >
              {magicBusy ? "Genererer …" : "Generér side"}
            </button>
          </div>
