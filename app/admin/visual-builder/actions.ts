 ...
 * skrive noget. Klienten genererer derefter node-for-node (step 2), så
 * sektioner kan streame live ind i preview. Gated bag magicBuilder.
 */
...
> {
  ...
  if (!brand.features.magicBuilder) {
    return {
      ok: false,
      error: "Magic Builder er ikke aktiveret. Slå 'Magic Builder' til (kræver redeploy).",
    };
  }
  if (intent.trim().length < 8) {
    return { ok: false, error: "Beskriv siden lidt mere (min. 8 tegn)." };
  }
  try {
    ...
    return {
      ok: false,
      error:
        err instanceof Error ? `Planlægning fejlede: ${err.message}` : "Planlægning fejlede",
    };
  }
}
...
> {
  ...
  const brand = await getBrand();
  if (!brand.features.magicBuilder) {
    return { ok: false, error: "Magic Builder er ikke aktiveret." };
  }
  if (!isSectionKey(node.key)) {
    return { ok: false, error: `Ukendt sektionstype: ${node.key}` };
  }
  const adapter = SOURCE_ADAPTERS[node.source];
  if (!adapter) {
    return { ok: false, error: `Ukendt kilde: ${node.source}` };
  }
  try {
    ...
    return {
      ok: false,
      error: err instanceof Error ? err.message : "Generering fejlede",
    };
  }
}
