  ...
  function apply(slug: string, applySkin: boolean) {
    ...
    startTransition(async () => {
      ...
      if (r.ok) {
        ...
        else if (r.skinSkipped) extras.push(`skin “${r.skinSkipped}” not installed`);
    ...
    <div className="flex flex-col gap-4">
      {!genomeResolveOn ? (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <strong>Note:</strong> “Resolvable Genome” is off, so the homepage still shows the
          original anchors. Turn it on under{" "}
          <Link className="underline" href="/admin/features">
            Funktioner
          </Link>{" "}
          to see an applied Voice live on the homepage (requires a website-mode design such as
          Aurora or Studio).
        </div>
      ) : null}
