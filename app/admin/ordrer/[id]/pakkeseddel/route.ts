...
/**
 * Standalone print HTML — deliberately a route handler (not a page) so the packing
 * slip does not inherit the admin layout's sidebar. The operator uses the browser's
 * "Save as PDF". Auto-print on load. No PDF dependency on serverless.
 */
...
<head>
  <meta charset="UTF-8" />
  <title>Packing slip ${esc(m.shortId)} — ${esc(brand.storeName)}</title>
  <style>
...
<body>
  <div class="head">
    <div>
      ...
      <div class="muted">Packing slip / pick list</div>
    </div>
    <div style="text-align:right;">
      <h1>Order ${esc(m.shortId)}</h1>
  ...
  <table>
    <thead>
      <tr><th>Item</th><th>SKU</th><th style="text-align:center;">Qty</th><th>Pick</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>

  <div class="total">Total: ${m.totalUnits} unit(s)</div>
