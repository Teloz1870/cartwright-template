  ...
  return (
    <div className="rounded-xl border-2 border-sol-ink/10 bg-sol-sand p-4">
      ...
      <p className="mb-3 text-xs text-sol-muted">
        Footeren linker til <code className="rounded bg-sol-ink/5 px-1">/info/privacy</code>,{" "}
        <code className="rounded bg-sol-ink/5 px-1">/info/terms</code> og{" "}
        <code className="rounded bg-sol-ink/5 px-1">/info/cookies</code>. Mangler de,
        opret dem her (boilerplate du bør gennemgå). Eksisterende sider røres ikke.
      </p>
      ...
      <button
        ...
        onClick={() =>
          startTransition(async () => {
            ...
            setMsg(
              r.created.length
                ? `Oprettet: ${r.created.join(", ")}.`
                : "Ingen manglende sider.",
            );
            router.refresh();
          })
        }
      ...
      >
        {pending
          ? "Opretter…"
          : missing.length
            ? `Opret ${missing.length} manglende side(r)`
            : "Alle sider findes"}
      </button>
