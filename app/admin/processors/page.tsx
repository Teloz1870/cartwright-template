...
export default async function AdminProcessorsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Databehandlere & GDPR"
        subtitle="Processor-register (GDPR art. 28) + status på juridiske sider. Registret redigeres i brand.config.ts (policies.processors)."
      />

      <AdminCard title={`Processor-register (${processors.length})`} padding="none">
        <AdminTable>
          <AdminThead>
            <AdminTr>
              <AdminTh>Processor</AdminTh>
              <AdminTh>Formål</AdminTh>
              <AdminTh>Delt data</AdminTh>
              <AdminTh>DPA</AdminTh>
            </AdminTr>
          </AdminThead>
          <AdminTbody>
