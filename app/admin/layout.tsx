...
export default async function AdminLayout({
  ...
  const aiStatusExtensions =
    voiceShopOn && voiceShopSettings && voiceUsage
      ...
      : undefined;
  // AI deaktiveret fra start: hverken chat-launcher eller status-pill floater i
  // hjørnet før en provider faktisk er konfigureret (key/endpoint). På en frisk
  // scaffold uden nøgle = ingen AI-UI overhovedet; pill'en viser ALDRIG en løs
  // "AI offline" i hjørnet. "degraded" tæller som konfigureret (værd at vise).
  const aiConfigured =
  ...
  return (
    <div data-admin-skin className="min-h-screen bg-sol-cream text-sol-ink md:flex">
      ...
      {/* Global ⌘K-launcher + AI status-pill — kun når en AI-provider er
          konfigureret. Fresh scaffold uden nøgle ⇒ intet AI-element i hjørnet
          (AI deaktiveret fra start). Sættes op i /admin/integrations. */}
      {aiConfigured && (
        <>
          <AdminChatLauncher />
          <AiStatusPill
