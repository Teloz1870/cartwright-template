...
type NavLink = {
  ...
  docsImport?: boolean;
  /** Only show when brand.features.visualBuilderEnabled is true. */
  visualBuilder?: boolean;
  /** Only show when brand.features.hoptify is true — the page notFound()s otherwise. */
  hoptify?: boolean;
};
...
const allNavLinks: NavLink[] = [
  ...
  { href: "/admin/visual-builder", label: "Visual Builder", visualBuilder: true },
  ...
  { href: "/admin/hoptify", label: "Hop off Shopify 🐸", hoptify: true },
...
export default async function AdminLayout({
  ...
  const visualBuilderEnabled = Boolean(
    (brand.features as { visualBuilderEnabled?: boolean }).visualBuilderEnabled,
  );
  const navLinks = allNavLinks.filter((l) => {
    ...
    if (l.visualBuilder && !visualBuilderEnabled) return false;
    if (l.hoptify && !features.hoptify) return false;
    return true;
  });
