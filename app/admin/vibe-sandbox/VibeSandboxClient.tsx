...
import { Monitor, Tablet, Smartphone, FileCode, Settings, Globe } from "lucide-react";
...
type Props = {
  pages: PageData[];
  /** Whether brand.features.v0Generator is on — gates the v0 engine option. */
  v0Enabled?: boolean;
};
...
<div className="py-20 text-center">
  <div className="max-w-4xl mx-auto px-4">
    <span className="text-xs font-black uppercase tracking-[0.2em] text-[var(--cw-brand-glow-1)]">Cartwright Engine</span>
    ...
    <p className="text-xl text-white/60 leading-relaxed font-light mb-8 max-w-2xl mx-auto">
      This design was generated 100% via AI (Vibe Coding) and renders directly across the platform's hardened backend architecture.
    </p>
...
export default function VibeSandboxClient({ pages, v0Enabled = false }: Props) {
  ...
  // Initialise selection from the ?id= URL param (derived state → lazy initialiser,
  // no effect needed). Selection afterwards is driven by user clicks.
  const [selectedPageId, setSelectedPageId] = useState<string>(() =>
    urlId && pages.some((p) => p.id === urlId) ? urlId : "new",
  );
  ...
  // Generation engine: Anthropic (default) or Vercel v0 (when flag on).
  const [engine, setEngine] = useState<"anthropic" | "v0">("anthropic");
  // Link to the v0 chat after a v0 generation, so the admin can iterate visually.
  const [v0WebUrl, setV0WebUrl] = useState<string | null>(null);

  // Load selected page into editor
  /* eslint-disable react-hooks/set-state-in-effect -- syncs ~9 editor fields when the selected page changes (admin Vibe editor); flagged for review (candidate for key-remount). */
  useEffect(() => {
  ...
  const handleGenerateAI = async () => {
    ...
    const activeEngine = v0Enabled ? engine : "anthropic";

    try {
      const res = await fetch("/api/admin/vibe/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: aiPrompt, engine: activeEngine }),
      });
      ...
      if (data.html) {
        ...
        if (typeof data.webUrl === "string") setV0WebUrl(data.webUrl);
        setMessage({
          type: "success",
          text:
            activeEngine === "v0"
              ? "Generated with v0 — cleaned + sanitised to HTML 🎉"
              : "AI Design genereret succesfuldt! 🎉",
        });
      }
    } catch (error) {
      console.error(error);
  ...
  const handleAutoClean = (code: string, isEnglish: boolean) => {
    ...
    setMessage({ type: "success", text: "The code was automatically stripped of React components!" });
  };
    ...
    startTransition(async () => {
      try {
        ...
        if (res.ok) {
          setMessage({
            type: "success",
            text: `The page is published! You can view it at /info/${slug}`
          });
          router.refresh();
        } else {
          setMessage({ type: "error", text: res.error || "Could not save the page." });
        }
      } catch (err: unknown) {
        setMessage({ type: "error", text: (err instanceof Error ? err.message : "") || "Network error while saving." });
      }
    });
  };
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-5 flex flex-col gap-5">
        ...
        {/* Page selector */}
        <div className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-4 shadow-sm">
          ...
          <select
            ...
            className="w-full rounded-lg border border-sol-ink/15 bg-sol-cream px-3 py-2 text-sm font-semibold text-sol-ink focus:border-sol-accent focus:outline-none"
          >
            <option value="new">+ Create new vibe page</option>
            {pages.map((p) => (
        ...
        {/* Editor Box */}
        <div className="rounded-2xl border border-sol-ink/10 bg-sol-sand shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex border-b border-sol-ink/10 bg-sol-cream/50 px-2 pt-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 text-xs font-black uppercase tracking-wider border-b-2 rounded-t-lg transition ${
                activeTab === "code-da"
                  ? "border-sol-accent text-sol-accent bg-sol-sand"
          ...
          {/* Editor Body */}
          <div className="p-4 bg-sol-sand">
            
            {/* AI Generator Box */}
            <div className="mb-6 p-4 rounded-xl border border-sol-accent/20 bg-sol-accent/5">
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs font-black uppercase text-sol-accent">
                  ✨ In-House AI Designer
                </label>
                {v0Enabled && (
                  <div
                    className="inline-flex rounded-full border border-sol-ink/15 bg-sol-cream p-0.5 text-[11px] font-bold"
                    role="group"
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. 'Build a beautiful pricing page with 3 columns and a dark background'"
              ...
              <p className="text-xs text-sol-muted mt-2">
                The AI already knows your brand and Vibe rules. It spits the HTML out below.
                {v0Enabled && engine === "v0" && (
                  <span className="text-sol-ink/70">
                    {" "}
                    v0 generates code → normalised + sanitised to HTML.
                  </span>
                )}
              </p>
              {v0WebUrl && (
                <a
                  ...
                  className="inline-block mt-2 text-xs font-bold text-sol-accent hover:underline"
                >
                  Open in v0 to iterate ↗
                </a>
              )}
            </div>
            ...
            {activeTab === "code-da" && (
              <div className="flex flex-col gap-3">
                ...
                <textarea
                  ...
                  placeholder="Paste your v0 or Lovable HTML/Tailwind code here..."
            ...
            {activeTab === "code-en" && (
              <div className="flex flex-col gap-3">
                ...
                <textarea
                  ...
                  placeholder="Paste your English version here (optional, falls back to Danish)..."
                ...
                <div className="rounded-xl border border-sol-ink/10 bg-sol-cream/30 p-3 mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2">
                      <label className="text-sm font-semibold text-sol-ink">Order:</label>
                      <input
                        type="number"
                        min="0"
                        value={navOrder}
                        onChange={(e) => setNavOrder(parseInt(e.target.value) || 0)}
                        className="w-16 rounded-lg border border-sol-ink/15 bg-sol-cream px-2 py-1 text-sm font-semibold text-sol-ink"
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="border-t border-sol-ink/10 bg-sol-cream/20 p-4 flex justify-between items-center">
            ...
            <div className="flex gap-2 ml-auto">
              ...
              >
                {isPending ? "Saving..." : selectedPageId === "new" ? "Publish Page" : "Save Changes"}
              </button>
            </div>
          </div>
      ...
      <div className="lg:col-span-7 flex flex-col gap-4">
        ...
        {/* Toolbar */}
        <div className="rounded-2xl border border-sol-ink/10 bg-sol-sand p-3 shadow-sm flex items-center justify-between">
          <div className="flex gap-1 bg-sol-cream rounded-lg p-1">
            <button
              onClick={() => setViewport("desktop")}
              className={`p-2 rounded-md transition ${
                viewport === "desktop" ? "bg-sol-sand text-sol-accent shadow-sm" : "text-sol-muted hover:text-sol-ink"
              }`}
              title="Desktop (Full)"
            >
            ...
            <button
              ...
              className={`p-2 rounded-md transition ${
                viewport === "tablet" ? "bg-sol-sand text-sol-accent shadow-sm" : "text-sol-muted hover:text-sol-ink"
              }`}
              title="Tablet"
            >
            ...
            <button
              ...
              className={`p-2 rounded-md transition ${
                viewport === "mobile" ? "bg-sol-sand text-sol-accent shadow-sm" : "text-sol-muted hover:text-sol-ink"
              }`}
              title="Mobile"
            >
