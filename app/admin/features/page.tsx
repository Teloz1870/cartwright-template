...
function StatusPill({ on }: { on: boolean }) {
  return (
    <AdminBadge tone={on ? "success" : "neutral"}>{on ? "On" : "Off"}</AdminBadge>
  );
}
...
export default async function AdminFeaturesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Features"
        subtitle="Turn the shop's features on and off. Runtime features can be changed live — changes take effect on the storefront within 30 seconds. Build-time features and identity are shown as status and changed in brand.config.ts."
      />
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {!f.implemented && (
                      <AdminBadge tone="neutral">Not implemented</AdminBadge>
                    )}
                    {f.implemented && f.overridden && (
                      <AdminBadge tone="attention">Overridden</AdminBadge>
                    )}
                    {f.tier === "compile-time" && (
                      <AdminBadge tone="neutral">Build-time</AdminBadge>
                    )}
                  </div>
                  ...
                  {f.enabled && f.blockedReason && (
                    <p className="mt-1 text-[11px] font-semibold text-amber-700">
                      Invalid enabled state: {f.blockedReason}
                    </p>
                  )}
                </div>
      ...
      <AdminCard
        title="Identity"
        description="Defines what the site IS. Locked — can never be changed from this page (brand.config.ts is the only source)."
      >
        <ul className="flex flex-col divide-y divide-sol-ink/5">
          {identity.map((i) => (
            <li
