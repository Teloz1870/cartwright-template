...
> **`--profile site` scaffold?** No Turso, no `AUTH_SECRET`, no environment variables: push the repo and import it at vercel.com, or run `npx vercel`. The rest of this guide is the default profile's. See `docs/simple-site.md`.
...
```bash
# From a local terminal, with the DB env pointed at Turso.
# NOTE: `prisma.config.ts` loads `.env` FIRST and then `.env.local` with
# override: true — so `.env.local` wins wherever both set the same variable.
# Check both before running anything destructive; a safe-looking `.env` says
# nothing about which database these commands actually hit.
npm run db:deploy   # = prisma migrate deploy
npx prisma db seed
```

> **Warning:** `prisma db seed` clears all tables — orders, carts and user accounts included — before inserting the template catalogue. Run it ONLY on a fresh DB.
>
> Since v0.42.x the seed refuses, and exits non-zero instead of deleting, whenever the database already holds someone's work: any order, product, page or category, more than the one admin account the seed creates, or any recorded admin action. That covers the pre-launch shop and the website-mode site, which sit at one admin and zero orders while holding everything their owner wrote. The consequence is deliberate — **after any successful seed, seeding again is a decision, not a default**. `pnpm db:setup` is unaffected: it only ever seeds a fresh database.
>
> To wipe a database on purpose, re-run with `ALLOW_DESTRUCTIVE_SEED=1`.
>
> **On a database the seed will not touch, `pnpm admin:create` is how you get a first admin** — one
> INSERT, nothing deleted, nothing overwritten. Locked out of an account that already exists? `pnpm
> admin:reset` (password only, all data kept). Full map: [`docs/dev-to-prod.md`](docs/dev-to-prod.md).
