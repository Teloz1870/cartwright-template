...
The exception to "default-off": **`cartwrightBadge`** is **default-on** — the "Built with
Cartwright" referral signal (à la "Made with Framer"). Since the ownership ledger
(`scaffold/manifest.json` → `engineOnly`) it is no longer one switch over one badge, so check
what it actually gates in *your* profile:

- The **footer badge** (and its "want one like this?" line) is gated on the `engineMarketing`
  capability as well as the flag, in both footers. In a `--profile site` scaffold that
  capability is `false`, so the badge never renders whatever the flag says.
- The **`SoftwareApplication` JSON-LD** lives on `/built-with-cartwright`, which the ledger
  marks engine-only. A scaffold cut with create-cartwright 2.9.5 still has that route on disk;
  from create-cartwright 2.10 the scaffolder deletes it, and the markup goes with the page.
- The **"Built with Cartwright" block in `llms.txt`** reads the flag alone, in every profile.
  It links cartwright.app and npm rather than a local route, so nothing gates it but you.

So: setting `cartwrightBadge: false` in `brand.config.ts` (or turning it off in
`/admin/features`) is still how you remove the signal, and it is the only one of the three that
removes the `llms.txt` block.
