...
7. **Real a11y minimums always:** one `<h1>` per page, semantic landmarks, alt
   ...
   animation.
8. **Bind real data — never hardcode the owner's content.** Services, prices and
   products must read from the database (`Service` model via the services helpers,
   `Product` via the catalog helpers, settings/genome for copy), and opening hours
   and contact details from `brand.config.ts` → `contact` (it holds `phone` and
   `hours`; no database column exists for either) — NOT be string literals in your
   JSX. A `--profile site` scaffold has no database at all: there the content lives
   in files, and the rule is the same — read it from there, don't retype it. Why: the owner
   ...
   was hardcoded.)
9. **Never invent the owner's facts.** Reviews, testimonials, customer names,
   follower counts, awards, certifications — if it did not come from the database,
   `brand.config.ts`, or a source the owner handed you, it does not go on the page.
   An empty state is honest ("Reviews coming soon"); an invented one is a liability
   the owner inherits at launch, and it survives review precisely because it looks
   plausible. For the categories rule 8 already owns — services, prices, products,
   opening hours, contact details — the answer is where rule 8 puts them, not a tidier literal.
   What a pack legitimately states on its own — the owner's history, provenance, how
   the place works — goes in ONE module, `designs/<slug>/facts.ts`, one field per fact
   with its source in a comment. `UNVERIFIED` is for a fact the owner gave you that you
   could not independently confirm: write it in that field's source comment
   (`// source: owner's email — UNVERIFIED`) and name it when you hand the work over, so
   the owner checks it before launch — never in the rendered value. It is not a
   licence to publish something you made up. (Found live: an AI-built farm shop shipped invented
   testimonials — one attributed to a sitting head of government — alongside invented
   like-counts, names and products.)
10. **Run the template-tells check before you call it designed.** Rules 1-6 catch bad
    layout; this one catches *generic*. Three or more of these together mean you have
    shipped the default AI look rather than a design: one rounded-card treatment
    reused for every kind of content; emoji standing in for icons; a tracked ALL-CAPS
    eyebrow over *every* heading; an arrow glyph on every link and CTA; the same badge
    repeated down the page; middle-dot metadata rows; more than three tints of one
    accent colour. Each is legitimate when chosen deliberately — `eyebrow` is a real
    prop on most shipped sections — but reached for by default they compose into a
    kit, and every AI-built site wearing that kit looks like every other one. (Found
    live: a pack briefed as "a truly unique design" that used all seven at once,
    including one certification badge fourteen times and seven different greens.)
11. **`tokens.palette.accent` is the UI accent, not a brand colour.** When your pack
    sets `applyPaletteAsTheme: true` and the shop has no `themeJson` override, your
    palette becomes the shop's theme — and `--color-sol-accent` then paints engine
    surfaces your pack does not own: the PDP price, the checkout submit button, field
    focus rings, cart link hovers. So pick a colour that survives being a
    call-to-action. A logo colour, or a certification mark's red (organic, fair-trade,
    eco-label), belongs in `tokens.extraTokens` or the pack's own stylesheet — never
    in `palette.accent`. Leave `applyPaletteAsTheme` off and those surfaces keep the
    engine default palette instead (or the shop's `themeJson`, when it has one) — the
    other half of the same trap. (Found live: a
    farm shop whose organic-label red became the price and every checkout button.)
...
2. **Screenshot at 1440px wide (desktop) AND 390px wide (mobile).** If your project
   has `scripts/dev-screenshot.mjs` — light and full scaffolds do; a `--profile site`
   scaffold prunes it — then `pnpm verify:design [path]` captures both widths into
   `.screenshots/` and **exits non-zero** on horizontal overflow or an `<h1>` count
   other than one. It needs `pnpm dev` already running; it never starts a server.
   `--full-page` covers work below the fold. `--selector "#thing"` captures a single
   component but **skips the overflow and `<h1>` checks** — an exit 0 there is a
   screenshot, not a passed gate, so run it without `--selector` to gate the page.
   Otherwise use whatever browser/screenshot tooling you have — a DevTools MCP or any
   headless capture; note that a site scaffold prunes `@playwright/test` along with the
   script. Where Playwright *is* installed, a bespoke script has to live inside the
   project, because `@playwright/test` resolves upward from the importing file.
