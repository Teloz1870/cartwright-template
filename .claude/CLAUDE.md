...
1. **Boot.** If `npx create-cartwright` just ran, install + DB + seed are already done — skip to `pnpm dev`. Manual clone: `pnpm install && pnpm db:setup && pnpm dev`. Admin login is in `.admin-credentials`. Verify: `curl -sL -o /dev/null -w '%{http_code}' http://localhost:3000/` → expect `200` (`/` redirects to your `brand.defaultLocale` — `/en` by default, `/da` if you set it; `-L` follows it).
...
4. **Apply a designed look** — one call, instant, no LLM involved:
   ...
   A **Voice** (`verticals/`: `cafe`, `carpenter`, `dentist`, `fable`, `fitness`, `kindergarten`, `restaurant`, `salon`) applies pre-written on-brand copy + palette + a suggested design + 3D scene in one step. To pick a specific **Skin** instead/as well, pass `"design":"<slug>"` (slugs in `designs/options.ts`) or call `design.set_slug`. A downloaded look installs via `composition.apply`.

5. **Verify the design landed:** `curl -sL http://localhost:3000/ | grep -o '<h1[^>]*>[^<]*'` (`-L` follows the redirect to your `brand.defaultLocale`) — the H1 and the `--color-sol-*` palette variables in the HTML should reflect the chosen look.
...
**The page wrapper (don't reach for `app/layout.tsx`):** the engine already wraps your homepage in `<main className="min-h-[60vh]">` (seam: `app/[locale]/layout.tsx`), so render `<section>`/`<div>` from `homepage.tsx`, never a second `<main>`. Control that wrapper declaratively via the optional `layout` field in `designs/blank/index.ts` — `{ mainClassName: "" }` (full-bleed), `{ mainClassName: "min-h-screen" }` (fullscreen), or `{ ownsMain: true }` (you render the `<main>`, e.g. via a `Shell`; see `designs/drive`). **CSS isolation:** scope under `.blank-canvas` OR use a `*.module.css` CSS Module (Next.js auto-scopes it). Want a typed-terminal/code hero? Copy `designs/stack` + `themes/studio.css` (CSS-only) — don't hand-roll timeouts.
...
- `cartwrightBadge` — **default-on** "Built with Cartwright" referral signal (like "Made with Framer"). It gates three things, and since the ownership ledger (`scaffold/manifest.json` → `engineOnly`) they no longer behave alike: the **footer badge** is gated on `profileCapabilities.engineMarketing` as well as the flag, so it never renders in a `--profile site` scaffold; the **`SoftwareApplication` JSON-LD** sits on `/built-with-cartwright`, an engine-only route that create-cartwright 2.9.5 still leaves on disk and 2.10 deletes; the **`llms.txt` block** reads the flag alone in every profile. Flipping the flag false is still the way to remove the signal, and the only way to remove the `llms.txt` block.
...
## Capturing leads (contact forms, calculators, quote wizards)

**Do not build your own inquiry table — the engine already has one.** An agent
building a custom site reached for a new `Lead` model because nothing in these
briefings mentioned the existing one; that rebuild would have been pure waste.

| Piece | Where |
|---|---|
| Model | `Lead` in `prisma/schema.prisma` |
| Public endpoint | `POST /api/inquiries` |
| No-DB variant | `app/api/inquiries/route.static.ts` (site profile — mails the owner) |
| Server action | `app/actions/lead.ts` (`createLead`) — built-in fields only, **not** `source`/`data` |
| Inbox | `/admin/leads` |
| Shipped form | `components/SmartContactForm.tsx` on `/[locale]/contact` |

POST any form to `/api/inquiries`. Beyond the built-in fields (`name`, `email`,
`phone`, `company`, `message`), two fields exist for **custom** forms:

- **`source`** — which form this was (`"hegnsberegner"`, `"quote-wizard"`).
  Without it every form produces indistinguishable rows.
- **`data`** — a JSON object of whatever your form collected (m², model,
  postcode…). Deliberately schemaless; shown as key/value rows in `/admin/leads`.
...
Bounds (it is a public, unauthenticated endpoint): `source` ≤ 64 chars, `data`
must be an object with ≤ 50 keys and ≤ 4 KB serialised. Over any cap the
request is a `400` naming WHICH one — `code: "source_too_long"` /
`"data_too_many_keys"` / `"data_too_large"` — it does **not** silently truncate.
`data` may nest 32 deep (`data_too_deep`). A nested `__proto__` is stripped at any depth, and an OBJECT under `constructor`/`prototype` is refused (`data_reserved_key`) — a STRING under those names is an ordinary field and is kept. An empty or null `source` counts as absent, never as a rejection. Unknown top-level keys ARE still stripped by
Zod, so put custom fields inside `data`, not beside it.

Adding your OWN column to `Lead` needs `pnpm db:push` before the code runs:
Prisma names every column in `INSERT … RETURNING`, so an unmigrated database
fails on writes as well as reads.

Optional flags: `leadAiTriage` (AI priority/summary/draft reply, default off),
`contactAttachments` (Blob uploads via `/api/contact/upload`, default off).
...
- **Test before push.** `pnpm dev` and click through the storefront — at minimum `/` (redirects to your default locale) and (if webshop) the products page (`/<defaultLocale>/produkter`) — before pushing to main.
