...
```
Then `pnpm build` for a full check. Open a PR — never push to `main` (it mirrors
to the public template in ~60s) without the 3-canary smoke (`bash scripts/smoke-canaries.sh`).
