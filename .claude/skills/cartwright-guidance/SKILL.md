...
Next builds a complete method table for every route module (`autoImplementMethods`, in `next/dist/server/route-modules/app-route/helpers/`). Three consequences, each measured against a production build of this engine on `next@16.3.0` (the version pinned in `package.json` — re-measure before trusting any of it on another major):
