...
- **Shared plumbing lives in `lib/model-context.ts`** (zero-import core;
  `lib/webmcp/` holds only a re-export shim). Use
...
- **Per-route tools, not a global toolbox.** Site-wide tools live in
  `components/WebMcpRegistrar.tsx` (mounted in the locale layout; every
  scaffold profile ships it — the old light-profile prune is gone).
  Page-contextual tools live in
...
## 11. Lead capture — never build a second inquiry table

A custom form (calculator, quote wizard, booking) should POST to the endpoint
that already exists, not motivate a new model:

`Lead` (`prisma/schema.prisma`) ← `POST /api/inquiries` → `/admin/leads`.
Server action: `app/actions/lead.ts` (built-in fields only — no `source`/`data`).
No-DB site profile:
`app/api/inquiries/route.static.ts` (mails the owner).
...
await fetch("/api/inquiries", {
  ...
  body: JSON.stringify({
    name, email,
    source: "hegnsberegner",              // WHICH form — else rows are indistinguishable
    data: { meters: 42, model: "Type 1" }, // WHAT the visitor configured
  }),
});
```

`source` ≤ 64 chars; `data` must be an object, ≤ 50 keys, ≤ 4 KB serialised —
it is a public, unauthenticated endpoint. Exceeding a cap returns `400` naming
which one (`source_too_long` / `data_too_many_keys` / `data_too_large`), never a
silent truncation. `data` may nest 32 deep (`data_too_deep`). A nested `__proto__` is stripped at any depth, and an OBJECT under `constructor`/`prototype` is refused (`data_reserved_key`) — a STRING under those names is an ordinary field and is kept. An empty or null `source` counts as absent, never as a rejection. Zod strips
unknown top-level keys, so custom fields go INSIDE `data`.
...
Flags: `leadAiTriage` (AI priority/summary/draft reply), `contactAttachments`
(Blob uploads) — both default off.
