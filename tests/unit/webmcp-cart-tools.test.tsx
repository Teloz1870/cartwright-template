...
describe("CartWebMcpTools — undo lives on the cart page", () => {
  ...
  it("update forwards to the action; quantity 0 points at remove_cart_item (single-function tool)", async () => {
    await mount();
    ...
    // Removal is remove_cart_item's whole job — two removal paths would be
    // exactly the tool overlap Chrome's best-practices doc warns about.
    const zero = (await t.execute({ cartItemId: "line-1", quantity: 0 })) as Record<string, unknown>;
    expect(String(zero.error)).toContain("remove_cart_item");
    expect(cartActions.updateCartItemAction).toHaveBeenCalledTimes(1);
  });

  it("schemas enumerate the KNOWN cart line ids (fresh on every re-registration)", async () => {
    await mount();
    for (const name of ["update_cart_item_quantity", "remove_cart_item"] as const) {
      const schema = tool(name).inputSchema as {
        properties: { cartItemId: { enum?: string[] } };
      };
      expect(schema.properties.cartItemId.enum).toEqual(["line-1", "line-2"]);
    }
  });
  ...
  it("go_to_checkout only navigates — it does not place anything", async () => {
    ...
    expect(t.description).toContain("ordering stays with the human");
    const result = (await t.execute({})) as Record<string, unknown>;
    // jsdom's pathname here is "/" — no locale segment, so the bare path.
    // The locale-derivation branch is pinned separately below.
    expect(assignedPath).toBe("/checkout");
    expect(result).toEqual({ status: "navigating", path: "/checkout" });
  });

  it("go_to_checkout reuses the page URL's locale segment (canonical path)", async () => {
    (window.location as unknown as { pathname: string }).pathname = "/en/cart";
    await mount();
    const result = (await tool("go_to_checkout").execute({})) as Record<string, unknown>;
    expect(result.path).toBe("/en/checkout");
    expect(assignedPath).toBe("/en/checkout");
  });
