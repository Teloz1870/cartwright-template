 ...
 *    The fifth came anyway, through a door the gate did not watch:
 *    `TrustBadges` wrote its three `/info/*` links as object PROPERTIES
 *    (`{ href: "/info/shipping" }`), not attributes, so the scan passed while
 *    both webshop canaries 307'd every trust badge into the default locale.
 *    Properties are scanned now too.
 *
...
function bareLocaleLinks(file: string, src: string, segments: string[]): string[] {
  ...
  // OBJECT PROPERTIES, not just attributes. `TrustBadges` built its badges as
  // `{ href: "/info/shipping" }` and mapped them onto plain `next/link` — the
  // attribute scans above never saw it, and it shipped on both webshop
  // canaries. A property has no owning tag to inspect, so the only exemption
  // is the file-level one: a file that imports the routing `Link` may be
  // feeding it these paths, and a false red in a fork is worse than the hole.
  const bareProps = [
    ...src.matchAll(/\b(\w*[Hh]ref)\s*:\s*"\/([a-z0-9-]+)([^"]*)"/g),
    ...src.matchAll(/\b(\w*[Hh]ref)\s*:\s*'\/([a-z0-9-]+)([^']*)'/g),
    ...src.matchAll(/\b(\w*[Hh]ref)\s*:\s*`\/([a-z0-9-]+)([^`]*)`/g),
  ];
  for (const match of bareProps) {
    if (!segments.includes(match[2]) || localeAwareLink) continue;
    offenders.push(`${file}: { ${match[1]}: /${match[2]}${match[3] ?? ""} }`);
  }
  return offenders;
}
...
describe("shared components link into the reader's locale", () => {
  ...
  it("the detector actually fires", () => {
    ...
    // The object-property form, in all three quote styles — the TrustBadges shape.
    for (const prop of [`{ href: "/${seg}/x" }`, `{ href: '/${seg}' }`, "{ href: `/" + seg + "` }"]) {
      expect(bareLocaleLinks("synthetic.tsx", prop, SEGMENTS), prop).toHaveLength(1);
    }
    expect(
      bareLocaleLinks("synthetic.tsx", "{ href: `/${locale}/" + seg + "` }", SEGMENTS),
      "a correctly prefixed property was reported",
    ).toEqual([]);
    // A file feeding the routing Link may write bare property paths.
    const viaRoutingLink = `import { Link } from "@/i18n/routing";\nconst items = [{ href: "/${seg}" }];`;
    expect(bareLocaleLinks("synthetic.tsx", viaRoutingLink, SEGMENTS)).toEqual([]);
  });
...
/**
 * ── The shopper's own route bodies ────────────────────────────────────────
 *
 * The two blocks above gate shared COMPONENTS. The route bodies under
 * `app/[locale]` were never gated at all, and the PDP — the page every
 * product card in the engine links to — had drifted furthest: twelve strings
 * that no locale could change, measured 2026-09-09 on `origin/main` and
 * confirmed live on `solbrillen.dk/da/product/aviator-classic` ("Category",
 * "In stock", "More from this category" as visible Danish-page copy).
 *
 * It had drifted in BOTH directions, which is the tell that nothing was
 * watching: eleven English strings on a page whose default locale is Danish,
 * and one Danish string — `generateMetadata`'s "Produkt ikke fundet" — that
 * an English reader got instead. The comment above the price prefix even
 * described the intended Danish output (`"fra X" hvis variants har
 * forskellige priser`) while the JSX below it rendered `from`.
 *
 * The strict helpers are deliberate. `jsxTextNodes` (two words, no entity
 * decoding) saw NONE of `What it&apos;s good for` — the entity carries a `;`
 * — nor the one-word `from` before `<Price>`. Only `anyJsxTextCopy` finds
 * both, so that is the one this block runs.
 */
const SHOPPER_ROUTES = ["app/[locale]/product/[slug]/page.tsx"].filter(existsSync);

describe("shopper route bodies follow the page locale", () => {
  /**
   * Not a pinned count. `modules/registry.ts` claims the PDP for `commerce`,
   * so a website-mode or `--profile site` scaffold prunes it and a fixed
   * number would go red in exactly the forks this test ships to — the R1/R2
   * release-blocker class. The invariant instead: a repo that ships a cart
   * ships a product page, and a product page that exists is gated here.
   */
  it("covers the PDP wherever a shop exists", () => {
    if (existsSync("app/[locale]/cart")) {
      expect(
        SHOPPER_ROUTES,
        "this repo has a cart but the PDP is not under the locale gate",
      ).toContain("app/[locale]/product/[slug]/page.tsx");
    }
    expect(SHOPPER_ROUTES.length).toBeLessThanOrEqual(1);
  });

  it("the detector fires on the copy this converted", () => {
    // Non-vacuity without naming a file: both shapes the strict helper adds
    // over `jsxTextNodes` must be caught, or the block below proves nothing.
    expect(anyJsxTextCopy("<h2>What it&apos;s good for</h2>")).toEqual(["What its good for"]);
    expect(anyJsxTextCopy("<p>from <Price /></p>")).toEqual(["from"]);
    expect(anyJsxTextCopy('<p>{t("priceFrom")} <Price /></p>')).toEqual([]);
  });

  it.each(SHOPPER_ROUTES)("%s renders no hardcoded copy at all", (file) => {
    expect(
      anyJsxTextCopy(read(file)),
      `${file} has JSX copy that no locale can change`,
    ).toEqual([]);
  });

  it.each(SHOPPER_ROUTES)("%s hardcodes no sentence in an expression", (file) => {
    expect(
      hardcodedSentences(read(file)),
      `${file} has copy in an expression that no locale can change`,
    ).toEqual([]);
  ...
  it.each(SHOPPER_ROUTES)("%s hardcodes no aria-label/placeholder/title copy", (file) => {
    expect(
      hardcodedAttributeCopy(read(file)),
      `${file} announces copy no locale can change`,
    ).toEqual([]);
  ...
  it.each(SHOPPER_ROUTES)("%s names only keys that resolve in every locale", (file) => {
    const src = read(file);
    const ns = /\b(?:useTranslations|getTranslations)\("(\w+)"\)/.exec(src)?.[1];
    expect(ns, `${file} declares no namespace`).toBeTruthy();
    const keys = [...src.matchAll(/\bt\("(\w+)"[,)]/g)].map((m) => m[1]);
