...
describe("discounts.try_apply — the shopper-facing lookup key", () => {
  it("refuses a whitespace-only code instead of looking up the empty one", async () => {
    ...
    // normaliser lives at the call site (lib/tools/customer.ts:254), so `.min(1)`
    // used to measure the raw string — `" "` passed, and the lookup asked for
    // `""`, which is exactly the row the pre-fix `discounts.create` could mint.
    ...
    // The DB lookup is unchanged for a real code: the schema now strips padding
    // that the call site stripped anyway. The parsed value itself does differ
    // (`.trim()` rewrites it) — nothing reads it except that lookup.
