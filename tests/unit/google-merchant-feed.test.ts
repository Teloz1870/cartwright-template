...
describe("buildGoogleMerchantXml", () => {
  ...
  it("emitter g:product_detail (name/value) for hver produkt-attribut", () => {
    const xml = buildGoogleMerchantXml(
      [item({ attributes: { "Frame color": "Black", Material: "Acetate" } })],
      channel,
    );
    expect(xml).toContain("<g:product_detail>");
    expect(xml).toContain("<g:attribute_name>Frame color</g:attribute_name>");
    expect(xml).toContain("<g:attribute_value>Black</g:attribute_value>");
    expect(xml).toContain("<g:attribute_name>Material</g:attribute_name>");
    expect(xml).toContain("<g:attribute_value>Acetate</g:attribute_value>");
  });

  it("udelader g:product_detail når produktet ingen attributter har (bagudkompat)", () => {
    expect(buildGoogleMerchantXml([item()], channel)).not.toContain(
      "<g:product_detail>",
    );
  });

  it("escaper attribut-navn/værdi i g:product_detail", () => {
    const xml = buildGoogleMerchantXml(
      [item({ attributes: { "Bredde <cm>": "1 & 2" } })],
      channel,
    );
    expect(xml).toContain("<g:attribute_name>Bredde &lt;cm&gt;</g:attribute_name>");
    expect(xml).toContain("<g:attribute_value>1 &amp; 2</g:attribute_value>");
  });
});
