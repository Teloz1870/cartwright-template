...
describe("sanitizeContentRow — vibeHtml (incl. nested translations)", () => {
  it("strips script/onerror from top-level vibeHtml, keeps safe content", async () => {
    const html = clean((await sanitizeContentRow({ slug: "p", vibeHtml: HOSTILE })).vibeHtml);
  ...
  it("strips script from nested translations.<locale>.vibeHtml", async () => {
    const out = await sanitizeContentRow({
      slug: "p",
  ...
  it("leaves non-HTML fields and a non-page layoutJson UNTOUCHED", async () => {
    const out = await sanitizeContentRow({ storeName: "Aluzaun", layoutJson: brandingLayout });
    expect(out.storeName).toBe("Aluzaun");
    expect(out.layoutJson).toBe(brandingLayout); // never run through the page-layout sanitizer
  });

  it("does NOT sanitize body/description/caption (escaped at render, not HTML sinks)", async () => {
    ...
    const out = await sanitizeContentRow({ body, description: body, caption: body });
    expect(out.body).toBe(body);
    expect(out.description).toBe(body);
    expect(out.caption).toBe(body);
  });

  it("is prototype-pollution safe (drops __proto__/constructor in untrusted JSON)", async () => {
    ...
    const out = await sanitizeContentRow(evil as Record<string, unknown>);
  ...
  it("leaves a non-string (object-valued) vibeHtml untouched rather than crashing", async () => {
    const out = await sanitizeContentRow({ vibeHtml: { not: "a string" } });
    expect(out.vibeHtml).toEqual({ not: "a string" });
  });

  it("throws a CONTROLLED error on pathologically deep nesting (no stack overflow)", async () => {
    let deep: unknown = { vibeHtml: "<p>ok</p>" };
    for (let i = 0; i < 500; i++) deep = { nested: deep };
    await expect(sanitizeContentRow(deep as Record<string, unknown>)).rejects.toThrow(/nesting|depth/i);
  });
});

describe("sanitizeLayoutJson — Page.layoutJson (the vibe-section html sink)", () => {
  it("sanitizes the vibe section html and keeps the layout schema-valid", async () => {
    const cleaned = clean(await sanitizeLayoutJson(pageLayout(HOSTILE)));
  ...
  it("returns null for absent / empty / non-JSON / invalid input", async () => {
    expect(await sanitizeLayoutJson(null)).toBeNull();
    expect(await sanitizeLayoutJson(undefined)).toBeNull();
    expect(await sanitizeLayoutJson("")).toBeNull();
    expect(await sanitizeLayoutJson("{not json")).toBeNull();
    expect(await sanitizeLayoutJson(JSON.stringify({ sections: [] }))).toBeNull(); // min(1)
    expect(await sanitizeLayoutJson(JSON.stringify({ sections: [{ id: "s1", key: "NOPE", enabled: true }] }))).toBeNull();
  });

  it("drops a layout whose vibe html is entirely unsafe (emptied → fails re-validation)", async () => {
    expect(await sanitizeLayoutJson(pageLayout("<script>alert(1)</script>"))).toBeNull();
  });

  it("sanitizes EVERY vibe section when a layout has more than one", async () => {
    ...
    const cleaned = clean(await sanitizeLayoutJson(twoVibes));
    expect(cleaned).not.toMatch(/<script/i);
    expect(cleaned).not.toMatch(/onerror/i);
...
describe("sanitizePageRow — vibeHtml + Page.layoutJson together", () => {
  it("sanitizes both the vibeHtml field and the layoutJson html sink", async () => {
    const out = await sanitizePageRow({ slug: "home", vibeHtml: HOSTILE, layoutJson: pageLayout(HOSTILE) });
  ...
  it("nulls an invalid layoutJson on a page row (body fallback)", async () => {
    const out = await sanitizePageRow({ slug: "home", layoutJson: "{not json" });
...
describe("parseSitePackContent — sanitizes the right collections, passes the rest", () => {
  ...
  it("sanitizes vibeHtml in EVERY content collection (pages, categories, services, posts, products, branding)", async () => {
    const c = await parseSitePackContent(entries());
  ...
  it("drops a non-object NDJSON row (primitive/array) without crashing", async () => {
    const m = new Map<string, Buffer>();
    ...
    const c = await parseSitePackContent(m);
    expect(c.pages).toHaveLength(1);
    expect(c.pages[0].slug).toBe("ok");
  });

  it("returns null branding when branding.json is a non-object (array)", async () => {
    const m = new Map<string, Buffer>();
    m.set("content/branding.json", Buffer.from("[1,2,3]", "utf8"));
    expect((await parseSitePackContent(m)).branding).toBeNull();
  });

  it("passes joins/metadata through untouched and PRESERVES branding's layoutJson", async () => {
    const c = await parseSitePackContent(entries());
  ...
  it("treats a missing collection as empty / null", async () => {
    const c = await parseSitePackContent(new Map());
...
describe("parseSitePackContent — end-to-end through openCartpack", () => {
  it("a consistent pack carrying hostile vibeHtml clears integrity, then is sanitized on parse", async () => {
