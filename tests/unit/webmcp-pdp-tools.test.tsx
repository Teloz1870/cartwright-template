...
describe("PdpWebMcpTools — the page's own add-to-cart", () => {
  ...
  it("carries the product's specifications, so the agent knows what the Google feed knows", async () => {
    // Before this, the PDP descriptor held name/price/stock/variants only,
    // while lib/feeds/catalog-feed.ts published the same product's specs to
    // Google/ChatGPT as g:product_detail — the on-page agent knew LESS about
    // the product than Gemini shopping did. The specs ride in the description
    // rather than a tool of their own, so no moat const or its exact-equality
    // test has to move.
    await mount({
      ...espresso,
      specs: { origin: "Ethiopia", roast: "2", weightG: "250" },
    });
    expect(registrations.map((r) => r.tool.name)).toEqual(
      Object.keys(PDP_WEBMCP_TOOL_BINDINGS),
    );
    const d = registrations[0].tool.description;
    expect(d).toContain("Specifications: origin: Ethiopia; roast: 2; weightG: 250.");
  });

  it("says nothing about specifications when the product has none", async () => {
    await mount(espresso);
    expect(registrations[0].tool.description).not.toContain("Specifications:");
    await mount({ ...espresso, specs: {} });
    expect(registrations[0].tool.description).not.toContain("Specifications:");
  });
