...
vi.mock("next/navigation", () => ({
  // Mirrors what the App Router gives a client component: the current path.
  // Reading it off window.location keeps `onPath()` below the single way a
  // test says "the shopper is on this page".
...
describe("search_products output redaction", () => {
  it("returns ONLY the advertised row shape — checkout endpoints and images never enter the browser surface", async () => {
    ...
    const apiRow = {
      ...
      price: 9900,
      currency: "DKK",
      unitPrice: { amountMinor: 1436, currency: "USD", formatted: "$14.36" },
    ...
    try {
      ...
      expect(Object.keys(row).sort()).toEqual(
        ["currency", "description", "formattedPrice", "id", "inStock", "price", "title", "url"].sort(),
      );
      expect(row).toMatchObject({ price: 1436, currency: "USD", formattedPrice: "$14.36" });
...
/**
 * The locale defect, pinned. An agent on /en asked for a product and was handed
 * a /da link, which pulled the whole conversation into Danish. The registrar
 * mounts once with an empty dep array and has no locale prop, so the prefix has
 * to be read at CALL time — a value captured at registration goes stale the
 * moment the shopper switches language without a full reload.
 */
describe("the tool surface speaks the locale of the page it is on", () => {
  const onPath = (pathname: string) => {
    window.history.replaceState({}, "", pathname);
  };

  it("sends the current locale with the search request", async () => {
    onPath("/en/produkter");
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ query: "x", resultsCount: 0, products: [] })),
    );
    vi.stubGlobal("fetch", fetchMock);

    await mount();
    const search = registrations.find((r) => r.tool.name === "search_products")!.tool;
    await search.execute({ query: "ethiopia" });

    expect(String(fetchMock.mock.calls[0][0])).toContain("locale=en");
    vi.unstubAllGlobals();
  });

  it("follows a client-side switch to another locale", async () => {
    // The stale-closure case: same registration, different page.
    onPath("/da/produkter");
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ query: "x", resultsCount: 0, products: [] })),
    );
    ...
    expect(String(fetchMock.mock.calls[0][0])).toContain("locale=da");
    vi.unstubAllGlobals();
  });

  it("writes example paths with the current prefix", async () => {
    onPath("/en");
    await mount();
    const nav = registrations.find((r) => r.tool.name === "navigate")!.tool;
    const cart = registrations.find((r) => r.tool.name === "get_cart")!.tool;

    // The segment stays `produkter` in every locale — that IS the route
    // (/en/products is a 404). Only the prefix was missing.
    expect(nav.description).toContain("/en/produkter");
    expect(nav.description).not.toContain("'/produkter'");
    expect(cart.description).toContain("/en/cart");
    expect(cart.description).not.toContain("'/cart'");
  });

  it("REBUILDS the descriptions when the shopper switches language in place", async () => {
    // The descriptions are plain strings, baked when the descriptor is built —
    // unlike the search fetch, they cannot be read at call time. So the
    // registrar has to re-register on a locale change. Without that, a shopper
    // going /en → /da client-side leaves an agent reading examples that point
    // at '/en/produkter', which walks the conversation back into English.
    onPath("/en/produkter");
    await mount();
    expect(
      registrations.find((r) => r.tool.name === "navigate")!.tool.description,
    ).toContain("/en/produkter");
    ...
    const nav = registrations.find((r) => r.tool.name === "navigate");
    expect(nav, "a locale switch must re-register the descriptors").toBeDefined();
    expect(nav!.tool.description).toContain("/da/produkter");
    expect(nav!.tool.description).not.toContain("/en/produkter");
  });

  it("does NOT re-register on an in-locale navigation", async () => {
    // The other half of the same decision: re-registering on every page change
    // would be churn the descriptors do not need — they say nothing about
    // which page you are standing on.
    onPath("/en/produkter");
    await mount();
    registrations.length = 0;

    onPath("/en/product/ethiopia-yirgacheffe");
    await act(async () => {
      root.render(<WebMcpRegistrar />);
    });
    await act(async () => {
  ...
  it("degrades to an unprefixed path rather than guessing a locale", async () => {
    onPath("/api-docs");
    await mount();
    const nav = registrations.find((r) => r.tool.name === "navigate")!.tool;
    // An unprefixed internal path still resolves, via one redirect. Inventing
    // a locale here would reproduce the bug in the other direction.
    expect(nav.description).toContain("'/produkter'");
  });
});
