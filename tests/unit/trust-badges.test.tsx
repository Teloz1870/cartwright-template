...
const mocks = vi.hoisted(() => ({
  // The request locale `getLocale()` resolves to. Switchable so the link
  // assertions can prove the locale is THREADED, not a hardcoded "en".
  locale: "en" as string,
  isStripeReady: vi.fn(),
...
// TrustBadges is a server component that resolves its labels via
// getTranslations("TrustBadges"). getTranslations needs a next-intl request
// context that doesn't exist in a bare unit test, so mock it to resolve against
// the real en.json — keeping these English-label assertions honest.
...
const VARIANTS = ["homepage", "product", "checkout", "footer", "category"] as const;
...
describe("TrustBadges", () => {
  ...
  it("links point to actual info paths", async () => {
    ...
    expect(html).toContain('href="/en/info/shipping"');
    expect(html).toContain('href="/en/info/returns"');
    expect(html).toContain('href="/en/info/faq"');
    expect(html).toContain('href="/en/privacy"');
  });

  // `/info/*` and `/privacy` exist only under `app/[locale]`, and the badges
  // render through plain `next/link`, which prefixes nothing. A bare path 307s
  // through the proxy into the shop's default locale — measured live on both
  // webshop canaries before this fix. Two locales, so a hardcoded "/en/" can't
  // pass; every variant, so a badge added to one variant is covered too.
  it.each(["en", "da"])(
    "every in-app link carries the request locale (%s) on every variant",
    async (locale) => {
      mocks.locale = locale;
      // The external e-mark link must stay absolute — it is not ours to prefix.
      vi.stubEnv("NEXT_PUBLIC_EMAERKET", "true");
      for (const variant of VARIANTS) {
        const html = await renderTrustBadges({ variant });
        const hrefs = [...html.matchAll(/href="([^"]*)"/g)].map((m) => m[1]);
        const internal = hrefs.filter((href) => href.startsWith("/"));
        expect(internal.length, `${variant}: rendered no in-app link`).toBeGreaterThan(0);
        for (const href of internal) {
          expect(href, `${variant} hands out a link outside /${locale}`).toMatch(
            new RegExp(`^/${locale}/`),
          );
        }
        expect(hrefs, `${variant}: the e-mark link was rewritten`).toContain(
