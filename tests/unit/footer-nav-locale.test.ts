 ...
 * `proxy.static.ts`, which routes `/icon` and `/og` by the same `isAssetExempt`
 * but has no `/oauth` branch (no OAuth routes ship there). A link to `/oauth/…`
 * from `Footer.static.tsx` would pass here and still 307→404 in a materialized
 * site scaffold. Neither footer links it today; if one ever does, split the
 * predicate per file rather than widening it.
 *
