...
describe("GET /api/products/search answers in the caller's language", () => {
  ...
  // Indexed access, not destructuring. `routing.locales` is a literal tuple, so
  // on the single-locale ["en"] shop a scaffold produces its TYPE is
  // `readonly ["en"]` — and `const [, second] = …` is then TS2493, "no element
  // at index 1". A compile error, not a runtime one, which is exactly why the
  // fork simulation I ran did not catch it: I ran vitest against it and never
  // tsc. The release scaffold gate did.
  const first = routing.locales[0];
  const other = routing.locales.find((l) => l !== first) ?? first;
