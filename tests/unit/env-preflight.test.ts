...
describe("assertEnv", () => {
  ...
  // The self-hoster escape hatch (DEPLOY.md §2). Without the preflight leg the
  // ALLOW_SQLITE_IN_PRODUCTION guard in lib/db.ts was unreachable dead code —
  // production + file: DATABASE_URL + the flag still crashed in preflight.
  it("prod accepts a file: DATABASE_URL when ALLOW_SQLITE_IN_PRODUCTION=1", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("AUTH_SECRET", "prod-secret");
    vi.stubEnv("ALLOW_SQLITE_IN_PRODUCTION", "1");
    vi.stubEnv("DATABASE_URL", "file:./data/prod.db");
    vi.stubEnv("TURSO_DATABASE_URL", "");
    vi.stubEnv("TURSO_AUTH_TOKEN", "");
  ...
  it("prod still requires Turso when the flag is set but DATABASE_URL is not file:", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("AUTH_SECRET", "prod-secret");
    vi.stubEnv("ALLOW_SQLITE_IN_PRODUCTION", "1");
    vi.stubEnv("DATABASE_URL", "");
    ...
    expect(() => assertEnv()).toThrow(
      "[Cartwright] Missing required env: TURSO_DATABASE_URL.",
    );
  });

  it("prod still requires Turso when DATABASE_URL is file: but the flag is unset", () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("AUTH_SECRET", "prod-secret");
    vi.stubEnv("ALLOW_SQLITE_IN_PRODUCTION", "");
    vi.stubEnv("DATABASE_URL", "file:./data/prod.db");
    vi.stubEnv("TURSO_DATABASE_URL", "");
    vi.stubEnv("TURSO_AUTH_TOKEN", "");
