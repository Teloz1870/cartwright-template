...
describe("createOrder", () => {
  beforeEach(() => {
    ...
    mocks.getCheckoutCurrency.mockResolvedValue("DKK");
  ...
  it("snapshotter base-currency + fxRate 1 på en default (mock-mode) ordre", async () => {
    ...
    expect(orderCreateMock).toHaveBeenCalledWith({
      data: expect.objectContaining({ currency: "DKK", fxRate: 1 }),
    });
  });

  it("opkræver + snapshotter den valgte presentment-currency i Stripe-mode", async () => {
    mocks.getCheckoutCurrency.mockResolvedValue("EUR");
    ...
    // Ordren snapshotter presentment-currency + den brugte rate (EUR @ 0.134).
    ...
    // Stripe opkræves det KONVERTEREDE beløb i EUR minor-units:
    // round(44700 * 0.134) = 5990 cents.
