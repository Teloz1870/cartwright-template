...
describe("llms.txt", () => {
  it("gating a line removes the LINE, not the layout — a scaffold gets one blank line, not two", async () => {
    // The engine-only `/changelog` entry sat in a `${[…].join("\n")}` on its
    // own template line, so a profile with neither that route nor a merchant
    // feed rendered an empty string BETWEEN two newlines: a double blank line
    // in the customer's llms.txt, in the default scaffold. Both canaries read
    // `engineMarketing: true`, so the engine could not see it — this test
    // substitutes the static twin the way the materializer does.
    vi.resetModules();
    vi.doMock("@/lib/profile-capabilities", async () =>
    ...
    expect(body, "the engine's own changelog is not the customer's").not.toContain("/changelog");
    expect(body).toMatch(/is in ØRE\.\n\n/);
    expect(body, "an omitted line must not leave a hole where it was").not.toMatch(
      /is in ØRE\.\n\n\n/,
    );
  });
  ...
  /**
   * llms.txt addresses a reader in ONE language and then lists the site's pages
   * by name. Reading the base column verbatim made /da/llms.txt publish English
   * titles next to a Danish fallback heading — a mixed-language document handed
   * to AI crawlers. The base column is the SOURCE text
   * (`brand.defaultLocale`), so asking for the SOURCE locale must stay
   * byte-identical while a secondary locale picks up
   * `translations[locale].title`.
   *
   * Both locales are DERIVED. Typing `da`/`en` was true only while this
   * engine's own default was Danish — on the single-locale `["en"]` shop
   * `create-cartwright` produces, `en` IS the source and this case failed on a
   * scaffold that had done nothing wrong. With one locale there is no
   * secondary document to compare, so it skips, stated.
   */
  const SOURCE = brand.defaultLocale;
  const SECONDARY = brand.locales.find((l) => l !== SOURCE);
  const whenMultiLocale = SECONDARY ? it : it.skip;

  whenMultiLocale("publishes page titles in the locale the document is written for", async () => {
    const t = (title: string) => ({ [SECONDARY!]: { title } });
    const pages = [
      { slug: "about", title: "Om os", status: "published", translations: t("About us") },
      { slug: "brewing", title: "Sådan brygger vi", status: "published", translations: t("How we brew") },
      { slug: "faq", title: "Ofte stillede spørgsmål", status: "published", translations: {} },
    ];

    mocks.getBrand.mockResolvedValue(makeBrand());
    mocks.getFeatureView.mockResolvedValue({ features: [] });
    mocks.findUnique.mockResolvedValue(null);
    mocks.findMany.mockResolvedValue(pages);
    const { GET } = await import("@/app/llms.txt/route");

    const secondary = await (
      await GET(new NextRequest(`https://shop.example/llms.txt?locale=${SECONDARY}`))
    ).text();
    expect(secondary).toContain(`- [About us](https://shop.example/${SECONDARY}/about)`);
    expect(secondary).toContain(`- [How we brew](https://shop.example/${SECONDARY}/info/brewing)`);
    // Untranslated rows keep their source text rather than going blank.
    expect(secondary).toContain(`- [Ofte stillede spørgsmål](https://shop.example/${SECONDARY}/info/faq)`);
    expect(secondary).not.toContain("[Om os]");

    // The source locale is served from the base column, untouched.
    const source = await (
      await GET(new NextRequest(`https://shop.example/llms.txt?locale=${SOURCE}`))
    ).text();
    expect(source).toContain(`- [Om os](https://shop.example/${SOURCE}/about)`);
    expect(source).toContain(`- [Sådan brygger vi](https://shop.example/${SOURCE}/info/brewing)`);
    expect(source).not.toContain("[About us]");
  });
  ...
  it("does not promise a complete index — the sitemap is what the site advertises", async () => {
    // Measured 2026-09-07 against the live canaries: the old sentence ("the
    // complete index of all public pages") was false three ways over —
    // /<locale>/built-with-cartwright served 200 on all three while appearing
    // in none of their sitemaps; /manifest appeared in all three sitemaps and
    // answered 307; and Teloz served /cartwright, /priser, /cases and /start
    // unlisted. The replacement is true by construction, and is the same
    // sentence the site profile's twin (app/llms.txt/route.static.ts) uses.
    const body = await renderLlmsTxt();
    expect(body).toContain(
      "- [Sitemap](https://shop.example/sitemap.xml): the index of every public page this site advertises",
    );
    expect(body).not.toMatch(/complete index of all public pages/);
  });
  ...
  it("links the WebMCP inventory iff the webMcp flag is on (locale-qualified)", async () => {
    const off = await renderLlmsTxt();
    expect(off).not.toContain("webmcp-check");

    const on = await renderLlmsTxt({ features: { webMcp: true } });
    expect(on).toContain("[WebMCP agent tools]");
    expect(on).toContain("/en/webmcp-check");
  });
