...
describe("llms.txt", () => {
  ...
  it("always links the MCP discovery surface (mcp.json is force-static, never 404s)", async () => {
    const body = await renderLlmsTxt();
  ...
  it("flag-off output is byte-identical to flag-on output minus exactly the gated lines", async () => {
    const off = await renderLlmsTxt({ ecommerceEnabled: true });
    const on = await renderLlmsTxt({
      ecommerceEnabled: true,
      features: { acp: true, a2a: true },
    });
    const onMinusGated = on
      .split("\n")
      .filter(
        (line) =>
          !line.includes("/api/acp/") && !line.includes("/api/agent-card"),
      )
      .join("\n");
    expect(onMinusGated).toBe(off);
  });
});
