...
describe.skipIf(!isEngineCheckout)("the site door inside the scaffold", () => {
  ...
  it("docs/simple-site.md's receipt names a released version, no more than one behind — and lives in one place", () => {
    // The shape assertions below match any version forever, so the guide kept
    // citing create-cartwright@2.9.3 / engine v0.56.1 inside the v0.56.2
    // scaffolds that shipped it: a receipt for a different purchase, with every
    // gate green. The engine version is in this repo, so the check is exact.
    // "This release" cannot mean the version being cut: a receipt only exists
    // after the tag, because the scaffold gate measures the PUBLISHED pair. An
    // assertion demanding the new version would make every release PR go red
    // until someone invented a number — and the shape regexes below would then
    // certify the invention. The honest rule: the guide must cite a RELEASED
    // version, and be at most one release behind this repo.
    const released = [...readFileSync(join(ROOT, "CHANGELOG.md"), "utf8").matchAll(/^## (v\d+\.\d+\.\d+)/gm)].map((m) => m[1]);
    const cited = /engine (v\d+\.\d+\.\d+) \(/.exec(guide)?.[1];
    expect(cited, "the Measured block must cite an engine version").toBeTruthy();
    expect(released, `the guide cites ${cited}, which is not a released version`).toContain(cited);
    expect(
      released.indexOf(cited!),
      `the guide cites ${cited} but ${released[0]} and ${released[1]} have shipped since — re-measure on the published pair and refresh it`,
    ).toBeLessThanOrEqual(1);
    // And the receipt lives in ONE place, so a re-measurement is one edit.
    expect(
      readFileSync(join(ROOT, "CHANGELOG.md"), "utf8"),
      "the changelog must not carry a second copy of the measured run",
    ).not.toMatch(/release scaffold gate run \d{6,}/);
  });
