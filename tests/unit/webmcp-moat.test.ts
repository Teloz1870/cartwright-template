...
// no network. The enumeration is review-gated: a pack tool that wants []
// must be argued into one of these two sets.
//
// EMPTY since `calculate_brew_ratio` grew a recommendation. It used to be the
// only member: brew-math.ts arithmetic the human already got as a widget. It
// now also READS the catalogue, to name the coffee it recommends, so it binds
// ["products.search"] and leaves this set. Note what did NOT change: it
// performs no mutation and keeps `readOnlyHint`. The point of the class is
// "touches no data", not "is safe" — a tool that quietly stayed here while
// querying products would make the moat's own inventory a lie.
const PURE_COMPUTE = new Set<string>([]);
