...
const INTERNAL_PATHS: ReadonlyArray<{ needle: string; excludedBy: string }> = [
  ...
  // Engine-private working notes that lived outside internal-docs/: the
  // overnight autonomous-build reports at the repo root, the superpowers
  // plan/spec drafts, the X launch draft and the agentic-readiness session
  // hand-off. Records of HOW the engine was built, not deliverables.
  //
  // Needle shape is chosen per name, because THIS FILE SHIPS: a needle that
  // matches a customer's own writing fails THEIR `pnpm test` for something
  // they did not do. The four distinctive names are matched bare, so a
  // relative link (`../OVERNIGHT-REPORT.md`) is caught as surely as the full
  // path; `superpowers` alone is an ordinary word and a widely-installed agent
  // plugin, so that one carries its directory instead. "OVERNIGHT-REPORT"
  // already matches the master report — the master's own entry is here to pin
  // its own exclude line, which is the point of the needle/exclude pairing.
  //
  // `docs/HUL-*.md` is deliberately NOT here and keeps shipping: nine shipped
  // source files cite those three files by path (brand.config.ts, proxy.ts,
  // prisma/schema.prisma, lib/db.ts, lib/acp/complete.ts, lib/ucp/oauth.ts,
  // lib/feature-flags/manifest.ts, app/oauth/register, app/api/acp/…/complete),
  // so excluding them would CREATE the dead pointers this test exists to stop.
  { needle: "OVERNIGHT-REPORT", excludedBy: "/OVERNIGHT-REPORT*.md" },
  { needle: "MASTER-OVERNIGHT-REPORT", excludedBy: "/MASTER-OVERNIGHT-REPORT.md" },
  { needle: "docs/superpowers", excludedBy: "docs/superpowers/" },
  { needle: "x-launch-draft", excludedBy: "docs/x-launch-draft.md" },
  { needle: "agentic-readiness-handoff", excludedBy: "docs/agentic-readiness-handoff.md" },
];
...
const SHIPPED_ROOTS = [
  ...
  "scaffold",
  "scripts",
  // `tests/guards/` ships FURTHER than `tests/unit/` does — `--profile site`
  // deletes tests/unit wholesale and keeps the guard zone, so a pointer in a
  // guard docblock reaches every customer including the ones this file's other
  // test root never gets to. It was not swept at all until this line existed.
  "tests/guards",
  "tests/unit",
  "themes",
  "types",
...
/**
 * Skipped with a reason each, not to hide a finding:
 ...
 *  - so are the overnight reports, the superpowers drafts, the X launch draft
 *    and the agentic-readiness hand-off, as of the excludes block that pairs
 *    with INTERNAL_PATHS above. The rule is the one the three entries before
 *    them already follow: a mirror-excluded file is not a shipped file, so a
 *    pointer inside it cannot become a dead pointer downstream. Not
 *    hypothetical here — `OVERNIGHT-REPORT.md` and `MASTER-OVERNIGHT-REPORT.md`
 *    point at each other and the superpowers plans cite their own specs by
 *    path, so those would fail on their own names the moment the names became
 *    needles;
...
const NOT_SWEPT = new Set([
  ...
  ".gitignore",
  "OVERNIGHT-REPORT.md",
  "OVERNIGHT-REPORT-HOPTIFY.md",
  "MASTER-OVERNIGHT-REPORT.md",
  join("docs", "x-launch-draft.md"),
  join("docs", "agentic-readiness-handoff.md"),
]);

/** Mirror-excluded as whole directories: everything beneath is exempt. */
const NOT_SWEPT_DIRS = [join("docs", "superpowers")];

const isNotSwept = (rel: string): boolean =>
  NOT_SWEPT.has(rel) || NOT_SWEPT_DIRS.some((dir) => rel.startsWith(dir + sep));
...
function sweptFiles(): string[] {
  ...
  for (const root of SHIPPED_ROOTS) {
    ...
    for (const entry of readdirSync(abs, { recursive: true }) as string[]) {
      ...
      if (!swept(entry) || isNotSwept(rel)) continue;
  ...
  for (const entry of readdirSync(ROOT) as string[]) {
    ...
    if (stat?.isFile() && !isNotSwept(entry)) {
      found.push(entry);
    }
  }
