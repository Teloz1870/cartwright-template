...
describe("magic-link email template", () => {
  it("renders the subject, html button, expiry note, and text fallback", () => {
    ...
    expect(rendered.subject).toBe(`Log in to ${brand.storeName}`);
    expect(rendered.html).toContain(`Log in to ${brand.storeName}`);
    ...
    expect(rendered.html).toContain("This link expires in 15 minutes");
