...
describe("marketing.create_campaign — input schema bounds", () => {
  ...
  it("pins the min(3)-BEFORE-trim order: whitespace padding buys past the length check", async () => {
    const { createCampaign } = await loadTools();
    // Same zod ordering quirk as lib/tools/discounts.ts: `.min(3)` runs on the RAW string, the
    // `.trim().toUpperCase()` transform runs after — so a padded 2-char code slips through and is
    // stored 2 chars long. Pins CURRENT behaviour (not an endorsement — see the propose-only
    // backlog item; tightening it is a source change that would also flip this assertion).
    expect(() =>
      createCampaign.input.parse({
        discountCode: "ab",
        discountType: "percent",
