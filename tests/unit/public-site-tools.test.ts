...
describe("public site tools", () => {
  ...
  it("list_pages enforces published status in the database query", async () => {
    ...
    const asked = brand.locales[0];
    const result = await listPublicPages.handler({ locale: asked }, { actor: "system:test" });
    expect(mocks.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { status: "published" } }));
    // Derived, not typed: the announced locale is now clamped to brand.locales,
    // so a fork without `en` would fail a hardcoded `/en/about` for no fault of
    // its own — the fork-hostility class v0.53.1 was cut to fix.
    expect(result).toEqual([expect.objectContaining({ slug: "about", url: `/${asked}/about` })]);
  });
  ...
  it("supports pre-draft databases only when Page.status is the missing column", async () => {
    ...
    const { listPublicPages } = await import("@/lib/tools/site");
    const asked = brand.locales[0];
    await expect(listPublicPages.handler({ locale: asked }, { actor: "system:test" })).resolves.toEqual([
      expect.objectContaining({ slug: "about", url: `/${asked}/about` }),
    ]);
...
describe("public site tools answer in the locale they were asked for", () => {
   ...
   *  Never `SECONDARY as string` below — on a single-locale scaffold this
   *  const CFA-narrows to plain `undefined` (an annotation does not stop
   *  that; measured in a real scaffold), and the cast is TS2352 there.
   *  `SECONDARY!` is the pattern that compiles in both contexts — the
   *  same one llms-txt.test.ts uses — and is safe because every use site
   *  is inside a `whenMultiLocale` test that is skipped when undefined. */
   ...
   */
  /**
   * `list_pages` announces `/{locale}/about` for a row whose slug is `om-os` —
   * the slug every shipped seed actually uses. An agent that reads that URL and
   * asks `get_page {"slug":"about"}` must reach that row; an exact-match lookup
   * missed it and fell through to the synthesised generic engine text, which
   * belongs to no shop. The canonical route and the MCP `public-trust` resource
   * already resolve the same alias set.
   */
  it("resolves the alias set the canonical route resolves", async () => {
    mocks.findFirst
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ ...translated, slug: "om-os", title: "Vores historie" });
    const { getPublicPage } = await import("@/lib/tools/site");
    await expect(
      getPublicPage.handler({ slug: "about" }, { actor: "system:test" }),
    ).resolves.toEqual(expect.objectContaining({ slug: "om-os", title: "Vores historie" }));
  });

  /**
   * A shop can hold BOTH rows (importing an `about` page into a Danish seed).
   * One route cannot render two rows, so announcing both would hand an agent a
   * URL that shows the other row's content. The route reads `about` first.
   */
  it("announces one entry per route when two rows canonicalise to it", async () => {
    mocks.findMany.mockResolvedValue([
      { ...translated, slug: "om-os", title: "Vores historie" },
      { ...translated, slug: "about", title: "About us" },
    ]);
    const { listPublicPages } = await import("@/lib/tools/site");
    const result = await listPublicPages.handler({}, { actor: "system:test" });
    expect(result).toEqual([
      expect.objectContaining({ slug: "about", url: `/${brand.defaultLocale}/about` }),
    ]);
  });

  /**
   * A case variant canonicalises to the same URL but the route's exact-match
   * lookup can never reach it, so it must not displace the row that IS
   * rendered. A naive `indexOf` ranked it -1 — which beat every real source.
   */
  it("prefers the row the route resolves over a case variant of it", async () => {
    mocks.findMany.mockResolvedValue([
      { ...translated, slug: "About", title: "Case variant" },
      { ...translated, slug: "about", title: "About us" },
    ]);
    const { listPublicPages } = await import("@/lib/tools/site");
    await expect(listPublicPages.handler({}, { actor: "system:test" })).resolves.toEqual([
      expect.objectContaining({ slug: "about", title: "About us" }),
    ]);
  });

  /**
   * A shop with no CMS rows at all: `/{locale}/info/om-os` redirects to
   * `/{locale}/about`, which renders the synthesised About text — so the tool
   * must not answer `found:false` for the alias it announced that URL under.
   */
  it("synthesises the alias's canonical fallback, not found:false", async () => {
    mocks.findFirst.mockResolvedValue(null);
    const { getDefaultLegalContent } = await import("@/lib/legal/default-content");
    const { getPublicPage } = await import("@/lib/tools/site");
    const expected = getDefaultLegalContent("about", brand.defaultLocale);
    expect(expected).not.toBeNull();
    await expect(
      getPublicPage.handler({ slug: "om-os" }, { actor: "system:test" }),
    ).resolves.toEqual(expect.objectContaining({ slug: "om-os", title: expected!.title }));
  });

  /**
   * `lib/legal/default-content.ts` carries exactly two languages and branched
   * on `locale === "en"`, so every OTHER locale got Danish. Clamping the tool
   * to the shop's own locale would therefore have handed a German shop a
   * Danish privacy policy where it previously got English — a regression the
   * clamp created. English is the neutral fallback for a locale with no copy.
   */
  it.each(["de", "sv", "no"])("falls back to English, not Danish, for %s", async (locale) => {
    const { getDefaultLegalContent } = await import("@/lib/legal/default-content");
    expect(getDefaultLegalContent("privacy", locale)?.title).toBe(
      getDefaultLegalContent("privacy", "en")?.title,
    );
    expect(getDefaultLegalContent("privacy", "da")?.title).not.toBe(
      getDefaultLegalContent("privacy", "en")?.title,
    );
  });

  /**
   * The URL is the other half of the same answer: an agent reads `url` and
   * fetches it. `locale` decided the words after #528 but the path was still
   * interpolated raw, so an unpublished locale was announced as a route the
   * shop does not serve, and an omitted one produced a locale-less `/about`
   * that exists only as a redirect. The expectations derive the shop's own
   * locale rather than typing one, so a single-locale scaffold is not wrong.
   */
  it("announces a URL under a locale this shop actually serves", async () => {
    mocks.findMany.mockResolvedValue([translated]);
    const { listPublicPages } = await import("@/lib/tools/site");
    await expect(
      listPublicPages.handler({ locale: "de" }, { actor: "system:test" }),
    ).resolves.toEqual([
      expect.objectContaining({ url: `/${brand.defaultLocale}/info/brewing` }),
    ]);
  });

  it("announces the shop's own locale when none was asked for", async () => {
    mocks.findMany.mockResolvedValue([translated]);
    const { listPublicPages } = await import("@/lib/tools/site");
    await expect(listPublicPages.handler({}, { actor: "system:test" })).resolves.toEqual([
      expect.objectContaining({ url: `/${brand.defaultLocale}/info/brewing` }),
  ...
  /**
   * `om-os` is the original CMS slug for the About page and `proxy.ts` redirects
   * `/{locale}/info/om-os` to `/{locale}/about`. Announcing the raw slug cost
   * every agent a hop, and disagreed with `llms.txt`, which already resolves it.
   */
  it("announces the canonical trust route, not the legacy CMS slug", async () => {
    mocks.findMany.mockResolvedValue([{ ...translated, slug: "om-os" }]);
    const { listPublicPages } = await import("@/lib/tools/site");
    await expect(listPublicPages.handler({}, { actor: "system:test" })).resolves.toEqual([
      expect.objectContaining({ slug: "om-os", url: `/${brand.defaultLocale}/about` }),
    ]);
  });

  /**
   * The synthesised legal fallback (no CMS row exists) was the one path still
   * defaulting to `"en"` while every other path defaulted to the shop's own
   * language — so a Danish-only shop was handed English privacy text by an
   * anonymous agent read. Compared against the helper's own output for this
   * shop's locale, so the assertion holds whatever that locale is.
   */
  it("synthesises the legal fallback in the shop's own language", async () => {
    mocks.findFirst.mockResolvedValue(null);
    const { getDefaultLegalContent } = await import("@/lib/legal/default-content");
    const { getPublicPage } = await import("@/lib/tools/site");
    const expected = getDefaultLegalContent("privacy", brand.defaultLocale);
    expect(expected).not.toBeNull();
    await expect(
      getPublicPage.handler({ slug: "privacy" }, { actor: "system:test" }),
    ).resolves.toEqual(expect.objectContaining({ title: expected!.title }));
  });

  /** Contract pin (passes against the pre-fix code by construction: an in-set
   *  locale was already echoed verbatim). It guards the NEXT edit to the clamp. */
  whenMultiLocale("synthesises the legal fallback in the locale that was asked for", async () => {
    mocks.findFirst.mockResolvedValue(null);
    const { getDefaultLegalContent } = await import("@/lib/legal/default-content");
    const { getPublicPage } = await import("@/lib/tools/site");
    const expected = getDefaultLegalContent("privacy", SECONDARY!);
    expect(expected).not.toBeNull();
    await expect(
      getPublicPage.handler({ slug: "privacy", locale: SECONDARY }, { actor: "system:test" }),
    ).resolves.toEqual(expect.objectContaining({ title: expected!.title }));
  });
