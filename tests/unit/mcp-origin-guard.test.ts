...
function publicMcpRequest(
  ...
  if (init.origin != null) headers.origin = init.origin;
  return new NextRequest("https://public.example/api/mcp", {
    method,
    headers,
    ...(method === "GET" || method === "DELETE" ? {} : { body: "{}" }),
  });
...
describe("/api/mcp — the public-https tier (2026-08-24 policy)", () => {
  // On a genuinely public https deployment the cookie-less anonymous surface
  // has nothing a foreign web origin can abuse (no ambient credentials, no
  // CORS read access), and TLS makes the rebinding case unreachable — so a
  // well-formed foreign https origin passes. Everything the strict tier
  // rejects for structural reasons (http origins, opaque origins without
  // their entry) stays rejected here too.
  it("accepts a foreign https origin — this is the scanner/web-client case", async () => {
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(publicMcpRequest({ origin: "https://is-agentic.com" }));

    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({ transport: "streamable-http" });
  });

  it("still rejects a foreign http origin — no downgrade rides the tier", async () => {
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(publicMcpRequest({ origin: "http://evil.example" }));

    expect(res.status).toBe(403);
  });

  it("still rejects an unlisted opaque origin — extensions keep needing their entry", async () => {
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(
      publicMcpRequest({ origin: "chrome-extension://abcdefghijklmnopabcdefghijklmnop" }),
    );

    expect(res.status).toBe(403);
  });

  it("a LOCAL request (http://localhost) keeps the strict allowlist — the rebinding posture", async () => {
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(mcpRequest({ origin: "https://is-agentic.com" }));

    expect(res.status).toBe(403);
  });
});

describe("isPublicHttpsRequest — the deployment classifier", () => {
  it("classifies by x-forwarded-proto first (the TLS-terminating platform's word)", () => {
    const req = new NextRequest("http://internal-hop.example/api/mcp", {
      headers: { "x-forwarded-proto": "https" },
    });
    expect(isPublicHttpsRequest(req)).toBe(true);
  });

  it("https to LOOPBACK is not public — local tooling keeps the strict tier", () => {
    expect(isPublicHttpsRequest(new NextRequest("https://localhost/api/mcp"))).toBe(false);
  });

  it("plain http is never public", () => {
    expect(isPublicHttpsRequest(new NextRequest("http://shop.example/api/mcp"))).toBe(false);
  });
});
