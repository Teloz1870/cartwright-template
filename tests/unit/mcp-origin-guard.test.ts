...
describe("mcpAllowedOrigins — what feeds the allowlist", () => {
  ...
  it("drops an entry whose origin is OPAQUE, so it cannot admit every opaque origin", () => {
...
describe("OPTIONS /api/mcp — the verb Next used to answer on its own", () => {
  it("mcpPublic OFF → the same 404 as every other verb, and no Allow header", async () => {
    ...
    expect(res.status).toBe(404);
    expect(await res.json()).toEqual({ error: "not_found" });
    // The leak was the header, not the status: an `Allow` here would still
    // enumerate the surface a 404 is claiming not to have.
