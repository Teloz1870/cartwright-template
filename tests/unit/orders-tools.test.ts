...
beforeEach(() => {
  ...
  mocks.prisma.orderNote.create.mockReset().mockResolvedValue({ id: "n1" });
...
describe("orders.update_status — a refund status never claims money moved (DRØM9-a)", () => {
  it.each([
    ["paid", "refunded"],
    ["disputed", "refunded"],
    ["completed", "partial_refund"],
  ] as const)("%s → %s without manual+confirm is refused before any read or write", async (from, to) => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: from, totalDkk: 10000 });
    const { updateOrderStatus } = await import("@/lib/tools/orders");

    await expect(
      updateOrderStatus.handler({ orderId: "o1", status: to }, ctx),
    ).rejects.toThrow(/^refund_requires_payment_path:/);
    expect(mocks.prisma.order.update).not.toHaveBeenCalled();
  ...
  it.each([
    [{ status: "refunded" }, /^refund_requires_payment_path:/],
    [{ status: "refunded", manual: true }, /^refund_requires_payment_path:/],
    [{ status: "refunded", confirm: true }, /^refund_requires_payment_path:/],
    [{ status: "refunded", manual: false, confirm: true }, /^refund_requires_payment_path:/],
    [{ status: "partial_refund", manual: true, confirm: true }, /needs refundDkk/],
    [{ status: "refunded", manual: true, confirm: true, refundDkk: 500 }, /only accepted with status partial_refund/],
    [{ status: "shipped", refundDkk: 500 }, /only accepted with status partial_refund/],
  ] as const)("the schema refuses %o", async (extra, message) => {
    const { updateOrderStatus } = await import("@/lib/tools/orders");
    const parsed = updateOrderStatus.input.safeParse({ orderId: "o1", ...extra });
  ...
  it.each([
    [{ status: "refunded", manual: true, confirm: false }, /^refund_requires_payment_path:/],
    [{ status: "partial_refund", manual: true, confirm: true, refundDkk: 0 }, /positive whole number/],
  ] as const)("the schema answers with the guard's own message for %o", async (extra, message) => {
    const { updateOrderStatus } = await import("@/lib/tools/orders");
    const parsed = updateOrderStatus.input.safeParse({ orderId: "o1", ...extra });
    expect(parsed.success).toBe(false);
    if (parsed.success) return;
    expect(parsed.error.issues.map((i) => i.message).join(" | ")).toMatch(message);
  });

  it.each([0, -500, 12.5])(
    "the handler itself refuses refundDkk %s, even for a caller that skips the schema",
    async (amount) => {
      mocks.prisma.order.findUnique.mockResolvedValue({ status: "shipped", totalDkk: 10000 });
      const { updateOrderStatus } = await import("@/lib/tools/orders");

      await expect(
        updateOrderStatus.handler(
          { orderId: "o1", status: "partial_refund", manual: true, confirm: true, refundDkk: amount },
          ctx,
        ),
  ...
  it.each([
    ["disputed", "refunded", undefined],
    ["completed", "partial_refund", 2500],
  ] as const)("a manual %s → %s is still a legal move", async (from, to, refundDkk) => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: from, totalDkk: 10000 });
    mocks.prisma.order.update.mockResolvedValue({ id: "o1", status: to });
    const { updateOrderStatus } = await import("@/lib/tools/orders");
      ...
      updateOrderStatus.handler(
        {
          ...
          manual: true,
          confirm: true,
          ...(refundDkk === undefined ? {} : { refundDkk }),
        },
        ctx,
      ),
    ).resolves.toEqual({ id: "o1", status: to, moneyMoved: false });
  });

  it("the schema still accepts ordinary moves and a complete manual registration", async () => {
    const { updateOrderStatus } = await import("@/lib/tools/orders");
    for (const body of [
      { orderId: "o1", status: "shipped" },
      // as on main: an ordinary move ignores confirm/manual whatever their value
      { orderId: "o1", status: "shipped", confirm: false },
      { orderId: "o1", status: "shipped", confirm: true, manual: false },
      { orderId: "o1", status: "refunded", manual: true, confirm: true },
      { orderId: "o1", status: "partial_refund", manual: true, confirm: true, refundDkk: 2500 },
    ]) {
      expect(updateOrderStatus.input.safeParse(body).success).toBe(true);
    }
  });

  it("a manual refunded registration writes the status AND a no-money note, and says moneyMoved: false", async () => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: "paid", totalDkk: 10000 });
    mocks.prisma.order.update.mockResolvedValue({ id: "o1", status: "refunded" });
    const { updateOrderStatus } = await import("@/lib/tools/orders");

    const r = await updateOrderStatus.handler(
      { orderId: "o1", status: "refunded", manual: true, confirm: true },
      ctx,
    );

    expect(r).toEqual({ id: "o1", status: "refunded", moneyMoved: false });
    expect(updateOrderStatus.output.safeParse(r).success).toBe(true);
    ...
    const note = mocks.prisma.orderNote.create.mock.calls[0][0].data;
    expect(note).toMatchObject({ orderId: "o1", type: "system", author: "apikey:k1" });
    expect(note.body).toMatch(/^Manual registration — no money moved: status set to refunded\.$/);
    expect(JSON.parse(note.metaJson)).toEqual({ manual: true, moneyMoved: false });
    // Still audited, with the caller's own words.
    expect(mocks.captured.meta!.args).toEqual({
      orderId: "o1",
      status: "refunded",
      manual: true,
      confirm: true,
  ...
  it("a manual partial_refund records the amount in the note", async () => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: "shipped", totalDkk: 10000 });
    mocks.prisma.order.update.mockResolvedValue({ id: "o1", status: "partial_refund" });
    const { updateOrderStatus } = await import("@/lib/tools/orders");
    ...
    expect(r).toEqual({ id: "o1", status: "partial_refund", moneyMoved: false });
    const note = mocks.prisma.orderNote.create.mock.calls[0][0].data;
    expect(note.body).toContain("2500 øre refunded outside the shop");
    expect(JSON.parse(note.metaJson)).toEqual({ manual: true, moneyMoved: false, refundDkk: 2500 });
  });

  it.each([10000, 12000])("a manual partial_refund of %i øre on a 10000 øre order is refused", async (amount) => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: "shipped", totalDkk: 10000 });
    const { updateOrderStatus } = await import("@/lib/tools/orders");
    ...
    ).rejects.toThrow(/must be below the order total/);
    expect(mocks.prisma.order.update).not.toHaveBeenCalled();
    expect(mocks.prisma.orderNote.create).not.toHaveBeenCalled();
  });

  it("the state machine still applies to a manual registration", async () => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: "pending_payment", totalDkk: 10000 });
    const { updateOrderStatus } = await import("@/lib/tools/orders");
  ...
  it("an ordinary move writes no note and adds no moneyMoved field", async () => {
    mocks.prisma.order.findUnique.mockResolvedValue({ status: "paid", totalDkk: 10000 });
    mocks.prisma.order.update.mockResolvedValue({ id: "o1", status: "shipped" });
    const { updateOrderStatus } = await import("@/lib/tools/orders");

    const r = await updateOrderStatus.handler({ orderId: "o1", status: "shipped" }, ctx);
