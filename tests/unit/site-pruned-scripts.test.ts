...
describe("the site-pruned script list", () => {
  ...
  it("names only scripts that exist", () => {
    // ENGINE ONLY, and the discriminator is exact: `.cartwright/profile.json`
    // exists in a scaffold and never in this repo. A pruned scaffold has
    // ALREADY removed some of these files — that is what the list is for — so
    // asserting their presence there fails a customer for doing exactly what
    // the profile promised. Measured: `light` prunes
    // `scripts/publish-agent-card.ts`, and the release gate went red on it.
    //
    // The guard still bites where it must: the engine has every file, and CI
    // runs there, so a stale entry is caught before it can ship.
    if (existsSync(`${repoRoot}.cartwright/profile.json`)) return;
