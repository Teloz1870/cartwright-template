 ...
 *     Since the admin-api-guard consistency refactor this is the ONLY accepted
 *     session guard: `requireAdmin()` (307 page redirect) and hand-rolled
 *     `auth()` + role checks are deliberately NOT accepted anymore, so a new
 *     route can't drift back to redirect-on-API or a subtly wrong inline check.
...
/**
 * Patterns that satisfy "this handler is admin-guarded": the canonical call
 * AND the canonical bail-out. Requiring both means a route can't call the
 * guard and then ignore the 401 Response it returned.
 */
const SESSION_GUARD_PATTERNS: RegExp[] = [
  /requireAdminApi\s*\(/,
  /instanceof\s+Response/,
];
...
describe("admin-API auth guard (no unauthenticated /api/admin route can exist)", () => {
  ...
  for (const routeFile of routeFiles) {
    ...
    it(`guards every handler: ${rel}`, () => {
      ...
      const hasSessionGuard = SESSION_GUARD_PATTERNS.every((re) =>
        re.test(combined),
      );
