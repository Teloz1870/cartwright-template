...
describe("/api/mcp — HTTP verbs", () => {
  it("rejects anonymous JSON-RPC batches before transport so one token cannot invoke many tools", async () => {
    ...
    // JSON-RPC in, JSON-RPC out. This used to answer application/problem+json,
    // which a conformant client cannot parse as a protocol response — it reads
    // as a broken server rather than a refused request. -32600 (Invalid
    // Request) is the right code: MCP removed batching in 2025-06-18, so this
    // is a statement about the request. The machine-readable code moved to
    // `error.data` and is still asserted below.
    ...
    await expect(res.json()).resolves.toMatchObject({
      ...
      error: {
        code: -32600,
        data: { code: "anonymous_mcp_batch_not_allowed" },
      },
    });
  ...
  it("answers a batched handshake with something a JSON-RPC client can parse", async () => {
    // The MEASURED request. A scanner performing a handshake in one round trip
    // sends exactly this, and demo.cartwright.app answered
    // `400 {"type":"https://cartwright.app/problems/anonymous_mcp_batch_not_allowed"}`
    // — a document with no `jsonrpc`, no `id` and no `error`, so a client that
    // speaks only JSON-RPC has nothing to read. Refusing the batch stays
    // correct; being unparseable does not.
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(
      new NextRequest("http://localhost:3000/api/mcp", {
        method: "POST",
    ...
    // The three fields a JSON-RPC client reads before anything else.
    expect(body.jsonrpc).toBe("2.0");
    expect(body).toHaveProperty("id");
    expect(body).toHaveProperty("error");
    expect((body.error as { code: number }).code).toBe(-32600);
    // And the guard still held — no tool ran.
    expect(registryMock.invokeTool).not.toHaveBeenCalled();
  });
