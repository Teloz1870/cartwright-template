...
describe("profiles — the additive superset contract", () => {
  ...
  it("google-fonts is listed by every database-backed profile and by none below — the superset test alone would let it fall out silently (E2 falsifier nit)", () => {
    for (const name of ["managed-site", "commerce", "agentic"] as const) {
      expect(getProfile(name)!.modules, `${name} must list google-fonts`).toContain("google-fonts");
    }
    expect(getProfile("site")!.modules).not.toContain("google-fonts");
  });
