...
describe("proxy.static.ts (the site profile's middleware) hands the asset prefixes past the locale rewrite", () => {
  /**
   * The `site` profile materializes `proxy.static.ts` over `proxy.ts`. It
   * used to exempt only `/api`, so `/og?title=…` — the URL `lib/og.ts:pageOg()`
   * writes into every page's `og:image` — 307'd to `/en/og` and 404'd on
   * every site scaffold (A2 docs falsifier, real 2.9.2 scaffold). Same witness
   * as above: reaching `intlSpy` is the bug.
   */
  const req = (pathname: string) =>
    ({
      nextUrl: Object.assign(new URL(`https://site.example${pathname}`), { search: "" }),
      headers: new Headers(),
      method: "GET",
    }) as never;

  let proxy: (req: never) => Response;

  beforeEach(async () => {
    intlSpy.mockClear();
    proxy = (await import("@/proxy.static")).default as unknown as typeof proxy;
  });

  it.each(["/og", "/icon", "/api/inquiries"])("%s never reaches the locale rewrite", async (pathname) => {
    const res = await proxy(req(pathname));
    expect(intlSpy).not.toHaveBeenCalled();
    expect(res.status).toBe(200);
    expect(res.headers.get("location")).toBeNull();
  ...
  it("the reported failure — /og?title=hello, query and all — is served as written", async () => {
    // The real request shape: `nextUrl.search` carries the query; `pathname` is
    // what the exemption reads. The 2.9.2 scaffold answered 307 /en/og?title=hello.
    const res = await proxy({
      nextUrl: new URL("https://site.example/og?title=hello&description=x"),
      headers: new Headers(),
      method: "GET",
    } as never);
    expect(intlSpy).not.toHaveBeenCalled();
  ...
  it.each(["/", "/contact", "/en/contact", "/ogle"])("%s still goes through the locale rewrite", async (pathname) => {
    await proxy(req(pathname));
    expect(intlSpy).toHaveBeenCalledTimes(1);
  });
});
