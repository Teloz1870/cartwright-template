...
describe("the AI assistant follows the page locale", () => {
  ...
  it("binds `en` inside ToolResultRenderer, not only in its siblings", () => {
    ...
    // Either binding form counts, but the locale must come from the request:
    // `const en = useLocale() === "en"` or, once the component also needs the
    // locale itself for its hrefs, `const locale = useLocale()` + `en = locale
    // === "en"` (the shape its three sibling components already use).
    const direct = /const en = useLocale\(\) === "en"/.test(body);
    const viaLocale =
      /const locale = useLocale\(\)/.test(body) && /const en = locale === "en"/.test(body);
    expect(direct || viaLocale, "`en` is not bound from the request locale").toBe(true);
