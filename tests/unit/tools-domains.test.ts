...
describe("Tools — domain coverage", () => {
  ...
  it("orders.update_status afviser ugyldige statuser", async () => {
    const r = await invokeTool(
      ...
      { orderId: "x", status: "not_a_status" }, // findes ikke i status-enum'et
      CTX,
      ["orders:write"],
    );
  ...
  // DRØM9-a: an orders:write key (REST/MCP) must not mark an order refunded
  // without moving money. Refused at validation — before the handler touches
  // the DB, which this suite has none of.
  it.each([
    { orderId: "x", status: "refunded" },
    { orderId: "x", status: "partial_refund" },
    { orderId: "x", status: "refunded", confirm: true },
    { orderId: "x", status: "refunded", manual: true },
    { orderId: "x", status: "partial_refund", manual: true, confirm: true },
  ])("orders.update_status refuses a refund status that moves no money (%o)", async (args) => {
    const r = await invokeTool("orders.update_status", args, CTX, ["orders:write"]);
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.status).toBe(422);
    if (!("manual" in args && "confirm" in args)) {
      expect(r.error).toMatch(/^refund_requires_payment_path:/);
    } else {
      expect(r.error).toMatch(/refundDkk/);
    }
  });
