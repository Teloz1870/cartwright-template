...
const mocks = vi.hoisted(() => ({
  ...
  marketingAutomationsEnabled: vi.fn(() => false),
...
vi.mock("@/lib/marketing/automations", () => ({
  marketingAutomationsEnabled: mocks.marketingAutomationsEnabled,
...
function reset() {
  ...
  mocks.marketingAutomationsEnabled.mockReset().mockReturnValue(false);
  mocks.emitMarketingEvent.mockReset().mockResolvedValue(true);
}
...
describe("runAbandonedCartJob", () => {
  ...
  it("emitter cart.abandoned + springer direkte send over når marketingAutomations er on", async () => {
    mocks.marketingAutomationsEnabled.mockReturnValue(true);
    mocks.emitMarketingEvent.mockResolvedValue(true);
    mocks.prisma.cart.findMany.mockResolvedValue([
      {
        id: "c1",
        user: { email: "a@b.dk", name: "Kim" },
    ...
    expect(mocks.emitMarketingEvent.mock.calls[0][0]).toBe(
      "cartwright.cart.abandoned",
    );
    expect(mocks.emitMarketingEvent.mock.calls[0][1]).toBe("a@b.dk");
    // INGEN direkte single-send når automations kører
    expect(mocks.sendAbandonedCartEmail).not.toHaveBeenCalled();
    expect(mocks.prisma.abandonedCartLog.create).toHaveBeenCalledWith({
      data: { cartId: "c1", emailMessageId: "resend-automation" },
    });
  });

  it("logger skipped-no-consent når emit blokeres (consent), uden direkte send", async () => {
    mocks.marketingAutomationsEnabled.mockReturnValue(true);
    mocks.emitMarketingEvent.mockResolvedValue(false);
    mocks.prisma.cart.findMany.mockResolvedValue([
      {
        id: "c1",
        user: { email: "a@b.dk", name: "Kim" },
        items: [{ quantity: 1, product: { name: "Kaffe", priceDkk: 12500 } }],
      },
    ]);
    const { runAbandonedCartJob } = await import("@/lib/abandoned-cart");
    const run = await runAbandonedCartJob({});
    ...
    expect(mocks.sendAbandonedCartEmail).not.toHaveBeenCalled();
    expect(mocks.prisma.abandonedCartLog.create).toHaveBeenCalledWith({
      data: { cartId: "c1", emailMessageId: "skipped-no-consent" },
    });
  });
});
