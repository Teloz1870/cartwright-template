...
describe("discounts.create — input schema bounds", () => {
  ...
  it("pins the min(3)-BEFORE-trim order: whitespace padding buys past the length check", async () => {
    const { createDiscount } = await loadTools();
    // Zod runs `.min(3)` on the RAW string and only then applies the
    // `.trim().toUpperCase()` transform, so a padded short code slips through
    // and is stored 2 chars long. "ab" is rejected; "  ab  " is not. This test
    // pins the CURRENT behaviour rather than asserting it is desirable — see
    // the propose-only backlog item; tightening it is a source change.
    expect(() =>
      createDiscount.input.parse({ code: "ab", type: "percent", value: 10 }),
    ).toThrow();
    expect(
      createDiscount.input.parse({ code: "  ab  ", type: "percent", value: 10 }).code,
    ).toBe("AB");
  });
});
