...
describe("compose tools — registry contract", () => {
  ...
  it("composition.export is a read-only settings:read tool (no confirm)", () => {
    const t = getTool("composition.export");
    expect(t).toBeDefined();
    expect(t?.scope).toBe("settings:read");
    expect(t?.revertible ?? false).toBe(false);
  });
});
...
describe("compose tools — the moat (allowlists)", () => {
  it("are exposed to the admin / build agent", () => {
    ...
    // …and read the current look out as a composition artifact
    expect(ADMIN_TOOL_ALLOWLIST as readonly string[]).toContain("composition.export");
  });
...
describe("compose tools — validation + scope enforcement", () => {
  ...
  it("chrome.set requires confirm: true (422)", async () => {
    const r = await invokeTool("chrome.set", { headerKey: "minimal-header" }, CTX, [
      ...ADMIN_SCOPES,
    ]);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.status).toBe(422);
  ...
  it("chrome.set rejects an unknown chrome key (handler throws → 500)", async () => {
    const r = await invokeTool(
      "chrome.set",
      { headerKey: "definitely-not-a-chrome", confirm: true },
      CTX,
      [...ADMIN_SCOPES],
    );
    expect(r.ok).toBe(false);
  ...
  it("composition.apply requires confirm: true (422)", async () => {
    const r = await invokeTool(
      "composition.apply",
      { composition: { schema: "cartwright-composition-v1", name: "X", skin: "studio" } },
      CTX,
      [...ADMIN_SCOPES],
    );
    expect(r.ok).toBe(false);
  ...
  it("composition.apply rejects an invalid composition at the input schema (422)", async () => {
    const r = await invokeTool(
      "composition.apply",
      {
        composition: { schema: "cartwright-composition-v1", name: "X", skin: "not-a-design" },
        confirm: true,
      },
      CTX,
      [...ADMIN_SCOPES],
  ...
  it("composition.apply enforces settings:write scope (403 without it)", async () => {
    const r = await invokeTool(
      "composition.apply",
      {
        composition: { schema: "cartwright-composition-v1", name: "X", skin: "studio" },
  ...
  it("composition.export enforces settings:read scope (403 without it)", async () => {
    const r = await invokeTool("composition.export", {}, CTX, []);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.status).toBe(403);
  });
});
