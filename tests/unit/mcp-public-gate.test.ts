...
/**
 * Streamable-HTTP conformance behaviours added for the WebMCP-challenge sweep:
 *
 *  1. A GET that explicitly asks for `text/event-stream` is a client opening
 *     the server-initiated SSE stream. The spec's contract is binary — serve
 *     the stream or answer 405. This stateless endpoint offers no server
 *     push, and answering the human-friendly JSON intro instead read as a
 *     broken handshake to streaming clients (measured against a public
 *     scanner 2026-08-27). Browsers (Accept lists text/html) keep the intro.
 *  2. `tools/call` on a name the registry has NEVER known is a protocol-level
 *     error (-32602), not a tool RESULT with `isError` — clients branch on
 *     `error.code`. Known-but-unauthorized tools still flow to the scope
 *     layer's structured error, so the moat's 403 shape is untouched.
 */
describe("/api/mcp — streamable-HTTP conformance", () => {
  it("GET with an explicit SSE Accept → 405 + Allow: POST (no stream offered)", async () => {
    featuresWith(true);
    const { GET } = await import("@/app/api/mcp/route");
    const res = await GET(
      new NextRequest("http://localhost:3000/api/mcp", {
  ...
  it("GET declining SSE via q=0 keeps the JSON intro; case-insensitive SSE gets 405", async () => {
    featuresWith(true);
    const { GET } = await import("@/app/api/mcp/route");
    // Explicitly DECLINING the stream (q=0) is not a stream request.
    ...
    // Header values are case-insensitive — mixed case still means SSE.
    const mixedCase = await GET(
      new NextRequest("http://localhost:3000/api/mcp", {
        headers: { accept: "Text/Event-Stream" },
      }),
    );
    expect(mixedCase.status).toBe(405);
  });

  it("POST tools/call NOTIFICATION (no id) for an unknown tool flows to the transport", async () => {
    featuresWith(true);
    registryMock.listTools.mockReturnValue([
      { name: "products.search", description: "d", scope: "catalog:read" },
    ]);
    const { POST } = await import("@/app/api/mcp/route");
    const res = await POST(
      new NextRequest("http://localhost:3000/api/mcp", {
    ...
    // Notifications never get a response body — the shim steps aside so the
    // transport can answer its spec-required 202 (stubbed here).
    expect(await res.text()).toBe("mcp-transport");
  });

  it("GET with a browser Accept (text/html included) keeps the JSON intro", async () => {
    featuresWith(true);
    const { GET } = await import("@/app/api/mcp/route");
    const res = await GET(
      new NextRequest("http://localhost:3000/api/mcp", {
        headers: {
          accept: "text/html,application/xhtml+xml,text/event-stream;q=0.1",
  ...
  it("POST tools/call on an UNKNOWN tool → JSON-RPC -32602, transport untouched", async () => {
    featuresWith(true);
    registryMock.listTools.mockReturnValue([
      { name: "products.search", description: "d", scope: "catalog:read" },
    ]);
    ...
    expect(body.error.message).toContain("no.such.tool");
    // Anonymous rate-limit metadata survives the short-circuit.
    expect(res.headers.get("ratelimit-limit")).toBe("60");
  });

  it("POST tools/call on a KNOWN tool still reaches the transport", async () => {
    featuresWith(true);
    registryMock.listTools.mockReturnValue([
      { name: "products.search", description: "d", scope: "catalog:read" },
    ]);
    ...
    const res = await POST(
      new NextRequest("http://localhost:3000/api/mcp", {
        ...
        body: JSON.stringify({
          ...
          id: 8,
          method: "tools/call",
          params: { name: "products.search", arguments: {} },
        }),
