...
describe("PlpWebMcpTools — the catalogue page teaches the agent its filters", () => {
  ...
  it("filter_products carries the live category enum in its schema and builds a locale-prefixed URL", async () => {
    ...
    const schema = t.inputSchema as {
      properties: { kategori: { enum?: string[] }; sort: { enum?: string[] } };
    };
    expect(schema.properties.kategori.enum).toEqual(["beans", "espresso"]);
    expect(schema.properties.sort.enum).toEqual(["nyeste", "pris-op", "pris-ned"]);
  ...
  it("rejects an unknown category and a bad sort BEFORE navigating", async () => {
    await mount([beans]);
    const t = tool("filter_products");
    const bad = (await t.execute({ kategori: "gear" })) as Record<string, unknown>;
    ...
    const badPrice = (await t.execute({ minPris: -5 })) as Record<string, unknown>;
    expect(badPrice.error).toContain("minPris");
