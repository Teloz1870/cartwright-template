...
const mocks = vi.hoisted(() => ({
  ...
  configDesignSlug: undefined as string | undefined,
...
vi.mock("@/brand.config", () => ({ brand: { features: mocks.features, get designSlug() { return mocks.configDesignSlug; } } }));
...
describe("shouldShowWelcomeCanvas — truth table", () => {
  ...
  it("config-level brand.designSlug set (the Blank Canvas path) → false", async () => {
    mocks.configDesignSlug = "blank";
    try {
      await expect(shouldShowWelcomeCanvas({ ...FRESH_SETTINGS }, null)).resolves.toBe(false);
    } finally {
      mocks.configDesignSlug = undefined;
    }
  });
...
describe("welcome canvas messages — locale parity", () => {
  ...
  it("core welcome copy is present (English-first wording)", () => {
    // Eyebrow is the "Powered by" lead-in above the Cartwright wordmark
    // (CartwrightLogo) on the canvas — the brand name now lives in the logo.
    expect(en.Welcome.eyebrow).toBe("Powered by");
