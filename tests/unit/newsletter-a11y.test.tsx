...
describe("NewsletterSignup a11y wiring", () => {
  ...
  // The name is asserted through the namespace-echo mock above, so this
  // still fails if the attribute is dropped OR if it starts reading a key from
  // the wrong namespace — and it no longer pins one language into the test.
  it("keeps its accessible name", () => {
    expect(html).toContain('aria-label="Storefront.newsletterEmailAria"');
  });

  /**
   * The submit label lives in a JSX EXPRESSION (`{pending ? "…" : t(…)}`), and
   * the source-level language guard in `shared-component-i18n.test.ts` cannot
   * cover this file's expressions: the literals it still holds are the
   * deliberately-English agent outcomes and the WebMCP tool descriptions.
   * Asserting the RENDER closes that gap without exempting anything — a
   * mutation putting "Sign up" back fails here.
   */
  it("labels its submit button from the catalogue", () => {
    expect(html).toContain(">Storefront.newsletterSubmit<");
  });
});
