...
describe("BrewWebMcpTools — the pack's own agent tool", () => {
  ...
  it("describes itself once, and only as what it is — a read-only calculation", async () => {
    // The description is the planner's routing signal. Pinned verbatim: the
    // ratios live on the `strength` parameter only (an external WebMCP audit
    // flagged them stated twice), and a read-only tool must never read as if
    // it touched the cart or placed an order.
    await mount();
    const tool = registrations[0].tool;
    expect(tool.description).toBe(
      "Calculate this roastery's recommended pour-over recipe from its own brewing guide: " +
        "cups of coffee → grams of ground coffee and grams of water. Pure, instant calculation.",
    );
    expect(tool.description).not.toMatch(/1:1[5-7]/);
    expect(tool.description).not.toMatch(/\b(order|cart|checkout|buy|purchase|pay)\w*/i);
    const strength = (tool.inputSchema.properties as Record<string, { description?: string }>).strength;
    expect(strength.description).toBe("strong = 1:15, balanced = 1:16, bright = 1:17. Defaults to balanced.");
  });
