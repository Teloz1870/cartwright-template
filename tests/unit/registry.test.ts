...
import { invokeTool, getTool, listTools, buildToolManifest } from "@/lib/tools/registry";
...
describe("registry — discovery", () => {
  ...
  it("buildToolManifest emits a NON-empty input JSON-Schema per tool (zod v4 native)", () => {
    // Regression guard: zod-to-json-schema@3 silently returned just
    // { $schema } for a Zod v4 schema, so `properties` was absent and a cold
    // agent (?schema=true / MCP tools/list) saw an empty param contract.
    const manifest = buildToolManifest();
    const search = manifest.find((m) => m.name === "products.search");
    expect(search).toBeDefined();
    const schema = search?.inputJsonSchema as Record<string, unknown>;
    expect(schema?.type).toBe("object");
    expect(schema?.properties).toBeTruthy();
    expect(Object.keys((schema?.properties ?? {}) as object).length).toBeGreaterThan(0);
    // Every tool that declares object args should expose them, none left blank.
    const blank = manifest.filter((m) => {
      const s = m.inputJsonSchema as Record<string, unknown> | undefined;
      return s?.type === "object" && Object.keys((s.properties ?? {}) as object).length === 0;
    });
