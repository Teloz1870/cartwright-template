...
describe("feature manifest ↔ brand.config sync", () => {
  ...
  // firstRunWelcome is deliberately NOT asserted to a fixed value here. A fresh
  // create-cartwright scaffold correctly ships it ON (the welcome canvas is the
  // intended first-run experience), while the engine/canaries keep it OFF. The
  // canary guarantee — "no welcome canvas on a live canary" — is enforced at the
  // deploy gate by a smoke test, not by this deployment-specific
  // brand.config value (which legitimately differs per scaffold).

  it("breadcrumbs er default-off (canary byte-identisk indtil tændt)", () => {
    expect(brand.features.breadcrumbs).toBe(false);
  });

  // webMcp asserts a CONSISTENCY invariant, not a fixed value — same reasoning as
  // firstRunWelcome above. The engine (and every website-mode canary) ships it off,
  // but create-cartwright deliberately scaffolds WEBSHOP templates with it ON
  // (cartwright-app#312: "WebMCP-native out of the box"), so a hardcoded `false`
  // fails in exactly the scaffolds that are configured correctly. What must hold
  // everywhere is the flag's declared precondition: WebMCP tools only exist on a
  // shop that sells. A deployment that must never expose agent tools asserts that
  // at its own deploy gate; this file only holds the config-consistency line.
  it("webMcp respekterer sin ecommerce-precondition (aldrig tændt uden en butik)", () => {
    if (brand.features.webMcp) {
      expect(brand.ecommerceEnabled).toBe(true);
    } else {
      expect(brand.features.webMcp).toBe(false);
    }
  });

  it("webMcp er runtime-tier MED ecommerce-precondition", () => {
    // runtime-tier = en merchant kan tænde den fra /admin/features uden
    // deploy, så "default-off" er en config-garanti, ikke en deploy-garanti.
    const d = FEATURE_MANIFEST.find((f) => f.key === "webMcp");
    expect(d).toBeDefined();
    expect(d?.tier).toBe("runtime");
    expect(d?.precondition).toEqual({ kind: "ecommerce" });
