...
/**
 * The lexical boost has to speak the same language as the haystacks it scores.
 *
 * Callers used to hand the ranker raw `name + brand + description`; they now
 * hand it `productHaystack(p)`, which is folded (æøå, diacritics) and split on
 * punctuation. The ranker went on lowercasing the raw query and splitting on
 * whitespace — so for exactly the queries the boost exists to serve, it
 * stopped firing: "pour-over" is one whitespace token against a haystack
 * reading "pour over", and "Café" lowercases to "café" against "cafe". A
 * product that matches the query LITERALLY could then rank below the limit.
 */
describe("hybridRankProducts — the lexical boost survives normalisation", () => {
  const normalisedHaystack = (raw: string) =>
    raw
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase()
  ...
  const rankTwo = async (query: string, matching: string, other: string) => {
    mocks.embedQuery.mockResolvedValue({ vector: [1, 0, 0], modelId: MODEL });
    // Identical vectors, so ONLY the lexical boost can separate them.
    mocks.prisma.productEmbedding.findMany.mockResolvedValue([
      { productId: "other", vectorJson: JSON.stringify([1, 0, 0]) },
      { productId: "match", vectorJson: JSON.stringify([1, 0, 0]) },
    ]);
    return hybridRankProducts(
      query,
      [
        { id: "other", haystack: normalisedHaystack(other) },
        { id: "match", haystack: normalisedHaystack(matching) },
      ],
      10,
    );
  };

  it("boosts a hyphenated query against its punctuation-split haystack", async () => {
    expect(await rankTwo("pour-over", "Hario V60 pour-over dripper", "Steel burr grinder")).toEqual([
      "match",
      "other",
    ]);
  });

  it("boosts an accented query against its folded haystack", async () => {
    expect(await rankTwo("Café", "Café Crème house blend", "Sparkling water")).toEqual([
      "match",
      "other",
    ]);
  });

  it("boosts a Danish query against its folded haystack", async () => {
    expect(await rankTwo("bønner", "Friske bønner fra Etiopien", "Kande i stål")).toEqual([
      "match",
      "other",
    ]);
  });

  it("ties without a boost, so the cases above prove the boost and not the order", async () => {
    // Anti-vacuity: with nothing to match, the two are indistinguishable and
    // the ranker returns them in candidate order — "other" first.
    expect(await rankTwo("zzz", "Hario V60 pour-over dripper", "Steel burr grinder")).toEqual([
      "other",
      "match",
    ]);
  });
