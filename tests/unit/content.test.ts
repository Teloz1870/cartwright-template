...
describe("renderContentBlocks", () => {
  ...
  it("behandler rå HTML som ren tekst (ingen HTML-fortolkning)", () => {
    // Importeret/ukendt indhold der indeholder HTML render'es som tekst-blokke,
    // aldrig som HTML — det er kontrakten der gør body XSS-safe.
    expect(renderContentBlocks("<script>alert(1)</script>")).toEqual([
