...
2. **Driver** — drop the libSQL adapter branch in `makePrismaClient()` for the
   Postgres case. Either let Prisma connect directly from `DATABASE_URL`, or use
   the `@prisma/adapter-pg` driver adapter. The Turso branch only fires when
   `TURSO_*` are set, so leaving `TURSO_*` unset and pointing `DATABASE_URL` at
   Postgres is the cleanest split.
...
- [ ] `datasource.provider = "postgresql"`, schema reviewed for SQLite→PG diffs.
- [ ] `makePrismaClient()` uses a Postgres connection (direct or `@prisma/adapter-pg`); `TURSO_*` left unset.
- [ ] App `DATABASE_URL` = Supavisor pooler (6543, transaction mode); migrations use direct (5432).
