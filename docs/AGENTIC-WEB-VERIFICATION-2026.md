...
| 6 | **WebMCP** (Chrome 149): "bringer MCP-agenter i browseren; annotér HTML-forms; `allow=\"tools\"` Permissions-Policy" | ✅ Ægte, men **umoden** | WebMCP er ægte: W3C Web Machine Learning CG **Draft** (5. juni 2026 — *ikke* en standard). Chrome 149 origin trial + `allow="tools"` iframe-policy er bekræftet. **Men: live-API'et er `document.modelContext.registerTool()` — `navigator.modelContext` er deprecated i Chrome 150.** Chrome-only, origin-trial, single-vendor. | **Bygget som default-off eksperiment** (`webMcp`-flag): delt plumbing i `lib/model-context.ts`, global registrar (`components/WebMcpRegistrar.tsx`), kontekstuelle per-side-tools (`components/webmcp/`), deklarative form-annotationer (SearchBox/kontakt/nyhedsbrev) og diagnose-siden `/webmcp-check`. Moat-testen (`tests/unit/webmcp-moat.test.ts`) holder de REGISTREREDE tools på kurv/katalog; form-annotationerne er det dokumenterede carve-out (menneskets egne formularer, human-confirmed, ingen bindings — navnene indgår i unikheds-checket). Stadig AF canary-mosaikken. |
...
| Modern Web Guidance + cartwright-guidance skills | ✅ Har | `.claude/skills/cartwright-guidance/SKILL.md` |
...
| WebMCP (in-browser agent-tools) | ✅ Har (default-off eksperiment, `webMcp`-flag) | `lib/model-context.ts`, `components/WebMcpRegistrar.tsx`, `components/webmcp/`, `types/webmcp-dom.d.ts`, `app/[locale]/webmcp-check/` |
