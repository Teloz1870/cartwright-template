...
- **Rate-limiting + logging** på `/oauth/register` mod registrerings-spam — **delvist på plads.**
  - *Rate-limiting:* `/oauth/*` fik sit eget sliding window i `proxy.ts` (samme mekanik som
    `/api/*`). **Inaktiv uden `UPSTASH_*`** — `proxy.ts` sætter `redis = null`, så et
    default-scaffold har ingen throttling overhovedet.
  - *Logging:* en **gennemført** registrering skriver en `AuditLog`-række (`tool:
    "ucp.register_client"`, aktør-præfiks `oauth-register:`, felterne trunkeret) med den
    registrerede `client_id` og hvad der blev bedt om. Én række pr. `OAuthClient`-række.
  - *STADIG ÅBENT — afviste forsøg logges IKKE.* Det er selve spam-signalet, men at logge
    dem er en ny **uautentificeret skrive-klasse**, og begge de nævnte grænser fejler åbent
    på et default-shop: limiteren er inaktiv uden `UPSTASH_*`, og
    `brand.policies.auditRetentionDays` er **`null` = behold for evigt** (`pruneAuditLog`
    sletter da intet). Kræver en altid-aktiv grænse (limiter eller et loft over
    fejl-rækker) før det kan shippes — ikke bare et logge-kald.
  - *Bemærk for enhver der aktiverer flaget:* sæt `auditRetentionDays`, ellers gemmes
    registranters `ip`/`userAgent` uden udløb.
