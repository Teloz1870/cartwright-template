...
| **Lead** | email | name, email, phone, company, message, data | slet hele rækken |
...
| **AuditLog** (registrering) | **ingen** — actor `oauth-register:anonymous` | ip, userAgent (trunkeret ved skrivning) | **ikke nåbar af DSAR-sletning** — se noten nedenfor |

Strategier: `null` (nullable), `redact` → `[slettet]`, `hash` (salted email-hash,
bevarer unikhed/linkage uden PII), `keep` (finansielt/lovligt).

> **Registrerings-rækker (`oauth-register:`) har intet subjekt-link, med vilje og med en pris.**
> RFC 7591-registrering på `/oauth/register` er uautentificeret pr. protokol: der findes ingen
> `userId` at nøgle rækken på, så `eraseUserData()`s `where: { actor: "user:<id>" }` kan
> **aldrig** matche dem — heller ikke i princippet. Registranten er typisk en agentic-platform
> (en juridisk person), ikke en shop-kunde, men `ip`/`userAgent` er stadig persondata.
> **Konsekvens:** den eneste grænse på disse rækker er `brand.policies.auditRetentionDays`,
> som er `null` (behold for evigt) som default. Aktiverer man `ucpIdentityLinking`, skal man
> sætte den. Rækkerne skrives kun for **gennemførte** registreringer (én pr. `OAuthClient`),
> så mængden er bundet af hvor mange klienter der faktisk blev oprettet.
