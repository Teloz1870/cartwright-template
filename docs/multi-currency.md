...
Update them manually (e.g. quarterly), or enable the shipped `fxAutoUpdate`
cron after configuring `CRON_SECRET`. Keep automatic refresh off for fixed-rate
demos so recorded and evaluated amounts remain reproducible.
...
2. Turn on `currencySwitcher` in `/admin/features` — a currency selector appears in the header
   (display only).
3. To also **charge** in the selected currency, turn on `multiCurrency` in
   `brand.config.ts` and redeploy (it `dependsOn` `currencySwitcher` and needs
   ≥2 currencies). It is deliberately not runtime-toggleable: charging
   identity must remain config-sovereign.
