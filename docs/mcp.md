...
Three neighbours of this gap are **not** closed. A verb the route handles for
nobody — `PUT`, `PATCH` — still gets Next's automatic `405`, which is its own
"something is mounted here" signal; and neither `/api/v1/tools*` nor
`/.well-known/mcp.json` exports an `OPTIONS`, so both still answer the
automatic `204` while their `GET`/`POST` return `404`. The `404` body is also
`{"error":"not_found"}` — JSON an absent route would not produce. "Turned off"
is therefore *near*-indistinguishable from "never existed", not identical to
it.
...
404 — near-indistinguishable from a site without the surface. On `/api/mcp`
that now holds for `OPTIONS` too (see the origin section for why that verb
needed an explicit handler); `/api/v1/tools*`, `/.well-known/mcp.json` and the
unhandled verbs still answer Next's automatic `204`/`405`. Note: `features.set`
