...
export const brand = {
  ...
  /**
   * Per-locale variants of the copy in THIS file, keyed by locale and then by
   * the dotted path of the field. Absent ⇒ every locale renders the base value,
   * which is exactly what a single-language shop wants.
   *
   * This exists because the strings here are the SHOP's, not the engine's:
   * `messages/{da,en}.json` cannot hold them, and the genome resolves one
   * string per key regardless of locale. A shop serving two languages had no
   * way to say its own assistant button in both.
   *
   * Example — a Danish shop that also serves /en:
   *   copyTranslations: {
   *     en: {
   *       "ai.assistantOpenText": "Ask the stylist",
   *       "ai.assistantLabel": "Stylist",
   *       "footer.tagline": "Hand-picked frames, shipped fast.",
   *     },
   *   },
   */
  copyTranslations: {} as Record<string, Record<string, string>>,
  ...
  policies: {
     ...
     * every other entry is relative to that base. Base-currency
    ...
    supportedCurrencies: {
      ...
      GBP: { rate: 0.115, label: "British Pound" },
      SEK: { rate: 1.5, label: "Swedish Krona" },
      NOK: { rate: 1.57, label: "Norwegian Krone" },
     ...
     */
    processors: [
      { name: "Turso (libSQL)", purpose: "Database / hosting of all shop data", data: "All customer and order data", dpa: true },
      { name: "Stripe", purpose: "Payments", data: "Name, email, address, amount", dpa: true },
      { name: "Resend", purpose: "Transactional email", data: "Email + order details", dpa: true },
      { name: "Anthropic", purpose: "AI assistant (chat + generation)", data: "Chat messages (anonymised session id)", dpa: false },
      { name: "Google (Gemini)", purpose: "Voice/vision AI (optional)", data: "Voice audio, product images", dpa: false },
      { name: "Vercel (v0 Platform API)", purpose: "AI code/design generation (admin-only, optional)", data: "Admin prompts + brand tokens (no customer PII)", dpa: false },
      { name: "Vercel Blob", purpose: "Media/file storage", data: "Uploaded images and video", dpa: true },
      { name: "Sentry", purpose: "Error monitoring", data: "PII-scrubbed error events", dpa: true },
      { name: "Google Analytics 4", purpose: "Analytics (consent-gated)", data: "Anonymised pageviews", dpa: false },
  ...
  website: {
    /**
     * Engine-transparency surfaces: the footer's "Audit feed" link and the
     * announcement bar's "see what I am doing today" link, both pointing at
     * /changelog (the ENGINE changelog rendered under the shop's own brand),
     * plus the route itself and its sitemap entry. True (default) keeps the
     * trusted-operation story visible; a shop that doesn't want engine
     * internals surfaced on its storefront sets false — the links disappear,
     * /changelog answers 404, and the sitemap entry is dropped.
     */
    showAuditFeed: true as boolean,
  ...
  footer: {
    ...
    /** Tom ⇒ storefronten renderer en lokaliseret "<storeName> · All rights
     *  reserved."-linje pr. locale. Sæt kun en værdi hvis du vil have PRÆCIS
     *  den streng på alle sprog (den oversættes ikke). */
    disclaimer: "",
    copyrightYear: 2026,
    /**
