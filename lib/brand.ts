...
export async function getBrand(): Promise<MergedBrand> {
  ...
  try {
    ...
    const merged = {
      ...
      ecommerceEnabled: row.ecommerceEnabled ?? true,
    ...
    const fallback = { ...brandDefaults, source: "fallback", ecommerceEnabled: true, defaultLocale: "da" } as unknown as MergedBrand;
    cache = { value: fallback, expiresAt: now + CACHE_TTL_MS };
    return fallback;
  }
}
