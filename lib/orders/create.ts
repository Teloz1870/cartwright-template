...
import { convertMinor, fxRate } from "@/lib/money";
...
export type CreateOrderResult =
  ...
  | {
      ...
      paymentMode: "stripe";
      /**
       * What the card is charged, in the currency it is charged in.
       *
       * Separate from `totalDkk` on purpose: that is the base-currency ledger
       * figure, and with multiCurrency on it is NOT the number Stripe sees.
       * The pay button used to render `totalDkk` through a base-currency
       * formatter, so the customer read one amount and was charged another.
       */
...
export async function createOrder(
  ...
  try {
    ...
    if (!cart || cart.items.length === 0) {
      return { ok: false, error: "The cart is empty", code: "EMPTY_CART" };
    }
    ...
    // Multi-currency: presentment currency the customer is charged in. Base
    // currency unless `multiCurrency` is on (see getCheckoutCurrency), so
    // default shops are unaffected. The order snapshots currency + fxRate;
    // totalDkk and the *Dkk columns stay the canonical base-currency ledger.
    const currency = await getCheckoutCurrency();
    await primeFxRatesFromDb();
    const orderFxRate = fxRate(currency);
    ...
    if (paymentMode === "stripe") {
      try {
        // The amount Stripe will actually charge, computed ONCE and reused for
        // both the intent and the number the pay button shows. They used to be
        // computed independently — the button through `formatPriceDkk`, which
        // always renders the BASE currency — so with multiCurrency on the
        // customer read one figure and was charged another.
        const chargeAmountMinor = convertMinor(totalDkk, currency);
        const intent = await createPaymentIntent({
          orderId,
          // Charge the customer in their presentment currency: convert the
          // base-currency total to that currency's minor units via the same
          // rate-table the order snapshotted. Base currency → unchanged.
        ...
        return {
          ...
          orderId,
          totalDkk,
          // What the card is charged, in the currency it is charged in. The
          // client must not re-derive this: `totalDkk` is the base-currency
          // ledger figure and is NOT what Stripe sees whenever the two differ.
          chargeAmountMinor,
        ...
        return {
          ok: false,
          error: "Could not initialize payment. Try again shortly.",
    ...
    try {
      ...
      console.error("[createOrder] could not send order confirmation", mailErr);
    }
    ...
    if (err instanceof Error && err.message.startsWith("OUT_OF_STOCK:")) {
      ...
      return {
        ok: false,
        error: `${productName} sold out while the order was being created. Try again or choose another item.`,
        code: "OUT_OF_STOCK",
      };
    }
...
async function restockOrder(orderId: string): Promise<void> {
  ...
  await prisma.$transaction(async (tx) => {
    // Delt variant-aware restock (lib/orders/restock.ts) — samme loop som
    // retur-flowet bruger. Idempotens her ankres af pending_payment-guarden
    // ovenfor (status sættes straks til cancelled).
    await restockLines(
      tx,
