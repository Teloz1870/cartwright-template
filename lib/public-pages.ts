...
export type PublicPageSummary = {
  ...
  updatedAt: Date;
  /**
   * Carried on the SUMMARY, not just the full record: every surface that
   * publishes a page title to a reader — llms.txt, the public `site.*` tools —
   * reads this list, and without the translation bag beside the base column
   * those surfaces can only ever publish the source language.
   */
...
function isMissingPageColumn(error: unknown, column: string): boolean {
  ...
  return (
    ...
    // SQLite names an UNQUALIFIED column exactly as the query wrote it, so a
    // raw `SELECT "translations" FROM "Page"` reports `no such column:
    // translations` with no table prefix. Every caller here queries the Page
    // table alone, so the unqualified form answers the same question. The
    // trailing `\\b` (not an end-of-line anchor) is what lets the driver wrap
    // the message — Prisma reports it as ``Message: `…no such column:
    // translations` `` — while still refusing `translations_old`.
    new RegExp(`no such column:\\s*["'\`]?${escaped}\\b`, "i").test(evidence)
  );
}
...
export async function listPublishedPageSummaries(): Promise<PublicPageSummary[]> {
  try {
    ...
    // SQLite blames the FIRST unresolvable column in the SELECT list, not the
    // one in the WHERE. Adding `translations` to the select therefore changed
    // which column a pre-draft database complains about — from `Page.status` to
    // `Page.translations` — and a status-only guard would have rethrown, killing
    // the whole legacy fallback this function has always provided.
    if (!isLegacyPageSchema(error) && !isMissingPageColumn(error, "translations")) {
      throw error;
    }
    try {
      return await prisma.$queryRaw<PublicPageSummary[]>`
        SELECT "slug", "title", "metaDescription", "updatedAt", "translations"
        FROM "Page"
        ORDER BY "slug" ASC
      `;
    } catch (retryError) {
      // `Page.translations` (2026-05-24) predates `Page.status` (2026-06-14), so
      // an installation old enough to need this fallback normally still has the
      // column. An even older one does not — and the fallback exists precisely
      // to keep such a database serving. Degrade to source text rather than
      // turning a graceful legacy path into a 500.
      if (!isMissingPageColumn(retryError, "translations")) throw retryError;
      const rows = await prisma.$queryRaw<Array<Omit<PublicPageSummary, "translations">>>`
        SELECT "slug", "title", "metaDescription", "updatedAt"
        FROM "Page"
        ORDER BY "slug" ASC
      `;
      return rows.map((row) => ({ ...row, translations: null }));
    }
  }
}
