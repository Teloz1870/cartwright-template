 ...
 *   oauth-register:anonymous — RFC 7591 dynamic client registration. Unauthenticated BY
 *                              PROTOCOL, so there is no subject to name; the ip/userAgent
 *                              columns carry what little is known about the caller.
...
export type AuditActor =
  ...
  | `annotation:${string}`         // admin in-place copy-edit på live storefront (/api/admin/annotate)
  | `oauth-register:${string}`     // RFC 7591 dynamic client registration — the caller is BY DEFINITION unauthenticated
