...
export const CompositionSchema = z
  ...
  .superRefine((comp, ctx) => {
    ...
    //
    // Deliberately the SLUG-SET answer, not a pack's own `mixable` override:
    // this validator is client-safe (see the file docblock) and a composition
    // is PORTABLE — it is validated on the shop that installs it, which need
    // not have the authoring shop's pack at all. The shop-local surfaces
    // (lib/theme.ts, chrome.set, /admin/designs) do hold the pack and pass its
    // field in, so the two can disagree in BOTH directions:
    //
    //  - Pack opts IN (`mixable: true` outside the slug set — the custom-pack
    //    case): the Part is selectable on the shop but dropped from an exported
    //    composition. export.ts leaves it unthreaded for exactly this reason,
    //    so what it emits always passes this validator.
    //  - Pack opts OUT (`mixable: false` on a slug that IS in the set): this
    //    validator accepts a neutral/foreign chrome that `applyComposition`
    //    then persists and `getActiveChromeConfig` immediately drops — the
    //    install reports the chrome, the shop renders its design's own. No
    //    shipped pack does this today, and the drop is the same fail-soft path
    //    a stale locked key already takes, but it IS a real divergence.
    //
    // Closing the second one means giving the server-side apply/export path the
    // loaded pack's effective mixability (or carrying it as client-safe
    // metadata) — a boundary decision, not a one-liner. Filed, not smuggled in.
    const checkChrome = (key: string | undefined, kind: ChromeKind) => {
      ...
      const rejection = explainChromeRejection(meta, comp.skin, { targetNoun: "skin" });
      if (rejection) {
        ctx.addIssue({
          code: "custom",
          path: ["chrome", `${kind}Key`],
