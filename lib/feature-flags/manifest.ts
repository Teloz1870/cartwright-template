...
const DESCRIPTORS: Record<FeatureKey, Omit<FeatureDescriptor, "key" | "runtimeToggleable">> = {
  ...
  breadcrumbs: {
    label: "Synlige breadcrumbs",
    description:
      "Hierarkisk breadcrumb-navigation på storefront-sider (kategori, alle-produkter, ydelser, blog) — viser stien fra forsiden til nuværende side. Afspejler den BreadcrumbList-JSON-LD siderne allerede emitter. Default off → byte-identisk indtil tændt.",
    group: "Storefront UX",
    tier: "runtime",
    implemented: true,
  },
  ...
  designSurfaces: {
    label: "Design-adaptive sider (kurv/checkout/konto …)",
    description:
  ...
  mcpPublic: {
    label: "Offentlig MCP",
    description:
      "Eksponér /api/mcp + /api/v1/tools offentligt (discovery-signal for AI-first shops). Slået fra ⇒ ruterne svarer 404 (håndhævet).",
    group: "Discovery & AI",
    tier: "runtime",
    implemented: true,
  },
  ...
  leadAiTriage: {
    label: "AI-triage af leads",
    description:
      "Prioritet, resumé og svarudkast på indkomne kontakthenvendelser, genereret EFTER svaret er sendt (after()) — aldrig på den besøgendes kritiske vej. Kræver en Anthropic-nøgle; uden den springes triagen stille over. Vises i /admin/leads.",
    group: "Discovery & AI",
    tier: "compile-time",
    requiresRedeployNote: REDEPLOY_NOTE,
    implemented: true,
  ...
  registryStats: {
    label: "Registry: install-statistik",
    description:
  ...
  speculationRules: {
    ...
    description:
      "Emit et document-level <script type=\"speculationrules\"> så Chromium prefetcher sandsynlige næste-navigationer (produkt/kurv/checkout, ekskl. /admin + /api + [data-no-prefetch]) ved `moderate` eagerness — hurtigere PLP→PDP→checkout uden at forlade server-modellen. Progressive enhancement: ignoreres i browsere uden support (Firefox/Safari), så ingen adfærdsændring dér. `type=\"speculationrules\"` er IKKE eksekverbar JS (ingen CSP-script-src-bekymring). Off ⇒ intet script emitteres ⇒ byte-identisk render (canary-safe).",
    group: "Modern web platform",
    tier: "compile-time",
    requiresRedeployNote: REDEPLOY_NOTE,
    implemented: true,
