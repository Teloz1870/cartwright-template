...
const DESCRIPTORS: Record<FeatureKey, Omit<FeatureDescriptor, "key" | "runtimeToggleable">> = {
  ...
  webMcp: {
    ...
    description:
      "Expose the storefront as browser-native WebMCP tools for in-browser AI agents (document.modelContext): site-wide search/cart reading/navigation plus CONTEXTUAL per-page tools — the PDP offers add-to-cart for its own product, the cart page edit/remove/go-to-checkout — plus declarative form annotations (search, contact, newsletter; only search may auto-submit). EXPERIMENTAL: Chrome-only origin trial, W3C draft — default-off, keep off the canary mosaic until the spec stabilizes.",
    group: "Discovery & AI",
    tier: "runtime",
    precondition: { kind: "ecommerce" },
    implemented: true,
