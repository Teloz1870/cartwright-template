...
export type FeatureStatus = FeatureDescriptor & {
  ...
  overridden: boolean;
  /** Dependency/precondition drift, regardless of whether the flag is on. */
...
export function computeFeatureStatuses(brand: MergedBrand): FeatureStatus[] {
  ...
  return FEATURE_MANIFEST.map((d) => {
    ...
    // Also evaluate an already-enabled flag. Otherwise a config/DB drift can
    // display a green "On" state even though its prerequisites are impossible.
    const blockedReason = d.implemented ? enableBlockReason(d, brand) : null;
    return {
      ...d,
      enabled,
      configDefault,
