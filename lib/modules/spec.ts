 ...
 * cuts a profile (`site` ⊂ `managed-site` ⊂ `commerce` ⊂ `agentic` — see
 * `internal-docs/site-profile-ultraplan.md`, owner-approved 2026-07-15).
 * The contract EXTENDS cartwright-plugin-v1: every field a plugin manifest
 * carries keeps its exact meaning here, and the 9 shipped plugins register as
 * modules with `kind: "plugin"` unchanged. What v1 adds is the graph — which
 * modules need which — plus the generation inputs (deps/env/prisma fragments)
 * the materializer assembles a scaffold from ADDITIVELY, replacing the
 * subtractive prune-lists in the CLI.
 *
