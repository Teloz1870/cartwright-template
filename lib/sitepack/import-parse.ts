...
async function sanitizeHtmlFields(value: unknown, keys: ReadonlySet<string>, depth = 0): Promise<unknown> {
  if (depth > MAX_DEPTH) {
    throw new Error("SitePack: content nesting exceeds the safe depth limit.");
  }
  if (Array.isArray(value)) {
    return Promise.all(value.map((v) => sanitizeHtmlFields(v, keys, depth + 1)));
  }
  if (value !== null && typeof value === "object") {
    ...
    for (const [k, v] of Object.entries(value as Row)) {
      ...
      // Sequential on purpose: sanitizeUserHtml shares one memoised jsdom
      // instance, and key order must stay stable for the re-validated output.
      out[k] = keys.has(k) && typeof v === "string"
        ? await sanitizeUserHtml(v)
        : await sanitizeHtmlFields(v, keys, depth + 1);
    }
    return out;
  }
  return value;
...
export async function sanitizeLayoutJson(raw: unknown): Promise<string | null> {
  if (typeof raw !== "string" || raw === "") return null;
  const layout = parsePageLayout(raw); // validates vs pageLayoutSchema; null if invalid/incompatible
  if (!layout) return null;
  const sanitized = await sanitizeHtmlFields(layout, LAYOUT_HTML_KEYS);
...
export async function sanitizeContentRow(row: Row): Promise<Row> {
  return (await sanitizeHtmlFields(row, VIBE_KEYS)) as Row;
}
...
export async function sanitizePageRow(row: Row): Promise<Row> {
  const out = await sanitizeContentRow(row);
  if ("layoutJson" in out) out.layoutJson = await sanitizeLayoutJson(out.layoutJson);
  return out;
}
...
export async function parseSitePackContent(entries: Map<string, Buffer>): Promise<SitePackContent> {
  ...
  const branding = jsonObj("content/branding.json");

  return {
    // Pages carry vibeHtml AND the builder layoutJson sink.
    pages: await Promise.all(nd("content/pages.ndjson").map(sanitizePageRow)),
    // The rest carry vibeHtml only (no layoutJson) → vibe pass.
    categories: await Promise.all(nd("content/categories.ndjson").map(sanitizeContentRow)),
    services: await Promise.all(nd("content/services.ndjson").map(sanitizeContentRow)),
    posts: await Promise.all(nd("content/posts.ndjson").map(sanitizeContentRow)),
    products: await Promise.all(nd("content/products.ndjson").map(sanitizeContentRow)),
    ...
    branding: branding ? await sanitizeContentRow(branding) : null,
