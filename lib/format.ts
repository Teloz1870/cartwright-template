...
export function formatPrice(
  ...
  // Konvertér via rate-table hvis target adskiller sig fra base.
  const rates = brand.policies.supportedCurrencies ?? {
    [baseCurrency]: { rate: 1, label: baseCurrency },
  };
  const rateEntry = rates[targetCurrency];
  const rate = rateEntry?.rate ?? 1;
  const majorTarget = (minorBase / 100) * rate;
