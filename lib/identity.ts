...
/**
 * One sentence explaining WHY a field is locked — and how to unlock it.
 *
 * The escape hatch is half the message. A lock that only says "you cannot edit
 * this here" is a dead end for anyone who WANTS to manage identity from the
 * admin, and that is a legitimate choice the policy already supports. Telling
 * them the field is owned elsewhere without telling them they can hand it back
 * would repeat the original sin in a friendlier font.
 */
export function identityLockNotice(
  policy: IdentityPolicy = identityPolicy(),
): string | null {
  return identityWritesLocked(policy)
    ? 'Owned by brand.config.ts (identitySovereignty: "config") — a value saved here would not render. ' +
        'Edit it there, or set identitySovereignty: "auto" to manage it from the admin instead.'
    : null;
}
