...
export async function chatModel() {
  const row = await prisma.integrationSettings.findUnique({
    where: { id: 1 },
    select: { aiProvider: true, localAiEndpoint: true, localAiModel: true }
  });

  // Delegate Agent Mechanism: Use local Ollama instance if selected
  if (row?.aiProvider === "local") {
    const localProvider = createOpenAI({
      baseURL: row.localAiEndpoint || "http://localhost:11434/v1",
      apiKey: "ollama", // Dummy key required by OpenAI SDK spec
    });
    return localProvider(row.localAiModel || "gemma:7b");
  }

  // Fallback to default Cloud AI (Anthropic)
  const apiKey = await getAnthropicApiKey();
  if (!apiKey) {
    throw new Error(
      "No Anthropic API key - set one in /admin/integrations or via ANTHROPIC_API_KEY in .env",
    );
  }
  
  // Cloudflare AI Gateway integration (Prompt Hardening)
