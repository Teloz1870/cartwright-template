 ...
 */
/**
 * True når mindst én AI-provider faktisk kan bruges right now (key/endpoint
 * konfigureret). Gater AL AI-UI — storefront-assistent-FAB, admin chat-launcher
 * og status-pill — så INTET AI-element floater på en frisk scaffold uden nøgle.
 * AI er deaktiveret fra start; UI'en dukker først op når provideren er sat op i
 * /admin/integrations (eller via .env). Læser den 30s-cachede getAiSettings, så
 * dette koster ikke et DB-kald pr. render.
 */
export async function isAiConfigured(): Promise<boolean> {
  const settings = await getAiSettings();
  return settings.anthropicConfigured || settings.localConfigured;
}
