...
/**
 * Discover a site's URLs via Firecrawl `/map` (sitemap + crawl-based discovery)
 * — the cheap first step before per-page scraping (the proven scrape.mjs flow).
 * Fail-soft: missing key / error → null. Returns de-duped absolute https URLs.
 */
export async function mapSite(
  ...
  const key = await getFirecrawlKey();
  if (!key) return null;
  // Clamp the discovery limit to a sane ceiling — the function is exported, so a
  // caller passing user input can't make Firecrawl attempt an unbounded crawl.
