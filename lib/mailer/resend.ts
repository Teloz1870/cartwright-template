...
export class ResendMailer implements Mailer {
  ...
  /**
   * Ordrestyring: send et færdig-renderet MailMessage (shipping-notifikation,
   * refund, retur). Templaten renderes af kalderen (lib/mailer.ts) så denne
   * metode er format-agnostisk.
   */
  async sendRaw(message: MailMessage): Promise<void> {
    const client = await this.getClient();
    if (!client) {
      throw new Error("Resend API-key not configured");
    }
...
/**
 * Er email-afsendelse overhovedet konfigureret? (Resend-key i DB eller env.)
 *
 * I modsætning til shouldUseResend() er denne **env-agnostisk** — den siger
 * "kan vi levere en mail til en rigtig indbakke?", ikke "skal vi bruge Resend
 * lige nu?". Bruges til UX-beslutninger: skal vi tilbyde "glemt adgangskode" /
 * magic-link, eller ville de bare ende i .mail-previews/ som en blindgyde?
 * Genbruger getResendApiKey()'s 30s-cache, så kald er billige.
 */
