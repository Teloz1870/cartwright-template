...
import { sectionEffectSchema } from "./effects";
...
const sectionNodeSchema = z.object({
  ...
  /**
   * Valgfri scroll-driven motion-effekt (PART 4). Whitelisted enum (lib/builder/
   * effects.ts) → en .motion-* klasse i themes/motion.css. Udeladt eller "none"
   * ⇒ ingen wrapper ⇒ byte-identisk render. Erstatter det tidligere ubrugte
   * `variant`-felt.
   */
