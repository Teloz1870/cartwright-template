...
export class ConfirmationLimitExceeded extends Error {
  constructor(message = "Too many open confirmations in the same session - wait for them to expire (5 min)") {
    super(message);
...
export function consumeConfirmation(args: {
  ...
  if (!entry) return { ok: false, reason: "Unknown or expired confirmation token" };

  // One-time-use: slet altid, uanset succes
  pending.delete(args.token);

  if (Date.now() - entry.createdAt > TTL_MS) {
    return { ok: false, reason: "Confirmation token has expired (5 min)" };
  }
  if (entry.ownerId !== args.ownerId) {
    return { ok: false, reason: "Token belongs to another session" };
  }
  if (entry.tool !== args.tool) {
    return { ok: false, reason: "Token was issued for another tool" };
  }
  if (entry.argsHash !== hashArgs(args.tool, args.toolArgs)) {
    return { ok: false, reason: "Arguments changed after confirmation was issued" };
  }
  return { ok: true };
}
