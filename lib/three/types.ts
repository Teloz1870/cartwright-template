...
/**
 * The launch scenes. The union itself lives in the dependency-free
 * `lib/three/scene-ids.ts` (B3: design packs must be able to reference scene
 * ids without pulling in the `three` dependency) — re-exported here so every
 * existing `import type { SceneId } from "@/lib/three/types"` keeps working.
 * Add a new scene = one union member there + one registry entry + one module.
 */
export type { SceneId } from "./scene-ids";
