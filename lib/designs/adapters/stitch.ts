  ...
  switch (kind) {
    ...
    case "value-props":
      return {
        ...
        description: s.description ? String(s.description) : undefined,
        items: ((s.items as Record<string, unknown>[]) ?? []).slice(0, 6).map((i) => ({
    ...
    case "process":
      return {
        ...
        items: ((s.items as Record<string, unknown>[]) ?? []).slice(0, 6).map((i, idx) => ({
    ...
    case "tech":
      return {
        ...
        items: ((s.items as unknown[]) ?? []).map(String).slice(0, 60),
      };
    case "cta":
    case "cta-footer":
    case "final-cta":
...
function stitchTargetToMode(_t?: string): "website" | "webshop" | "both" {
  // v0.9.4: default "both" så en imported design ALTID er synlig + valgbar i
  // /admin/designs uanset om shoppen er website- eller webshop-mode. Tidligere
  // gav web → "website", hvilket gjorde Stitch-imports usynlige på en webshop
  // (mode-filteret skjuler website-designs der) → "importeret men forsvundet".
  // "both"-designs renderer fint i begge modes; brugeren kan finjustere mode i
  // den genererede design.md + re-importe hvis de vil låse det.
  return "both";
}
