...
export async function scaffoldDesign(
  ...
  // v0.9.4: imported designs genbruger de delte Studio-section-atoms (cw-*) og
  // chrome (sol-*). For at deres palette faktisk renderes — i dev OG prod, uden
  // build-time @theme-registrering af en custom prefix — normaliserer vi prefix
  // til "sol" (en altid-registreret token-familie) og lader app/layout.tsx
  // mappe paletten via applyPaletteAsTheme. Det undgår den tidligere bug hvor
  // bg-<custom>-cream-utilities aldrig blev genereret → transparent/ulæselig.
  const spec: DesignMdSpec = {
    ...specIn,
    tokens: { ...specIn.tokens, prefix: "sol" },
  };
...
function renderSection(section: DesignSection): string {
  const indent = "      ";
  switch (section.type) {
    case "hero":
...
export const ${camelCase(spec.slug)}Design: DesignPack = {
  ...
  source: "design.md",
  // v0.9.4: imported design → palette mappes til sol-* + cw-* af app/layout.tsx
  // mens designet er aktivt (render-time, prod-safe). Se DesignPack-typen.
