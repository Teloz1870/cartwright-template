    ...
    components: {
      securitySchemes: {
        bearerAuth: {
          ...
          // OpenAPI only allows a `scopes` object on oauth2/openIdConnect
          // schemes, and inventing an OAuth flow we don't run would be a lie.
          // The named per-scope grants live in the spec-legal extension below
          // (name → human description) and in each operation's
          // x-cartwright-required-scope — granular access, honestly declared.
          description: `Cartwright API key with named per-scope grants (${supportedScopes.join(", ")}). Each operation declares its required scope in x-cartwright-required-scope; keys carry only the scopes the operator granted.`,
          "x-cartwright-scopes-supported": supportedScopes,
          "x-cartwright-scopes": Object.fromEntries(
            supportedScopes.map((scope) => {
              const [domain, verb] = scope.split(":");
              return [
                scope,
                `${verb === "read" ? "Read" : "Manage"} ${domain.replace(/-/g, " ")}`,
              ];
            }),
          ),
        },
