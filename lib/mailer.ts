...
export type OrderEmailData = {
  ...
  currency?: string;
  /**
   * The order's SNAPSHOTTED base→presentment rate (`Order.fxRate`), i.e. the rate
   * the customer was actually charged at. Passed so the receipt matches the charge
   * exactly even on a cold serverless webhook where the live FX-override cache is
   * unprimed (otherwise the email would fall back to static anchors → drift when
   * fxAutoUpdate is on). Omit for base-currency / non-multi-currency orders.
   */
  fxRate?: number;
};
...
export function renderOrderConfirmationHtml(data: OrderEmailData): string {
  ...
  // Use the order's SNAPSHOTTED fxRate as an explicit override so the receipt
  // matches the charge exactly — independent of the live FX-override cache, which
  // is unprimed in the webhook/cold-serverless context (else it would fall back
  // to static anchors and drift from the charged amount when fxAutoUpdate is on).
    ...
    formatPrice(
      ...
      data.currency
        ? {
            ...
            fxRateOverrides: data.fxRate
              ? { fetchedAt: "", rates: { [data.currency]: data.fxRate } }
              : undefined,
          }
        : {},
    );
