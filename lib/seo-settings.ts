...
export type AiCrawlers = "allow" | "block";
...
export async function getSeoSettings(): Promise<SeoSettings> {
  ...
  try {
    ...
    const value: SeoSettings = {
      ...
      aiCrawlers: row?.aiCrawlers === "block" ? "block" : "allow",
    };
...
export async function applySeoSettings(
  ...
  if (patch.aiCrawlers) data.aiCrawlers = patch.aiCrawlers === "block" ? "block" : "allow";
    ...
    await withAudit(
      ...
      async () => {
        await prisma.brandingSettings.upsert({
          where: { id: 1 },
          update: data,
          create: { id: 1, storeName: "Cartwright", heroImage: "", announcement: "", ...data },
        });
      },
    );
  } catch (err) {
