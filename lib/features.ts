...
/**
 * WebGL2 rendering context. Baseline ~2022 (all modern browsers). The
 * Cartwright Live Canvas (3D hero) requires this; without it we fall back
 * to the design-pack's static gradient/HeroVideo. Actually creates a probe
 * context in a try/catch — some browsers expose the constructor but refuse
 * a context (blocklisted GPU, headless, etc.), and that must read as "no".
 */
...
/**
 * WebGPU availability — existence check only (`navigator.gpu`). Requesting
 * an actual adapter is async and belongs in the renderer factory, not here.
 * Used by the Phase 7 WebGPU renderer path with automatic WebGL2 fallback.
 */
