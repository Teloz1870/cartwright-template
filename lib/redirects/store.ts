...
export async function createRedirect(
  ...
  if (fromPath.length < 2) return { ok: false, error: "A source path is required." };
  if (!/^https?:\/\//i.test(toPath) && !toPath.startsWith("/")) {
    return { ok: false, error: "The destination must start with / or http(s)://" };
  }
  if (fromPath === toPath) return { ok: false, error: "Source and destination must not be the same." };
