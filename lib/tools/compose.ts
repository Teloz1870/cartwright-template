  ...
  handler: async (args, ctx) => {
    ...
    const check = (key: string | null, kind: "header" | "footer") => {
      ...
      if (!isChromeSelectable(meta, activeSlug)) {
        throw new Error(
          `"${key}" is a locked-theme chrome that only renders on the "${meta.designSlug}" design (active design: ${activeSlug ?? "none"}). Pick a mixable chrome or switch design first.`,
        );
      }
    };
    ...
    await withAudit(
      ...
      async () => {
        await prisma.brandingSettings.upsert({
          where: { id: 1 },
          update: { chromeJson },
          create: { id: 1, storeName: "Cartwright", heroImage: "", announcement: "", chromeJson },
        });
      },
    );
    invalidateThemeCache();
