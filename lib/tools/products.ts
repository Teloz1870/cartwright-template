...
import { attributeMapSchema } from "@/lib/validation";
...
const createShape = z.object({
  ...
  // Same rule as the admin form (lib/validation.ts) — text, number, true/false
  // or a list of those, reserved keys refused. This was `Record<string,string>`
  // with no key guard, so the agent surface was the ONLY write path that could
  // not store the engine's own coffee seed (`roast: 2`, `notes: [...]`).
