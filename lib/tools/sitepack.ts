...
async function exportLiveSite(): Promise<{ cartpack: Buffer; report: ExportReport; designSlug: string; name: string }> {
  ...
  // Through the sovereign resolver, not the raw row: this name is also the
  // input to stableSitePackId(), so a contaminated row would have changed the
  // pack's identity AND its dedup key. Under the default policy this resolves to
  // exactly what the row said, so today's IDs are unchanged.
  const storeName = sovereignStoreName(
    typeof brandingRow.storeName === "string" ? brandingRow.storeName : null,
  );
