...
import { findFirstPublishedPageBySlugs, listPublishedPageSummaries } from "@/lib/public-pages";
 ...
 *
 * It governs the announced URL too. An unclamped locale published a path the
 * shop does not route (`/de/about` on a `["da","en"]` shop), and an omitted one
 * published a locale-less `/about` that only exists as a redirect.
 */
...
export const listPublicPages = defineTool({
  ...
  handler: async ({ locale }) => {
    ...
    // Two published rows can share one canonical route: a shop that holds BOTH
    // `about` and the legacy `om-os` (an import into a Danish seed does this).
    // The route renders whichever comes first in `publicPageSourceSlugs`, so
    // only that row may be announced — the other one's URL would show content
    // that is not its own.
    const byRoute = new Map<string, ReturnType<typeof summarise>>();
    function summarise(page: (typeof pages)[number]) {
      return {
        slug: page.slug,
        title: translatedField(page, "title", readLocale, page.title),
        metaDescription: page.metaDescription ?? null,
        updatedAt: serializedUpdatedAt(page.updatedAt),
        // The same helper `llms.txt`, the sitemap and the trust-route redirect
        // use, so a legacy CMS slug is announced as the canonical route it
        // redirects to instead of costing every agent an extra hop.
        url: canonicalPublicPagePath(page.slug, readLocale),
      };
    }
    // The route resolves its sources by EXACT match, so an exact source always
    // outranks a case variant that merely canonicalises to the same URL —
    // otherwise `About` (rank -1 under a naive indexOf) would displace the
    // `about` row the route actually renders.
    const routeRank = (slug: string) => {
      const sources = publicPageSourceSlugs(slug);
      const exact = sources.indexOf(slug);
      if (exact !== -1) return exact;
      const insensitive = sources.indexOf(slug.toLowerCase());
      return insensitive === -1
        ? Number.MAX_SAFE_INTEGER
        : sources.length + insensitive;
    };
    for (const page of pages) {
      const summary = summarise(page);
      const held = byRoute.get(summary.url);
      if (!held || routeRank(page.slug) < routeRank(held.slug)) {
        byRoute.set(summary.url, summary);
      }
    }
...
export const getPublicPage = defineTool({
  ...
  handler: async ({ slug, locale }) => {
    // Resolve the alias set the canonical route resolves. `list_pages`
    // announces `/{locale}/about` for a row whose slug is `om-os` (every
    // shipped seed uses that slug), so an agent that reads the URL and asks
    // for `about` has to reach that row instead of falling through to the
    // synthesised generic text.
    const page = await findFirstPublishedPageBySlugs(publicPageSourceSlugs(slug));
    if (page) {
    ...
    // Same clamp as the published path above: the synthesised legal text is
    // this shop's own content, so an absent locale means the shop's language —
    // not English, which was only ever the engine's second locale.
    // The canonical target, not the requested alias: `/{locale}/info/om-os`
    // redirects to `/{locale}/about`, which renders this synthesised text — so
    // `get_page {"slug":"om-os"}` must not answer `found:false` while the URL
    // it was announced under shows content.
    const fallback = getDefaultLegalContent(
      publicPageSourceSlugs(slug)[0] ?? slug,
      readableLocale(locale),
    );
    if (fallback) {
      return {
        slug,
