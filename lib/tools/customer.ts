...
const tryDiscountInput = z.object({
  // `.trim()` BEFORE `.min(1)`. Same defect class as lib/tools/discounts.ts,
  // one step removed: the normaliser used to live at the call site, so the
  // minimum was measured on the raw string. A whitespace-only `" "` therefore
  // passed and looked up the empty code — which a shop can actually hold, the
  // pre-fix `discounts.create` minted one from `"   "`. This is the shopper-
  // facing `cart:write` tool, so that lookup returned a real discount.
  // `.toUpperCase()` lives here too so the lookup key is normalised in ONE
  // place — the schema — and every consumer of the parsed input agrees.
...
export const getCartSummary = defineTool({
  ...
  handler: async () => {
    ...
    const lines = cart.items.map((i) => ({
      ...
      lineTotalDkk: (i.variant?.priceDkk ?? i.product.priceDkk) * i.quantity,
...
export const tryApplyDiscount = defineTool({
  ...
  handler: async (args) => {
    ...
    // The schema already trimmed + uppercased — parse output IS the lookup key.
