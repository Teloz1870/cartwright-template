...
import { sectionEffectSchema } from "@/lib/builder/effects";
...
export const pagePlanNodeSchema = z.object({
  ...
  /**
   * Optional scroll-driven motion effect (PART 4). Whitelisted enum (the model
   * may only pick a performant, on-brand, reduced-motion-safe value) → threaded
   * onto the layout node and rendered as a .motion-* class. Additive/optional →
   * existing plans still parse.
   */
  effect: sectionEffectSchema.optional(),
});
