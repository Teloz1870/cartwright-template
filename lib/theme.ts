...
export async function getActiveDesign(): Promise<DesignPack | null> {
  ...
  try {
    ...
    designCache = { value: fallback, expiresAt: now + CACHE_TTL_MS };
    return fallback;
  }
}
...
let chromeCache: { value: ChromeConfig | null; expiresAt: number } | null = null;
...
export async function getActiveChromeConfig(): Promise<ChromeConfig | null> {
  const now = Date.now();
  if (chromeCache && chromeCache.expiresAt > now) return chromeCache.value;
  try {
    const row = await prisma.brandingSettings.findUnique({
      where: { id: 1 },
    ...
    chromeCache = { value: config, expiresAt: now + CACHE_TTL_MS };
    return config;
  } catch {
    // DB down (or the chromeJson column not yet pushed): no chrome override.
    chromeCache = { value: null, expiresAt: now + CACHE_TTL_MS };
    return null;
  }
}
