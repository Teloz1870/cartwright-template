...
function getKeyPepper(): string {
  ...
  if (!pepper) {
    throw new Error(
      "AUTH_SECRET is missing - required as pepper for API key hashing",
    );
  }
  return pepper;
}
...
export async function authenticateApiKey(
  ...
  // expiresAt har altid været i schemaet men blev aldrig håndhævet — en
  // udløbet key var funktionelt evig. Håndhæv den nu: udløbet ⇒ samme 401-
  // klasse som revoked (ingen info-leak om hvorvidt keyen findes/fandtes).
  if (key.expiresAt && key.expiresAt.getTime() <= Date.now()) {
    return { error: { status: 401, body: { error: "API key expired" } } };
  }
...
export function apiErrorResponse(error: ApiAuthError): Response {
  // ok:false so an agent can branch on a single `body.ok` check across every
  // error shape (auth/404/400/422/500), matching the {ok:true,result} success
  // envelope — auth errors previously shipped {error} with no ok field.
  return Response.json({ ok: false, ...error.body }, { status: error.status });
}
