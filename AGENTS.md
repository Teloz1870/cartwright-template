...
**Don't build your own inquiry table — the engine ships one.** An AI building a
custom site on v0.54.0 reached for a new `Lead` model because nothing in these
rules files mentioned the existing one. The rebuild would have been waste.

- Model `Lead` (`prisma/schema.prisma`) · public `POST /api/inquiries` · inbox
  `/admin/leads` · server action `app/actions/lead.ts` (built-in fields only —
  it does NOT carry `source`/`data`) · no-DB site-profile
  variant `app/api/inquiries/route.static.ts` (mails the owner instead).
- Built-in fields: `name`, `email`, `phone`, `company`, `message`.
- **Custom** forms add two: **`source`** (which form — `"hegnsberegner"`) and
  **`data`** (a JSON object of whatever you collected: m², model, postcode).
  Both render in `/admin/leads`.
...
await fetch("/api/inquiries", {
  ...
  body: JSON.stringify({
    ...
    data: { meters: 42, model: "Type 1" },
  }),
});
```

Bounds (public, unauthenticated endpoint): `source` ≤ 64 chars; `data` an object
with ≤ 50 keys and ≤ 4 KB serialised. Over a cap is a `400`, never a silent
truncation. Keys OUTSIDE `data` are stripped by Zod — put custom fields inside it.
...
Optional flags: `leadAiTriage`, `contactAttachments` (both default off).
