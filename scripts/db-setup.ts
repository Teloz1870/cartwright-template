...
/**
 * Postgres is an explicit opt-in (`DATABASE_DRIVER=postgres`, mirroring
 * lib/db.ts:isPostgresDriver) or a `postgres://` URL. The libSQL fallback is
 * SQLite/Turso-only, so the Postgres path skips it entirely (see setupPostgres).
 */
function isPostgres(): boolean {
  if (cleanEnv(process.env.DATABASE_DRIVER) === "postgres") return true;
  return /^postgres(ql)?:\/\//i.test(cleanEnv(process.env.DATABASE_URL) ?? "");
}
...
function tryDbPush(): boolean {
  ...
  const res = runPrisma(["db", "push"], { capture: true });
  if (res.ok) {
  ...
  // A usage / unknown-flag error is OUR bug, not the schema-engine flake — fail
  // LOUDLY so it surfaces in testing/CI instead of being silently masked by the
  // fallback (which is exactly how an invalid `--skip-generate` once slipped by).
  if (/unknown or unexpected option|unknown option|unexpected argument|invalid option/i.test(trimmed)) {
    fail(
      "`prisma db push` was called with an unsupported option — this is a bug in scripts/db-setup.ts,\n" +
        `not a Prisma schema-engine flake. Fix the invalid flag. Prisma said:\n${trimmed}`,
    );
  }
  warn(
...
/** Run the seed (fresh DB only) and print the final login guidance. */
function seedAndReport() {
...
/**
 * Postgres path. Postgres uses Prisma's pg engine (no SQLite schema-engine
 * flake), so `prisma db push` is reliable and authoritative — we surface its
 * error directly rather than attempting the SQLite/Turso-only libSQL fallback.
 */
async function setupPostgres() {
  info("Postgres target — applying schema via `prisma db push` (the libSQL fallback is SQLite/Turso-only).");
  const r = runPrisma(["db", "push"], { capture: false });
  if (!r.ok) {
    fail(
      "`prisma db push` failed against Postgres (the error is shown above). The libSQL fallback does not\n" +
        "apply here. Check DATABASE_URL, and that prisma/schema.prisma has datasource.provider = \"postgresql\"\n" +
        "(run `prisma generate` after switching the provider).",
    );
  }
  ok("Schema applied via `prisma db push` (Postgres).");

  // Seed only a fresh DB. Count via the app's own Prisma client (the pg adapter
  // works here; the libSQL client does not). Best-effort: if the count can't be
  // read, treat the DB as fresh and seed.
  let users = 0;
  try {
    const { PrismaClient } = await import("../app/generated/prisma/client");
    const { PrismaPg } = await import("@prisma/adapter-pg");
    const prisma = new PrismaClient({
      adapter: new PrismaPg({ connectionString: cleanEnv(process.env.DATABASE_URL) ?? "" }),
    });
    try {
    ...
    users = 0; // unknown — treat as fresh first-run
  }

  if (users > 0) {
    info(`DB already has ${users} user(s) — skipping seed (no data touched).`);
...
async function main() {
  ...
  // Postgres uses a different Prisma engine (no SQLite schema-engine flake), so
  // it gets its own path and never touches the SQLite/Turso libSQL fallback.
  if (isPostgres()) {
    await setupPostgres();
    return;
  }
  ...
  if (alreadyInitialized) {
    ...
    if (!forceFallback) {
      const r = runPrisma(["db", "push"], { capture: true });
