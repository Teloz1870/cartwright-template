...
export function auditProfile(
  ...
  const MATERIALIZER_DELETED = [
    ...
    "prisma.config.ts",
    // Read, never repeated: `scaffold/site-pruned-scripts.ts` is the one list,
    // and the CLI reads the same thing out of scaffold/manifest.json. It used
    // to be typed out again here, so this audit could assert a pruning the CLI
    // did not perform.
    ...SITE_PRUNED_SCRIPTS,
  ];
